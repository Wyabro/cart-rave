...
export const CONFIG = {
  ...
  postFx: {
    // * Phase 1 isolation (docs/planning/handover-postfx-black-frames.md).
    // * ONE letter at a time → hard-refresh → human playtest 2–3 min.
    // * Override without edit: ?postfx=A (or B/C/D/E). Do not thrash lights/exposure.
    // *
    // * A HalfFloat composer RTs, bloom OFF, OutputPass ON
    // * B HalfFloat composer RTs, bloom ON (stock HalfFloat mips), OutputPass OFF
    // * C UnsignedByte composer, bloom OFF, OutputPass ON
    // * D UnsignedByte, bloom ON (byte mips), pre-tonemap order (bloom → Output), HDR knobs
    // * E UnsignedByte, bloom ON (byte mips), post-tonemap order (Output → bloom), display knobs
    isolationTest: "A",

    // * Renderer exposure — lower keeps diffuse surfaces subdued while emissive neon blooms.
    // * HalfFloat-era target: 0.4. Do not crush further until pipeline/RT type is decided.
    ...
    // * HalfFloat-era / pre-tonemap HDR bloom (good grade reference when path has headroom).
    bloom: {
    ...
    // * Display-referred bloom (OutputPass → bloom on 0..1). Used by isolation test E only.
    bloomDisplay: {
      strength: 0.3,
      radius: 0.32,
      threshold: 0.58,
    ...
    vhs: {
      amount: 0.28, // unitless — master 0..1 applied while The Storerooms is loaded
      // Modest grain — high noise + dark carpet + bloom read as sparkles, not CCTV.
      noise: 0.05, // unitless — luminance-only tape-noise floor amplitude (scaled by amount)
