 ...
 * @property {(renderer: import("three").WebGLRenderer) => void} [prepareComposerFrame]
...
export function updateVisualsAndEffects(deps, frameCtx) {
  ...
  // * Reset viewport/scissor/RT so a dirty pass never leaves half-frame pure black.
  // * (Imported lazily-safe via deps to avoid circular scene↔frameVisuals imports.)
  deps.prepareComposerFrame?.(deps.renderer);
