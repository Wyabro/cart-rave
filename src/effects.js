...
import { disposeObject3D } from "./effects/meshHelpers.js";
import {
  clearAmbientDust,
  setAmbientDustStyle,
  spawnTrashBurst,
...
// * Ambient dust / trash debris state moved to src/effects/ambientParticles.js
// * (EFFECTS-SPLIT-1). `sceneRef` stays here until ram-boost streaks move out.
...
export function initEffects(scene, options = {}) {
  ...
  // * Trash pool + ambient dust system live in ambientParticles.js; this keeps the
  // * composition order load-bearing: pools must exist before levelOrchestration reads them.
  ...
  return { ramBoostStreaks, ambientParticles: getAmbientParticles() };
}
