...
export function buildNetcodeGameBridge(getContext, session, onFirstHello = null) {
  return {
    ...
    onHostDemoted: () => getContext()?.onHostDemoted?.(),
    ...
    onRemoteNpcChargeStart: (cart) => getContext()?.onRemoteNpcChargeStart?.(cart),
    onRemoteBoostStart: (cart, options) => getContext()?.onRemoteBoostStart?.(cart, options),
    ...
    onLocalSpillCredit: () => getContext()?.onLocalSpillCredit?.(),
