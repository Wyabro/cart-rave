...
| Pri | Item | Notes |
|-----|------|-------|
| High | NET-LAG-1 — Friends/QP lag + rubber-band before playtest | **Wave 1 landed 08-20. NET-LAG-1-PT-1 PASS 09-14** (trail gone). **2-PC FAIL 09-22:** direct srflx path RTT 200–217 ms average / 489 ms p95; active ack ~241 ms; host queue ~46 ms; 939 replay drops, 33.6 m max correction, 7 teleports. Root cause: the 12-step CPU cap deleted the live newest unacked inputs on every reconcile. Local wave 2 fix selects oldest 12 without mutation, retains 60 frames (~1s), and separates deferred-work from true-loss diagnostics. Replay cost stays 12. **NET-LAG-PT-2** owed after ship. Do not lower the host jitter buffer, restore lerp, or increase replay steps without new evidence. |
