...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 queued | 08-16 audit: **INPUT-LOCK-1** · **SD-WIN-CREDIT-1** |
...
| **3** — WYATT LANE (blocked on you) | 👤 ongoing | **LAST-STANDING-DEAD-1** (revive-vs-delete call) · **PATTERNS-FOIL-PT-1** · SHIP-1 D-tier · **WARM-QP-ROTATE-PT-1** (after ship) |
| **4** — PERF RESIDUAL (measure-first) | 🟡 queued | **WARM-QP-ROTATE-1** · WARM-SOLO-1 · PERF-WATCH-1 · NET-PERF-1 / NET-PERF-3 |
| **5** — SWEEP (cheap Lows) | 🟡 queued | MOTION-A11Y-1 · COUNTDOWN-QUICKPLAY-1 · COUNTDOWN-LEAK-1 · CART-HUE-RED-1 · 08-16 audit Lows (10) |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 29 | 2 | 12 | 14 (+1 partial) |
| [Art](#art) | 1 | 0 | 0 | 1 |
| [Audio](#audio) | 1 | 0 | 0 | 1 |
| [Design / Gameplay](#design--gameplay) | 3 | 0 | 2 | 1 |
| 🟢 [Playtest owed](#playtest-owed) | 3 | 1 | 1 | 1 |
...
more engineering. Open owed checks: **PATTERNS-FOIL-PT-1** (prod) · **WARM-QP-ROTATE-PT-1**
(after ship) · **SHARD-PT-2** (launch day). Closed PASS history lives only in
[completed-work.md](./completed-work.md).
...
1. **INPUT-LOCK-1** — pre-GO input lock: host boost charges during countdown + stale remote inputs drive carts through the next countdown (one fix point).
2. **SD-WIN-CREDIT-1** — guests get zero Sudden Death win credit online (Clutch Winner / redMirror host-only).
...
2. **THOST-CEILING-1** — host-clock subsystem dead in production (one-constant fix).
3. **NPC-ABORT-BURST-1** — NPC charge-abort burst bypasses self-KO safety gates.
4. **ZOMBIE-HOST-PICK-1** — zombie connections eligible for migration host pick.
5. **GAMEPAD-FREEZE-1** — frozen gamepad input drives a hidden host's cart.
6. **SD-SPECTATOR-CHARGE-1** — SD spectator phantom charge loops SFX up to 45s.
...
- **PATTERNS-FOIL-PT-1** — foil on earned patterns; Honeycomb and Diamond stay lit `[solo]`.
...
| Pri | Item | Notes |
|-----|------|-------|
| High | INPUT-LOCK-1 — pre-GO input lock has three disagreeing gates: host can charge boost during countdown; stale remote inputs drive carts through the next countdown | **Filed 08-16 from the gameplay bug audit (read-only, adversarially reviewed).** The "input locked outside running" contract is enforced in three layers that disagree. (a) `runFixedPhysicsStep`'s countdown gate zeroes only `forward`/`turn` while its own comment claims nitro is locked too — `axis.boostHeld` passes through ([simulation.js](../../src/simulation.js) ~3926-3931), `getAxis()` is phase-ungated ([input.js](../../src/input.js) ~491-522), and `triggerRamBoost` has no phase check — so the host (and solo) can hold boost from "3", auto-release a full 28-impulse burst mid-countdown, and have a second full burst ready at GO. Non-hosts can't (their input path only runs while running) — host-only first-hit advantage in MP. (b) The host's remote-input branch applies `throttle/steer/nitro` with **no phase gate** (simulation.js ~3943-3958), and `remoteInputsByConnId` is never cleared at round boundaries (only session reset / host promote / migration / init; `resetClientPredictionState` doesn't touch it) — a non-host holding W (or boost) at round end has that stale input drive their cart off the booth during the next 3-2-1 on the host sim: free head start or free self-KO, every rematch, no mod needed; stale `nitro:true` also pre-charges their cart through the same gap. One fix point: gate all three axes + the remote branch on `phase === "running"`, and clear the remote map at round boundaries. Not a reopen of COUNTDOWN-QUICKPLAY-1 / COUNTDOWN-LEAK-1 (different mechanisms). |
...
| Medium | LAST-STANDING-DEAD-1 — Last Cart Standing is dead end-to-end: flourish unreachable, verdict + Sole Survivor challenge + server `lastStanding` branch never fire | **Filed 08-16 from the gameplay bug audit (adversarially reviewed).** Traced every SD resolution branch: ram-kill with one tied survivor, self-fall with one survivor, leader-self-fall fallback, and simultaneous wipeout all call `addScore` synchronously, which ends the round before the live-phase re-check can ever see `aliveOnArena === 1` — `scheduleLastCartStandingFinish` is effectively unreachable (stuck-respawn is instant via `doRespawn`, and disconnects convert to NPC, so no lone-survivor window exists). On top of that, `endRound`'s SD branch overwrites `endReason:"lastStanding"` with `"timer"` (roundLifecycle.js ~927-945; the `else if` that sets it is dead code — every slot-passing caller runs during SD). Dead as a result: the "LAST CART STANDING" verdict text (~518-521), the `LAST_STANDING` / Sole Survivor challenge (~429-431), and the server's `lastStanding` validation branch ([roundValidation.ts](../../../party/roundValidation.ts) ~217-242). **Blocked on Wyatt: revive-vs-delete call before this is actionable** — revive the flourish path (and fix the overwrite) or delete the feature end-to-end. Not a reopen of NET-SD-1. |
...
STOREROOMS-NPC-SELFKO-PT-1, STOREROOMS-NPC-SELFKO-PT-2.
