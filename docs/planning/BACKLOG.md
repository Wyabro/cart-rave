...
| High | HYGIENE-1 — acked fixed-list sweep (07-30 review fold-in) | Exactly 4 items, one commit each, off-list = out of scope: (1) `vite.config.js:98` `sourcemap: false` — `.map` files publicly served today; (2) widen `index.html:1126` boot-error filter — forward when `ev.error` present (module-eval crashes currently reach zero telemetry); (3) GitHub default branch → `cart-clash` (settings change, Wyatt/manual) + delete remotes `redesign/fight-night-ui` (merged PR #3), `vercel/install-vercel-web-analytics-szsph5`, `vercel/vercel-web-analytics-to-projec-npj24h`; KEEP `next-level`, `docs/agent-config-rewrite`; remote deletes never share a commit with code; (4) `tools/perf-profile.mjs` `--dpr` + GPU flags (DPR hardcoded 1 — the High tier's `pixelRatioCap: 2` has never been measured; prereq to ANY tier retuning). |
...
| High | SEC-UNLOCK-1 — DEV-gate the `?devUnlocks` URL one-shot | Before external testers. `src/stores/unlockStore.js:22-31` runs ungated in prod — a shared `?devUnlocks=all` link permanently unlocks everything. Gate the URL one-shot behind `import.meta.env.DEV`; keep manual localStorage override (documented intent); fix the false "manual override only" header claim. |
| High | SEC-ROUTE-1 — Worker routes `includes()` → `startsWith()` | Before external testers. `party/index.ts:1555/1579/1614/1728` match on `pathname.includes(...)` — any path containing the substring routes there; `:1777` shows the correct `startsWith` pattern. Add route-match tests. |
...
| Medium | CARGO-RACE-1 — bay built empty if grocery GLTFs lose the load race | `createCargoBay()` snapshots `loadedGeometries` ONCE at cart creation; `GroceryPool.init` is deliberately non-blocking (main.js:2826). A play entry before init resolves → permanently empty bay until the next KO-respawn rebuild. Reproduces 100% on cold headless `?room=solo` (blocked CARGO-VIS-1 evidence); real-world window = cold-cache fast play entry. Fix sketch: bay self-heals on init resolve (re-run item build for live bays), mirroring the pendingSpills replay pattern. |
...
| High | CARGO-HUD-1a — cargo-readout mock on BOTH hosts | Static 3-state styling only (stripped/stocked/boss — locked 07-30), no wiring, no netcode: (a) CSS2D nameplate (`makeNameLabel` main.js:3655, `nametagHtml` hud.js:112); (b) score-strip indicator slot (`syncRowIndicators` hud.js:917, `buildRosterRows`/`updateScores` :966/:985). Screenshots → Wyatt picks the host → CARGO-HUD-1 gets framed. |
| High | CARGO-HUD-1 — opponent cargo readout (real card) | Display-only, plumbing verified: `lifeCargoPoints`/`cargoFullness01` replicated as `lc` on BOTH wire paths (netcode.js:1775 send / :1267 apply; binary.js:117/:248); zero cargo refs in hud.js today. The gap: boss incoming-ram 0.52× vs stripped 1.32× — the harder-to-see half carries the bigger physics swing. **Blocked on the 1a pick.** Full brief: [CARGO-HUD-1-handover.md](./CARGO-HUD-1-handover.md). |
