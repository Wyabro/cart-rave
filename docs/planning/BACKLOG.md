...
more engineering. Open owed checks: **BOOT-TBT-PT-1** · **PODIUM-DOUBLE-CREDIT-PT-1** · **MENU-ARROW-PT-1** · **SPILL-DOUBLE-VFX-PT-1** · **SHARD-PT-2** (launch day). Closed PASS history lives only in
[completed-work.md](./completed-work.md).
...
3. 08-16 audit Lows, top first: **HOST-PRESENT-ORDER-1** · **ZOMBIE-ROOM-RESET-1** · **DEMOTE-COUNTDOWN-1**.
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | BOOT-TBT-PT-1 — Classic still loads after boot yields `[solo]` | **Owed: Wyatt playtest — BOOT-TBT-PT-1 — Classic menu stays live, preview swaps, and Solo still starts.** Hard-refresh first.<br>1. Sit on the main menu with Classic selected. After a few seconds the buttons must still click. FAIL if the menu freezes for several seconds.<br>2. Use the arena arrows: Classic to another arena, then back to Classic. FAIL if the floor stays black or the swap never finishes.<br>3. Press SOLO on Classic. FAIL if the overlay hangs or the record never appears. PASS if the round starts and the floor looks normal. |
| Low | PODIUM-DOUBLE-CREDIT-PT-1 — host YOUR STATS tick once per MP round `[2pc]` | **Owed: Wyatt playtest — PODIUM-DOUBLE-CREDIT-PT-1 — finish a Quickplay or Friends round as host; played/wins tick once.** Hard-refresh first (prod `0211a408`).<br>1. Host a Friends or Quickplay match and finish one full round.<br>2. On the host machine, open YOUR STATS and Recent matches.<br>3. FAIL if played or wins went up by two, or if Recent matches shows two rows for this one round. PASS if played/wins tick exactly once and Recent matches shows one entry. |
