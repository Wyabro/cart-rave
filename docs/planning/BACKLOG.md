...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **NET-LAG-1-PT-1** parked · **ONBOARD-WEBP-PT-1** retest on prod `e5ca329b` |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | AUDIO-RAM-IMPACT-PT-1 — ram impacts are clearly audible `[1pc]` | **Owed: Wyatt playtest — AUDIO-RAM-IMPACT-PT-1 — cart-vs-cart rams make a clearly audible crash at default volume sliders, AND gentle bumps now thud quietly instead of silence.** Parent **AUDIO-RAM-IMPACT-1** (`crashVolumeFloor` 0.25→0.45, curve ×0.7→×1.0; cap-362/363 proved the path fired but sat at ~0.24 effective gain. Round 2 same day: sub-minSpeed touches now fire a quiet contact tap — `playContactTap` via `playSoftContact` — since ALL crash SFX sat behind the scoring gate; soft taps never reached sound at all). Prod `67778a9f` (Worker version `67778a9f-786c-4fec-aed9-2f2aa9f48e92`, commit `0644094f`; hard-refresh. VERIFY_OK attempt=1 entry `/assets/index-DF2aXsU8.js` refs=28 zero-404; `playSoftContact` symbol in live `gameBoot-BSDH5sml.js`, tap volume `.12+.22*e` in live `audioManager-YtSyOQv9.js`).<br>1. Start a round (solo vs NPCs fine). Ram carts hard at several speeds incl. a boost ram → loud crash.<br>2. Then gently bump/drift into a cart below ram speed → quiet plastic tap, no silence.<br>3. FAIL if any hard ram is inaudible over music, soft taps are silent or annoyingly loud, or gentle bumps award hit credit they must not.<br>4. Press F8 after a few rams. |
