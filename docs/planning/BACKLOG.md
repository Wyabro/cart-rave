...
| **2** — PRE-SHIP (before public post) | 🟡 queued | ONBOARD-ART-1 · VOICE-BUS-1 · CHUNK-DEFER-1 · NPC-BOOST-1 (measure) · UI-FRAME-1 + ESC-PANEL-1 · PAD-MENU-1 · KILLFEED-PHONE-1 · ARENA-BUMPER-HINT-1 (after product call) |
...
5. **UI-FRAME-1 + ESC scoring panel** `[SHIP-1 E1]` look pair.
6. **Controller menu navigation polish** *(pre-ship 07-19 residual)*.
7. **KILLFEED-PHONE-1** — kill feed awkward/overlapping on phone.
8. **ARENA-BUMPER-HINT-1** — menu advertises LB/RB arena; nothing reads bumpers. Product call first (wire or drop the hint).
