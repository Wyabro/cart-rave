...
| Low | ZAN-BOLLARD-PT-1 — Sundial bollard/gnomon impact clang `[solo]` | **Owed: Wyatt playtest — ZAN-BOLLARD-PT-1 — bumping a Sundial corner bollard or the center gnomon blade makes the wall-clang impact FX/sound (was silent).** ZAN-BOLLARD-CLASS-1 (classification-only) re-registered the deck bollards + gnomon as "edge" surfaces, matching booth legs. **FAIL 08-13 (silent) — three fixes. Fix 1:** the edge Δv subtracted the pre-step snapshot from itself (always 0) — now samples post-step body velocity. **Fix 2:** a real-Rapier probe measured 1.6–1.7 m/s per-step Δv at any approach speed (solver spreads the impulse), so the old 2.5 m/s threshold was unreachable — retuned to 0.75 threshold + `edgeIntensityRange: 6` (floor curve untouched). **Fix 3 (audibility):** the F8 capture (cap-362) proved the impacts fire at intensity 0.4–1.0 yet the sound was still inaudible — the edge clang played the quiet "floor" sample. Tried the hop thump as a stopgap (cap-363: 123 `impact_play` events with valid Howler ids, context running, assets byte-identical — engine-side playback proven, still inaudible to the listener). **Fix 4:** Wyatt supplied `56254__qk__metal_04.wav` → converted to `clang.opus` (48 kHz mono opus, peak −1.6 dB, 0.14 s) — the edge clang now has its own metallic sample. **Fix 5 (over-triggering):** the clang was firing on the whole "edge" class (pit wall + booth legs + posts). The Sundial deck posts (8 corner bollards + gnomon) now have their own `bollardColliderHandles` → new `clang` classification (sound + spark); booth legs / pit wall stay `edge` = FX only, no sound. Impact plays record `sim/impact_play` with howlState/howlerState/ctxState. Spark projection fix included (inboard contacts stay at the post). Redeployed for retest; prod after hard-refresh.<br>1. Sundial solo, drive into a corner bollard at speed — you should hear/see the impact clang, and the spark should appear at the post, not floating over the pit.<br>2. Do the same against the tall center gnomon blade.<br>3. FAIL if either post is silent, the spark is clearly in the wrong place, or the KO round never completes. PASS if both posts clang and the round plays normally. |
