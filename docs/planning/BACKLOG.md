...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Classic + Sundial PASS · Storerooms shipped `9051a0ce`, PT owed · **CART-POP-PT-1** open until Storerooms F8 · **NET-LAG-1-PT-1** parked |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Classic PASS 08-20** Wave I Worker `5360f482`. **Sundial PASS 08-20** prod `bb29c13b` Worker `3f3e5fbd`. Do not retouch Classic or Sundial floors. **Storerooms shipped 08-21** prod `9051a0ce` Worker `dfa5a26d` (VERIFY_OK `index-BhgEDdEM.js`, live `backroomsSupermarket-veaqmbrG.js` has `FIX_INTERNAL_EDGES`). Isolated Rapier: 9-cuboid r16@24 pops=1; trimesh r16@24 pops=0. Chamfer hulls kept. No physics-knob change. **CART-POP-PT-1** open until Storerooms F8 PASS. Lane: **Critical**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | CART-POP-PT-1 — normal driving stays planted across all arenas `[1pc]` | **Owed: Wyatt playtest — CART-POP-PT-1 — Storerooms stay planted.** **Wyatt PASS 08-20 Classic** (Wave I `5360f482`) **and Sundial** (prod `bb29c13b`). Parent **CART-POP-1**. Storerooms on prod `9051a0ce` (Worker `dfa5a26d`). Hard-refresh. Still FAIL until Storerooms F8 PASS.<br>1. On production with `?diag=1`, drive ~30 s on Storerooms; straight runs and full-lock turns, no hop or boost.<br>2. FAIL if a cart pops off the flat floor without a ram, obstacle hit, or edge contact.<br>3. PASS if ordinary driving stays planted on Storerooms while rams and hits still have visible lift.<br>4. Press F8 once after the arena. |
