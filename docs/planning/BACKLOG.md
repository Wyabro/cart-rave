...
| **3** — WYATT LANE (blocked on you) | 👤 ongoing | HIT-SFX-VAR-1 clips · ANNOUNCER-RERECORD-1 · CARGO-BAY-INSTANCE-1 stability · SHIP-1 D-tier cut/keep |
...
| Pri | ID | Item | Notes |
|-----|----|------|-------|
| Medium | LOOP-SAFETY-2 | Harden loop command and artifact boundary | **Wyatt-selected 08-11.** Replace model-provided PowerShell text with fixed host-defined read-only operations. Move loop state/results and DEV-GRAPH artifacts to one atomic per-run store. Test capability rejection, fixed argv, artifact isolation, and atomic writes. DEV-GRAPH-1 owns only the model-free state machine, lock, review schema, and synthetic traces. **Do not connect maker/reviewer stages:** DEV-GRAPH-2 needs five clean plan-only dry runs first. |
...
MENU-SWAP-FLASH-1.
ANLX-GEO-1, ANLX-PAGEHOST-1, ANLX-GLITCH-1, DEV-LOOP-1, DEPLOY-MAP-1, MENU-SFX-1, CAPTURE-RING-LIMIT-1, CARGO-LATCH-1, CARGO-LATCH-PT-1, NPC-BOOST-2, NPC-BOOST-2-PT-1, NPC-BOOST-1, AI-EASY-SOFTEN-1, BOOTH-RAIL-COL-1, SUNDIAL-LOW-WATER-1, UI-SCALE-P2-MEDIA-1, ORIENT-HINT-SCROLL-1, DEV-GRAPH-1.
