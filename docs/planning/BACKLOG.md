...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **NET-LAG-1-PT-1** parked · **ONBOARD-WEBP-PT-1** retest on prod `ae7d7a81` |
...
2. **ONBOARD-WEBP-1** — HOW TO PLAY WebP playback must loop (PT FAIL 08-21).
...
| High | ONBOARD-WEBP-1 — HOW TO PLAY animated WebPs fail on some PCs | **Shipped 08-21** `ae7d7a81` Worker `2a5af903-73f2-4278-96e6-2b658fbeeade`. VERIFY_OK `index-Im349Dd2.js`; live entry has `replaceWith` + `frame-change`. Files are valid infinite WebPs. Cause: Chromium stops looping after canvas `drawImage`. Lever: after a frame-change verdict, replace the sampled node with a never-sampled copy. **Owed: ONBOARD-WEBP-PT-1 on prod (hard-refresh).** Distinct from reduced-motion `still.webp` (ONBOARD-ART-1). Do not reopen **ONBOARD-SLIDES-1**. |
...
| Low | ONBOARD-WEBP-PT-1 — HOW TO PLAY media moves or falls back cleanly `[1pc]` | **Owed: Wyatt playtest — ONBOARD-WEBP-PT-1 — the HOW TO PLAY pictures loop when supported and stay clean when they cannot.** Parent **ONBOARD-WEBP-1**. Prod `ae7d7a81` (Worker `2a5af903`, hard-refresh) with `?diag=1`. **FAIL 08-21** before this ship: each clip played once (~2.8s) then froze.<br>1. Open HOW TO PLAY. Page through DRIVE, BOOST, RAM THEM OUT, READ THE HUD, and THE LIVING STORE.<br>2. FAIL if any picture is blank, broken, jumps in size, locks the menu, or plays once and then freezes. A clean still picture is an acceptable fallback only when motion cannot play at all.<br>3. PASS if each picture loops for as long as the slide stays open, or changes to a clean matching still within about a second; the teaching text and controls stay readable.<br>4. Press F8 after the five picture slides. |
| Low | SNAP-FINITE-PT-1 — snapshot finite guard changes nothing visible `[2pc]` | **Owed: Wyatt playtest — SNAP-FINITE-PT-1 — normal netplay is unchanged after every snapshot body write got a NaN guard (SNAP-FINITE-1).** Pushed but not yet deployed — `npm run dev:local` on both machines, or ship first.<br>1. Friends match, two machines. Drive, boost, ram, and hop for a full round on the non-host.<br>2. FAIL if any cart freezes in place, teleports to origin, ghosts through the floor, or remote carts stutter where they did not before.<br>3. PASS if remote carts track smoothly and collisions feel identical to pre-guard play.<br>4. Press F8 on both machines at round end. |
| Low | CID-AUTH-PT-1 — refresh mid-lobby keeps your seat `[2pc]` | **Owed: Wyatt playtest — CID-AUTH-PT-1 — refreshing the page mid-lobby re-seats you as the same player instead of dropping you to NPC.** Parent **CLIENT-ID-AUTH-1** (clientId hijack guard). Prod after ship, or `npm run dev:local` with two machines.<br>1. Two clients in one Friends room, both seated and ready.<br>2. Hard-refresh the HOST's page mid-lobby; let it rejoin.<br>3. FAIL if the refreshed client comes back as an NPC cart, loses its name/color, or the room strands hostless.<br>4. PASS if it re-seats under the same name with a human slot and the other client never sees a ghost. Press F8 at the end. |
| Low | RING-ALIAS-PT-1 — tab-away round trip shows no remote pops `[2pc]` | **Owed: Wyatt playtest — RING-ALIAS-PT-1 — hiding the non-host tab for ~10s mid-round then returning causes no visual pops or teleports on remote carts (RING-ALIAS-1 decode-ring generation guard).** Pushed but not yet deployed — `npm run dev:local` on both machines, or ship first.<br>1. Friends match, two machines; non-host drives mid-round.<br>2. Hide the non-host's tab for ~10 seconds, then return and keep driving.<br>3. FAIL if remote carts pop/teleport on return beyond a single brief catch-up snap, or the F8 shows `ringAliasFlushes` climbing during normal (non-hidden) play.<br>4. Press F8 on both machines at round end. |
