...
| 🟢 [Playtest owed](#playtest-owed) | 6 | 1 | 3 | 2 |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NIGHT-SHIFT-PICKER-1 — Night Shift selectable in the arena list | **Top 09-18 (Wyatt, new-window).** Follows NIGHT-SHIFT-CITY-2 PASS `47ad5d80`. Player: Solo and Friends host can pick Night Shift and play a round. Not Quickplay (that is **NIGHT-SHIFT-QP-1**). Not more city art (**NIGHT-SHIFT-DETAIL-1**). Today the menu builds `LEVEL_OPTIONS` from `ARENA_CATALOG.filter(quickplay)` in `src/ui/cart-rave-menu.js`, so rooftop is hidden even though it is in the catalog. `index.html` has no `data-level="rooftop"` card. Catalog `quickplay: false`, empty `music`, empty ambience, unlock hint "Blockout". `MULTIPLAYER_LEVEL_IDS` already allows rooftop. `FREE_LEVEL` stays `zanzibar`. Lever: show production arenas in the menu without flipping `quickplay` (keep `testArena` out). Add a Night Shift menu card. Assign a temporary existing music list and an existing ambience bed so Solo is not silent — do not author new audio. Change unlock hint off "Blockout". Tests: `tests/levels/arenaCatalog.test.js`, `arenaBrowse.test.js`, `arenaMenuOrder.test.js`, `friendsLevel.test.js`. Fail if Night Shift is missing from Solo/Friends pick, if it enters `QUICKPLAY_ARENA_IDS`, or if Test Arena appears. Done: Wyatt playtest — pick Night Shift in Solo, play a round. Lane: Standard. Seed a Playtest-owed row with `<br>N.` steps when it ships. |
| High | NIGHT-SHIFT-QP-1 — Night Shift in Quickplay rotation | **Top 09-18 (Wyatt).** Depends on **NIGHT-SHIFT-PICKER-1**. Player: a public Quickplay room can roll Night Shift. Files: `shared/arenaPool.js` `QUICKPLAY_ARENA_IDS`, `src/levels/arenaCatalog.js` `quickplay: true`. Catalog music + ambience bed must be non-empty (arenaCatalog test). Prefetch uses the same pool. Worker initial arena pool must match catalog order (QP-ORDER-1). Lane: **Critical** (shared arena pool / public rooms). Fail if Night Shift never appears in QP, if `testArena` leaks into MP, or if catalog quickplay ids drift from `QUICKPLAY_ARENA_IDS`. Done: Wyatt playtest — Quickplay can start on Night Shift. Do not ship without PICKER. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NET-LAG-PT-2 — high-RTT non-host stays visually stable `[2pc]` | **Owed: Wyatt playtest — NET-LAG-PT-2 — maximum visual stability with slightly softer input response.** Prod `8fa504ac`, Worker `7bb04c34-cda5-4777-bd49-15db25bfdd35`, entry `index-Bgsw9GFQ.js`. Use Friends on the same two machines that produced caps 396–399.<br>1. Hard-refresh https://www.cartclash.lol/ on both machines with `?diag=1&nettest=1`; host a Friends room and play on the non-host for two minutes.<br>2. Drive, turn, boost, ram, and recover from collisions while the host also moves.<br>3. Press F8 on both machines after any snap and again near round end.<br>4. FAIL if the non-host repeatedly teleports or loses current steering; fail evidence if `predictionHistoryDrops` is above 0. PASS only if the non-host path stays continuous and the softer response is playable. |
