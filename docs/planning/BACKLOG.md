...
| 🟢 [Playtest owed](#playtest-owed) | 13 | 0 | 3 | 10 |
| [UI / UX](#ui--ux) | 3 | 0 | 1 | 2 |
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | ANIM-BUGS-1 — animations.js entrance + lifecycle bugs | **Acked plan.** Six levers in one wave: (1) `animateMenuCardEnter` multi-target + real `stagger` indices, (2) dismiss `settled`/`then`/timeout finish, (3) `fadeIn` pointer-events cleanup only, (4) pointer capture in `wireButtonPressFeedback` + menu `wireMenuPressFeedback`, (5) clear HOW TO attract map on cancel, (6) cart pulse snap on interrupt + delete dead joystick helpers. **Files:** `src/animations.js`, `src/ui/cart-rave-menu.js` (press only). Behavior change → playtest seed. |
