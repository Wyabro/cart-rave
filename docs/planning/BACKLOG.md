...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave F caps 392–393 pulled 08-20.** Controls, restitution, wedge seams, and pitch/roll are not sufficient causes. Upright/no-spin carts reach `−8.718 → +1.43` at one flat-floor contact on the round-cuboid lower front corner; tangent impulse, solver friction, and solver restitution are zero. Candidate: support is lost before the normal rebound. **CART-POP-PT-1 remains FAIL/open.** Isolate the support-loss source before a physics change. Lane: **Critical**. |
