...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **NET-LAG-1-PT-1** parked · **ONBOARD-WEBP-PT-1** retest on prod `310a5f86` |
...
| High | ONBOARD-WEBP-1 — HOW TO PLAY animated WebPs fail on some PCs | **Shipped 08-22** `310a5f86` Worker `503680cd-0e3e-46de-80e8-a4d74e90fc8c`. VERIFY_OK `index-Be8iEYJL.js`; live entry has `onboardLoop`. Files are valid infinite WebPs. Cause: Chromium stops looping after canvas `drawImage`; same-URL remounts can reuse the frozen decoder. Lever: add a unique query to the never-sampled replacement. **Owed: ONBOARD-WEBP-PT-1 on prod (hard-refresh).** Distinct from reduced-motion `still.webp` (ONBOARD-ART-1). Do not reopen **ONBOARD-SLIDES-1**. |
