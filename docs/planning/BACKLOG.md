...
| **2** — PRE-SHIP (before public post) | 🟡 queued | **CUSTOMIZE-SPAM-1** unlanded; PT owed (**CUSTOMIZE-SVG-FLASH-PT-1** · **COUNTDOWN-HOST-STAMP-PT-1** · **FRIENDS-LOBBY-ORDER-PT-1**) |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 21 | 0 | 8 | 12 (+1 partial) |
| [Design / Gameplay](#design--gameplay) | 2 | 0 | 2 | 0 |
| 🟢 [Playtest owed](#playtest-owed) | 4 | 0 | 3 | 1 |
...
more engineering. Open owed checks: **CUSTOMIZE-SVG-FLASH-PT-1**
(prod) · **COUNTDOWN-HOST-STAMP-PT-1**
(prod) ·
...
1. **CUSTOMIZE-SPAM-1** — rapid Customize open/close remounts WebGL until the attract canvas dies (cap-370).
...
| Medium | CUSTOMIZE-SPAM-1 — rapid Customize open/close remounts WebGL and kills attract `[solo]` | **Filed 08-17 from cap-370** (RTX 4090, High, `cartclash.lol/?diag=1`, lobby, prod `748ea991`). Do not reopen **ATTRACT-JANK-1** (reduced-motion cadence) or **CUSTOMIZE-PERF-1** (steady-state 0.3 ms). Distinct from **CUSTOMIZE-SVG-FLASH-1** (cartoon SVG). Timeline: 10 `menuCartMount` in 54 s; longtask after almost every remount (max 1714 ms); `gpucontextlost` on the main attract canvas at t=43921; no `gpucontextrestored` by F8 at t=54515. Attract loop still ticks (`attractWindow` cheap, `attractCut` continues) but the game GL is dead. Each open disposes the menu cart and creates a Customize WebGL+PMREM; each close disposes that and remounts the menu cart. `mountCartPreview` also nulls the in-flight promise, so a stacked import can create a second Customize context. **Work from this capture.** Candidate: debounce menu-cart remount until Customize has stayed closed; add a generation token so in-flight mounts cannot stack. |
| Medium | CUSTOMIZE-SVG-FLASH-1 — DONE/apply flashes the legacy cart SVG `[solo]` | **Landed 08-17; open-path follow-up 08-17.** Close already hides before dispose. Open no longer paints `makeCartSVG` as a chunk-load placeholder — holder stays empty until 3D is ready; SVG only if CartPreview load/init throws. Playtest owed: **CUSTOMIZE-SVG-FLASH-PT-1** (retest after ship). |
...
| Medium | CUSTOMIZE-SVG-FLASH-PT-1 — DONE/apply does not flash the legacy cart SVG `[solo]` | **Owed: Wyatt playtest — CUSTOMIZE-SVG-FLASH-PT-1 — DONE and apply do not flash the old cart drawing.** Parent **CUSTOMIZE-SVG-FLASH-1**. FAIL 08-17: SVG flashed on open (close was clean). Retest after next ship. Hard-refresh.<br>1. Menu. Open Customize. Pick a color. Press DONE.<br>2. FAIL if a flat cartoon cart drawing flashes while the screen opens or closes. PASS if the 3D cart stays or the holder stays empty until the 3D cart appears.<br>3. Open Customize again. Change color. Stay on the screen. FAIL if the 3D cart turns into that flat drawing. PASS if the 3D cart updates in place. |
