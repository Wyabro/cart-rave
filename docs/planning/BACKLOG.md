...
2. **NET-LAG-1** — Friends/QP lag + rubber-band (F8 both machines; measure first). Wave 1 trail PASS; residual lag open (Wyatt note 09-14: non-host lag drags experience, looks amateur).
3. **RAM-ARREST-1** — silent non-host rams on solver-arrested contact (agent active; **SNAP-FINITE-PT-1** FAIL 09-14).
