...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **NIGHT-SHIFT-PICKER-1** then **NIGHT-SHIFT-QP-1** |
...
| [Art](#art) | 1 | 0 | 1 | 0 |
| [Design / Gameplay](#design--gameplay) | 3 | 2 | 1 | 0 |
| 🟢 [Playtest owed](#playtest-owed) | 7 | 2 | 3 | 2 |
...
**Block 1 — NOW (player-facing correctness / High).** Playtest blockers; do not start an external playtest until this block drains. One card at a time, top first. Wyatt 09-01: all six Crazy Games / prod-bug cards are High.
...
Landed 09-02 — **MENU-SHORTWIN-1** (short-desktop hero spacing; SETTINGS above hint bar at 1077×606); **MENU-SHORTWIN-PT-1** PASS (local Playwright on Vite :4000, 1077×606 slack +26 px).
Landed 09-02 — **CG-ZIP-1** (`npm run zip:cg` → `tmp/cart-clash-crazygames.zip`, 23.96 MB / 182 files; zip is gitignored). **CG-ZIP-PT-1** PASS (local Playwright on unzip + `lvh.me`; Wyatt delegated).
Landed 09-14 — **SITE-TRUST-1** (privacy, terms, contact, Leave Feedback under Controls, branded 404); **SITE-TRUST-PT-1** PASS (prod `c58c96ff`, Wyatt authorized).
1. **NET-LAG-1** — Friends/QP lag + rubber-band (F8 both machines; measure first). Wave 1 trail PASS; residual lag open (Wyatt note 09-14: non-host lag drags experience, looks amateur).
2. **RAM-ARREST-1** — silent non-host rams on solver-arrested contact (agent active; **SNAP-FINITE-PT-1** FAIL 09-14).
Landed 09-08 — **ONBOARD-COPY-1** (HOW TO PLAY AISLE 1+2 body matches live input mode); **ONBOARD-COPY-PT-1** PASS (prod `39010c93`).
...
**Block 3 — WYATT LANE.**
✅ drained 08-17 — **TRUST-1** · **LEADERBOARD-1** cut from V2 (Wyatt). **CG-COVERS-1** is High in Block 1 (Wyatt art, still you).
...
- Taste-gated Design: **CLUTCH-SLOMO-1** — only on new evidence or explicit pull-forward.
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NET-LAG-1 — Friends/QP lag + rubber-band before playtest | **Wave 1 landed 08-20. NET-LAG-1-PT-1 PASS 09-14** (trail gone). **2-PC FAIL 09-22:** direct srflx path RTT 200–217 ms average / 489 ms p95; active ack ~241 ms; host queue ~46 ms; 939 replay drops, 33.6 m max correction, 7 teleports. Root cause: the 12-step CPU cap deleted the live newest unacked inputs on every reconcile. **Wave 2 shipped `8fa504ac`, Worker `7bb04c34`:** select oldest 12 without mutation, retain 60 frames (~1s), separate deferred-work from true-loss diagnostics. VERIFY_OK `index-Bgsw9GFQ.js`, 28 refs, symbol live; prod Quickplay 6/6 + Friends 14/14. **NET-LAG-PT-2 FAIL 09-24:** caps 400/402 show 11/10 hard corrections up to 19.9/22.1 m, RTT 29/5 ms average, zero history drops, and 5/7 replay-budget events. Root cause of remaining divergence open. **Diagnostic trace shipped `820176d2`, Worker `e0573a74`:** live symbol, 28 refs, 0×404, prod Friends 14/14. Do not lower the host jitter buffer, restore lerp, or increase replay steps without new evidence. |
| High | RAM-ARREST-1 — silent ram on solver-arrested qualified contact | **Filed 09-14 from SNAP-FINITE-PT-1 FAIL** (silent non-host rams ~50%, no SFX). Lever: presentation fallback from pre-step closing when live post-collision reads ~0 (`simulation.js` `applyRammingImpulse` + `ramKnockbackForClosing` + `fireRamContactPresentation`). Physics (`pendingRam`), combo tier, host tail stay live-gated (AI-1). Tests: `tests/effects/optimisticLocalHitFx.test.js` +3. Retest: **SNAP-FINITE-PT-1** `[2pc]` + F8 both machines (owed — no F8 rig this wave). |
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | NIGHT-SHIFT-DETAIL-1 — denser Night Shift city/roof set dressing | **Filed 09-18 from NIGHT-SHIFT-CITY-2 PASS.** Wyatt: improvement pass is good; the skyline still needs more detail. Visual-only set dressing. Do not reopen **NIGHT-SHIFT-CITY-2**. No collider, physics, netcode, menu, or Quickplay work. Lane: Standard. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NIGHT-SHIFT-PICKER-1 — Night Shift selectable in the arena list | **Top 09-18 (Wyatt, new-window).** Follows NIGHT-SHIFT-CITY-2 PASS `47ad5d80`. Player: Solo and Friends host can pick Night Shift and play a round. Not Quickplay (that is **NIGHT-SHIFT-QP-1**). Not more city art (**NIGHT-SHIFT-DETAIL-1**). Today the menu builds `LEVEL_OPTIONS` from `ARENA_CATALOG.filter(quickplay)` in `src/ui/cart-rave-menu.js`, so rooftop is hidden even though it is in the catalog. `index.html` has no `data-level="rooftop"` card. Catalog `quickplay: false`, empty `music`, empty ambience, unlock hint "Blockout". `MULTIPLAYER_LEVEL_IDS` already allows rooftop. `FREE_LEVEL` stays `zanzibar`. Lever: show production arenas in the menu without flipping `quickplay` (keep `testArena` out). Add a Night Shift menu card. Assign a temporary existing music list and an existing ambience bed so Solo is not silent — do not author new audio. Change unlock hint off "Blockout". Tests: `tests/levels/arenaCatalog.test.js`, `arenaBrowse.test.js`, `arenaMenuOrder.test.js`, `friendsLevel.test.js`. Fail if Night Shift is missing from Solo/Friends pick, if it enters `QUICKPLAY_ARENA_IDS`, or if Test Arena appears. After implementation: Wyatt checks that Night Shift can be picked in Solo and played for one round. Lane: Standard. Seed a Playtest-owed row with `<br>N.` steps when it ships. |
| High | NIGHT-SHIFT-QP-1 — Night Shift in Quickplay rotation | **Top 09-18 (Wyatt).** Depends on **NIGHT-SHIFT-PICKER-1**. Player: a public Quickplay room can roll Night Shift. Files: `shared/arenaPool.js` `QUICKPLAY_ARENA_IDS`, `src/levels/arenaCatalog.js` `quickplay: true`. Catalog music + ambience bed must be non-empty (arenaCatalog test). Prefetch uses the same pool. Worker initial arena pool must match catalog order (QP-ORDER-1). Lane: **Critical** (shared arena pool / public rooms). Fail if Night Shift never appears in QP, if `testArena` leaks into MP, or if catalog quickplay ids drift from `QUICKPLAY_ARENA_IDS`. After implementation: Wyatt checks that Quickplay can start on Night Shift. Do not ship without PICKER. |
| Medium | CLUTCH-SLOMO-1 — Clutch slow-mo (Pass 5 deferral) | ✅ shipped 09-14 Worker `c214c758` (VERIFY_OK `index-D28UWcmx.js`). SD-win latch + ~1.2s slow-mo; **CLUTCH-SLOMO-PT-1** owed. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NET-LAG-PT-2 — high-RTT non-host stays visually stable `[2pc]` | **Owed: Wyatt playtest — NET-LAG-PT-2 — human retest after a cause-specific fix.** FAIL 09-24 on prod `8fa504ac`, Worker `7bb04c34-cda5-4777-bd49-15db25bfdd35`: repeated non-host hard corrections in Friends caps 400/402 and video. Do not retest this diagnostic-only build.<br>1. After a cause-specific fix ships, hard-refresh on the same two machines with `?diag=1&nettest=1` and play Friends for two minutes.<br>2. Drive, turn, boost, ram, and recover from collisions while the host also moves.<br>3. FAIL if the non-host repeatedly snaps or loses current steering. PASS only if the path stays continuous and input response is playable.<br>4. Press F8 on both machines at round end. |
| High | NET-LAG-DIAG-PT-1 — capture the source of non-host correction `[2pc]` | **Owed: Wyatt playtest — NET-LAG-DIAG-PT-1 — capture correction evidence on prod `820176d2`, Worker `e0573a74` (entry `index-MgKvsR2U.js`).** This capture is evidence, not a PASS for NET-LAG-PT-2.<br>1. Hard-refresh https://www.cartclash.lol/?diag=1&nettest=1 on both machines. Host a Friends room and play on the non-host.<br>2. Drive, turn, boost, and ram while both carts move. Press F8 on the non-host immediately after a visible snap; press F8 on the host in the same round.<br>3. Report the time of the snap and a short non-host video if possible. Confirm the non-host capture has `reconcileTrace` entries with `preToHostM`, `postToHostM`, `replayedInputs`, `pendingCount`, and `snapshotGapMs`.<br>4. If a snap occurs but the trace is empty, report that as diagnostic failure. Do not mark visual stability PASS from this capture. |
| Medium | JOIN-LOAD-PT-1 — second player joins a live round and can drive `[2pc]` | **Owed: Wyatt playtest — JOIN-LOAD-PT-1 — a second player can join a live Quickplay round and drive.** Prod `a29736a1` Worker `241bb329-6ef1-444a-ab1c-82fbd47114c7` (entry `index-CEplJ-zV.js`, hard-refresh https://www.cartclash.lol/). Local battery 8/8 before ship.<br>1. Host starts Quickplay and waits until the round is running.<br>2. Second machine joins that same room mid-round.<br>3. FAIL if the joiner stays on the loading overlay, never gets a cart, or the tab dies.<br>4. PASS if the joiner seats, drives off spawn, and the host still drives. |
| Medium | GLITCH-TECH-PT-1 — Desktop App install_id validates before create | **Owed: Wyatt playtest — GLITCH-TECH-PT-1 — Desktop App launch install_id is validated; a leftover local user_install_id does not mint a new Glitch row.** Prod `4ab166a8` Worker `57c54d38-5a8c-4660-aa1f-4e4c2b1eb934` (entry `index-B5xOzF-P.js`). Hard-refresh.<br>1. Launch Cart Clash from the Glitch Desktop App so launch `install_id` is present.<br>2. FAIL if a second Glitch install row appears for the same machine, or if identity looks like a new user.<br>3. PASS if the same Desktop App install continues (validate path, no extra create). |
...
| Low | SNAP-FINITE-PT-1 — snapshot finite guard changes nothing visible `[2pc]` | **Owed: Wyatt playtest — SNAP-FINITE-PT-1 — normal netplay is unchanged after every snapshot body write got a NaN guard (SNAP-FINITE-1).** Prod `6c5c139b` (entry `index-BQxMmoL7.js`, hard-refresh https://www.cartclash.lol/). Retest: non-host rams must read (crash SFX + trash) — **RAM-ARREST-1** fallback shipped.<br>1. Friends match, two machines. Drive, boost, ram, and hop for a full round on the non-host.<br>2. FAIL if any cart freezes in place, teleports to origin, ghosts through the floor, or remote carts stutter where they did not before. FAIL if non-host rams stay silent with no trash.<br>3. PASS if remote carts track smoothly and collisions feel identical to pre-guard play.<br>4. Press F8 on both machines at round end. |
| Medium | CLUTCH-SLOMO-PT-1 — SD-deciding KO stretches before podium | **Owed: Wyatt playtest — CLUTCH-SLOMO-PT-1 — the Sudden Death deciding fall plays in slow-mo before the podium.** Prod `c214c758` (entry `index-D28UWcmx.js`, hard-refresh https://www.cartclash.lol/). Force SD (tie at timer), watch the deciding KO.<br>1. Solo or Friends to a tied timer → Sudden Death.<br>2. FAIL if the podium cuts the instant the winner scores (no stretch), if the game hangs ~1s with no end, or if a regulation end slows.<br>3. PASS if the deciding fall visibly stretches ~1s under the gameplay cam, then the normal podium lands and the Clutch Winner daily still credits. |
...
BOOT-TBT-1, AUDIO-RAM-IMPACT-1, AUDIO-RAM-IMPACT-PT-1, LAST-STANDING-DEAD-1, DEEPSEC-1, NPC-BOOTH-TARGET-1, NIGHT-SHIFT-BLOCKOUT-1, NIGHT-SHIFT-CITY-1, NIGHT-SHIFT-CITY-2, CART-MODEL-1, SHADES-MAT-1, MENU-CART-1, FRIENDS-JOIN-LAYOUT-1, FRIENDS-LEVEL-1, FRIENDS-LEVEL-PT-1, ONBOARD-ATTRACT-1,
ONBOARD-ART-1,
...
MENU-SHORTWIN-1, MENU-SHORTWIN-PT-1,
CG-ZIP-1, CG-ZIP-PT-1,
SITE-TRUST-1, SITE-TRUST-PT-1,
ONBOARD-COPY-1, ONBOARD-COPY-PT-1, NET-LAG-1-PT-1,
CG-COVERS-1, CAM-COMFORT-1, TASTE-P4-1.
