...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CG-COVERS-1** |
...
| [Art](#art) | 1 | 0 | 1 | 0 |
| [Design / Gameplay](#design--gameplay) | 1 | 0 | 1 | 0 |
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | NIGHT-SHIFT-CITY-2 — Fourside-style authored skyline + party roof | **Filed 09-18.** Successor to NIGHT-SHIFT-CITY-1 (closed-for-now baseline, do not reopen). Player: mall-roof party, dense neon city with dark gaps, playable roof is the brightest surface. Inspo: SSBM Fourside composition (navy sky, moon, window-lit towers, a few landmark silhouettes). Not from Fourside: helicopter, UFO copy, walk-off, layout change. Files: `nightShiftVisuals.js`, `nightShiftMaterials.js`, `rooftop.js`, Night Shift mood bullet in `docs/reference/art-direction.md`. No new colliders, physics, netcode, menu, or Quickplay. Near band 4–6 landmarks (mast stays). Fail if: old random boxes, city neon outshines the roof, near band too thin to read as a city, toy/clean window grids, Low over current Night Shift draw-call budget. Done: Wyatt visual PASS in Solo Dev on `?level=rooftop` + focused tests (landmark count, no colliders in visuals, Low draw-call budget) + wide/roof captures (`night-shift-city-2-wide.png`, `night-shift-city-2-moon.png`) + `npm run qa`. Lane: Standard. Picker and Quickplay stay later cards. |
