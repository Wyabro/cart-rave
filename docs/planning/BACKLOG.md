...
| **H** — principal-engineering audit batch (5 open, 10 closed) | 🟡 in progress | **H1 5/7 landed 08-07** (desk-only, not deployed) + **GAMEPAD-LOBBY-1** deployed 08-07, awaiting its two-PC playtest. **H2 3/5 + H3 2/3 landed 08-07** (desk-only). Next unstarted: **HOLE-FRICTION-COMBINE-1**. Detail below in Block H. |
...
unpushed-to-prod desk-only fixes — no playtest owed (GAMEPAD-LOBBY-1 deployed, owed its playtest;
...
1. **GAMEPAD-LOBBY-1** — the one High. A pad-only player cannot use the Friends lobby at all;
   ...
   a one-line fix. **Implemented + deployed 08-07** (`0365b61`) — still owed its two-PC playtest
   (see the UI/UX row).
...
| High | GAMEPAD-LOBBY-1 [2pc] — gamepad navigation is dead in the Friends lobby; a pad-only player cannot ready up | **Implemented and deployed 08-07** (`0365b61`, Worker `64c7d252-3e4e-47c6-9662-9215759d84a6`); source-pin test added, targeted 26/26 and QA 7/7 passed. **Owed: Wyatt playtest — GAMEPAD-LOBBY-1 — Controller navigation works in a Friends lobby.**<br>1. As the host, use the stick or D-pad to focus a difficulty choice, READY UP, and LEAVE ROOM; press A on each and confirm it performs the labeled action.<br>2. As a guest, use the stick or D-pad and A to READY UP and LEAVE ROOM; difficulty choices should not appear.<br>3. Press B or Start in the lobby; it should open pause, not leave the room.<br>4. Check pause, results, and a running round still use their current controller behavior. **Residual:** document-wide navigation can still reach visually covered controls; scoping that set is a separate lever. |
