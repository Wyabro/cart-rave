...
| 🟢 [Playtest owed](#playtest-owed) | 6 | 0 | 2 | 4 |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | GAMEPAD-TEXT-ENTRY-PT-1 — controller name + Friend Code entry `[solo]` | **Owed: Wyatt playtest — GAMEPAD-TEXT-ENTRY-PT-1 — controller text entry works without a native browser keyboard.** Parent **GAMEPAD-TEXT-ENTRY-1**. Pushed but not yet deployed.<br>1. On desktop with a controller and on Steam Deck, use LB/RB to reach PROFILE / FRIENDS. Choose CHANGE NAME, type a new name with D-pad and A, then START to confirm.<br>2. Open ENTER FRIEND CODE. Enter a valid mixed-case Friend Code; confirm it becomes uppercase and joins. Repeat with an invalid code; correct it in the open keyboard, then cancel once.<br>3. FAIL if the browser's native text box opens, text cannot be corrected, cancel changes the name or room, controls reach behind the keyboard, or phone/tablet touch behavior changes. PASS if both controller routes are clear and reliable. |
| Low | GAMEPAD-MENU-ROUTES-PT-1 — controller routes for desktop + handheld menus `[solo]` | **Owed: Wyatt playtest — GAMEPAD-MENU-ROUTES-PT-1 — controller menus have an obvious route to match setup, and pause opens on RESUME.** Parent **GAMEPAD-MENU-ROUTES-1**. Pushed but not yet deployed.<br>1. On desktop with a controller or a controller handheld, open the main menu. Use LB/RB to move between COMMANDS and MATCH SETUP; use the D-pad to reach arena and difficulty controls, then choose an arena and a difficulty.<br>2. Start a solo round, pause it, and confirm the focus ring is already on RESUME. Use D-pad and A to resume; use B from another pause button to resume without changing a main-menu setting.<br>3. FAIL if a bumper changes arena by itself, a setup control cannot be reached, focus goes behind an open overlay, or any phone/tablet touch behavior changes. PASS if the route is clear and both controller types work. |
