...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave H live `94f97bea` · **CART-POP-PT-1** FAIL / remains open · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 8 playtest blockers |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave H live 08-20** Worker `94f97bea` (VERIFY_OK `index-DNHgq9AI.js`). Event 1686 (Classic) contacts record segments 11–12. Isolated 16 wedges do not pop at rest; a drop impact yields +12.7 dvy with restitution 0.05. F8 gzip-base64s 2.5 MB timelines. No physics values changed. **CART-POP-PT-1 remains FAIL/open.** Hard-refresh `?diag=1` and F8. Need a supported +10 from preVy≈0 before a geometry change. Lane: **Critical**. |
