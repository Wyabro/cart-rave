...
| Pri | Item | Notes |
|-----|------|-------|
| High | NIGHT-SHIFT-CITY-1 — Night Shift visual-structure pass | **MAST/DRESSING LOCAL HANDOFF — WYATT REVIEW PENDING.** The working name is not final. Approved square roof, spawns, raised roofs, camera, and AC physics remain frozen. City base through `282c7e2`; mast `46e47ec`; subtle dish/beacon life `4c3848d`; Full-only edge dressing `ae23749`. The mast follows an img2threejs reference contract and sits on distant tower `near-7`; all driveable dressing is flush, solid props stay unreachable, and no collider was added. Budget: 9 Low / 15 Full architecture draw calls; mast 2,112 / 2,916 triangles. Focused 11/11, typecheck, and build green. Full QA remains red only on the unrelated exact-50-row backlog canary. Captures: `.diag-captures/night-shift-mast-t0.png`, `night-shift-mast-low.png`, `night-shift-mast-close-t0.png`. Not pushed, deployed, or closed. |
