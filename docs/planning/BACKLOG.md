...
| **2** — PRE-SHIP (before public post) | 🟡 queued | all landed, PT owed (**SD-SPECTATOR-CHARGE-PT-1** · **KO-CENTER-RING-PT-1** · **CUSTOMIZE-SVG-FLASH-PT-1** · **COUNTDOWN-HOST-STAMP-PT-1** · **FRIENDS-LOBBY-ORDER-PT-1**) |
...
| **5** — SWEEP (cheap Lows) | 🟡 queued | MOTION-A11Y-1 · COUNTDOWN-LEAK-1 · MIG-KO-DROP-1 · 08-16 audit Lows |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 23 | 0 | 9 | 13 (+1 partial) |
...
more engineering. Open owed checks: **SD-SPECTATOR-CHARGE-PT-1**
(not on prod until ship) · **COUNTDOWN-HOST-STAMP-PT-1**
(prod) · **KO-CENTER-RING-PT-1** (prod) ·
**CUSTOMIZE-SVG-FLASH-PT-1** (prod) ·
**FRIENDS-LOBBY-ORDER-PT-1** (prod) ·
...
Landed 08-16 — **COUNTDOWN-HOST-STAMP-1** (skip host countdown stamp while host clock unlocked); playtest owed
(see `## Playtest owed`).
Landed 08-17 — **FRIENDS-LOBBY-ORDER-1** (CHECKOUT LINE humans first, connect order); playtest owed
**FRIENDS-LOBBY-ORDER-PT-1**.
Landed 08-17 — **KO-CENTER-RING-1** (drop center doomed ring; keep edge pulse); playtest owed
**KO-CENTER-RING-PT-1**.
Landed 08-17 — **CUSTOMIZE-SVG-FLASH-1** (DONE/apply does not paint legacy SVG); playtest owed
**CUSTOMIZE-SVG-FLASH-PT-1**.
Landed 08-17 — **SD-SPECTATOR-CHARGE-1** (gate ram-boost charge for SD spectators); playtest owed
**SD-SPECTATOR-CHARGE-PT-1**.
...
3. 08-16 audit Lows, top first: **MIG-KO-DROP-1** (KO feed row lost at migration) · **SPILL-DOUBLE-VFX-1** · **PODIUM-DOUBLE-CREDIT-1** · **KEYUP-STUCK-1** · **HOST-PRESENT-ORDER-1** · **ZOMBIE-ROOM-RESET-1** · **DEMOTE-COUNTDOWN-1** · **SPECTATOR-ANNOUNCER-1** · **RD-COUNTER-1**.
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | COUNTDOWN-HOST-STAMP-1 — first non-host 3-2-1 hangs on 3 then slams 2+1 `[2pc]` | **Landed 08-16** — skip `host_round` / hello `countdownStartedAtMs` while `hostClock.samples === 0` (keep `gameStart` local stamp). Host and rematch (`samples > 0`) still apply. Abort `0` still applies. No `tHost` on `host_round`. Mid-join during countdown is out of this lever. Playtest owed: **COUNTDOWN-HOST-STAMP-PT-1**. Not a reopen of COUNTDOWN-SYNC-1 / COUNTDOWN-QUICKPLAY-1 / THOST-CEILING-1. |
| Medium | KO-CENTER-RING-1 — local KO center ring is redundant with the edge pulse `[solo]` | **Landed 08-17** — deleted `.hud-doomed-shockwave` DOM node + CSS. `showDoomedFeedback` still fires the red edge pulse (`pulseHitDirection`). Hitmarker on a scored KO stays. Do not reopen **KO-DOOMED-1**. Playtest owed: **KO-CENTER-RING-PT-1**. |
| Medium | CUSTOMIZE-SVG-FLASH-1 — DONE/apply flashes the legacy cart SVG `[solo]` | **Landed 08-17** — `renderCustomizePreview` never writes `makeCartSVG` unless Customize is actually open; close persists, then hides, then disposes so `customization-changed` cannot paint the legacy SVG on dismiss. SVG stays as chunk-load / WebGL-fail fallback. Playtest owed: **CUSTOMIZE-SVG-FLASH-PT-1**. |
| Medium | FRIENDS-LOBBY-ORDER-1 — Friends lobby lists bots above later humans `[2pc]` | **Landed 08-17** — `joinOrder` on hello + every `MSG.slots`; CHECKOUT LINE sorts humans first in that order. Seats unchanged. Playtest owed: **FRIENDS-LOBBY-ORDER-PT-1**. Not a reopen of FRIENDS-JOIN-LAYOUT-1. |
| Medium | SD-SPECTATOR-CHARGE-1 — eliminated SD spectator can start a phantom charge; charge-up SFX loops up to the 45s cap | **Landed 08-17** — `triggerRamBoost` returns when `cart.isSuddenDeathSpectator` is set. Keyboard / touch / gamepad / sim re-arm / remote-nitro latch all share that choke. Sim still skips flagged carts; mid-charge-on-fall is already stopped by `onCartOutOfPlay`. Playtest owed: **SD-SPECTATOR-CHARGE-PT-1**. |
...
No open audio cards. **HOWLER-UPGRADE-1** closed 08-17 (pooling + buses shipped; spatial deferred).
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | SD-SPECTATOR-CHARGE-PT-1 — parked SD spectator boost stays silent; no charge bar `[solo]` | **Owed: Wyatt playtest — SD-SPECTATOR-CHARGE-PT-1 — after you are knocked out in overtime, boost stays silent.** Parent **SD-SPECTATOR-CHARGE-1**. Pushed, not yet on prod — `npm run dev` or wait for ship. Hard-refresh after ship.<br>1. Solo. Play until overtime. Get knocked out so you are only watching.<br>2. Press and hold boost. FAIL if the charge-up whine starts or the charge bar fills. PASS if it stays silent and the bar stays idle.<br>3. Rematch or a new live round. While you are still driving, press boost. FAIL if charge does not start. PASS if it charges and releases as usual. |
| Medium | COUNTDOWN-HOST-STAMP-PT-1 — first non-host 3-2-1 is even; 3 does not hang then slam 2+1 `[2pc]` | **Owed: Wyatt playtest — COUNTDOWN-HOST-STAMP-PT-1 — first non-host 3-2-1 is even; 3 does not hang then slam 2+1.** Parent **COUNTDOWN-HOST-STAMP-1**. You are not host. Hard-refresh. On prod (VERIFY_OK `7896b9f4`).<br>1. Two PCs. Friends or Quickplay. You are not host. First match.<br>2. FAIL if 3 hangs, then 2 and 1 slam. PASS if 3-2-1-GO is about 1 s each.<br>3. Rematch in the same room. FAIL if that countdown hangs or slams. PASS if it stays even.<br>4. Host screen: FAIL if host 3-2-1 changed. |
| Medium | KO-CENTER-RING-PT-1 — when you get KO'd, only the red edge pulse flashes `[solo]` | **Owed: Wyatt playtest — KO-CENTER-RING-PT-1 — when you get KO'd, the only flash is the red edge pulse; no centered expanding ring.** Parent **KO-CENTER-RING-1**. On prod (VERIFY_OK `7896b9f4`). Hard-refresh.<br>1. Solo or Quickplay. Get KO'd by a ram or a hazard (fall off the edge works).<br>2. FAIL if a centered circle/ring expands from mid-screen. PASS if only the red edge pulse flashes.<br>3. In the same match, KO an opponent — FAIL if the center hitmarker is gone. PASS if it still shows. |
| Medium | CUSTOMIZE-SVG-FLASH-PT-1 — DONE/apply does not flash the legacy cart SVG `[solo]` | **Owed: Wyatt playtest — CUSTOMIZE-SVG-FLASH-PT-1 — DONE and apply do not flash the old cart drawing.** Parent **CUSTOMIZE-SVG-FLASH-1**. On prod (VERIFY_OK `7896b9f4`). Hard-refresh.<br>1. Menu. Open Customize. Pick a color. Press DONE.<br>2. FAIL if a flat cartoon cart drawing flashes while the screen closes. PASS if the 3D cart stays or the screen just closes.<br>3. Open Customize again. Change color. Stay on the screen. FAIL if the 3D cart turns into that flat drawing. PASS if the 3D cart updates in place. |
| Medium | FRIENDS-LOBBY-ORDER-PT-1 — Friends CHECKOUT LINE lists humans first, first-connect first `[2pc]` | **Owed: Wyatt playtest — FRIENDS-LOBBY-ORDER-PT-1 — Friends CHECKOUT LINE lists humans first, first-connect first.** Parent **FRIENDS-LOBBY-ORDER-1**. On prod (VERIFY_OK `7896b9f4`). Hard-refresh.<br>1. Two PCs. Friends room. Host picks a cart color that is not the seat-0 bot color (or confirm host is not seat 0).<br>2. Guest joins. Both screens: FAIL if a bot sits above a human. PASS if humans are at the top, host above guest, bots under.<br>3. FAIL if a human's color or seat mark no longer matches their cart. PASS if color and mark still match the cart.<br>4. Not a FAIL: the small score list at 3-2-1 may still show seat order. |
...
WARM-QP-ROTATE-PT-1, HOWLER-UPGRADE-1, CART-HUE-RED-1, CART-HUE-RED-PT-1, LAST-STANDING-DEAD-PT-1,
