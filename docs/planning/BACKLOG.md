...
| Pri | Item | Notes |
|-----|------|-------|
| Low | FEEDBACK-PT-1 — the match receipt prints a working survey line `[solo]` | **Owed: Wyatt playtest — FEEDBACK-PT-1 — the podium receipt ends with TELL US HOW WE DID and it opens the feedback form in a new tab.** Pad exclusion is machine-covered (`tests/orchestration/podiumFocus.test.js`), so this card is only the human read. *Pushed but not yet deployed* — use `npm run dev`.<br>1. Play one Solo round through to the podium.<br>2. Look at the bottom of the MATCH RECEIPT, below THANK YOU FOR SHOPPING. The survey line must be there, and must print last as the slip feeds out.<br>3. Click it. PASS if the feedback form opens in a NEW tab and the game tab stays on the podium. FAIL if nothing opens, if the game tab navigates away, or if the line is missing or prints above the receipt total. |
