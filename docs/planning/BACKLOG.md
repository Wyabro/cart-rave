...
| Medium | MENU-MUSIC-PT-1 — menu plays both songs, never under the game `[solo]` | **Owed: Wyatt playtest — MENU-MUSIC-PT-1 — the menu plays one song at a time and dies when Solo starts.** Parent MENU-MUSIC-2. Not on prod. Run `npm run dev:local`.<br>1. Open the menu. One song starts.<br>2. Start Solo. Menu music stops. Arena music only.<br>3. Quit to menu. One menu song starts. FAIL if silence, both songs at once, or menu music under the game. |
