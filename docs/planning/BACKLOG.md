...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 11 | 0 | 4 | 6 (+1 partial) |
| [Art](#art) | 1 | 0 | 0 | 1 |
| [Audio](#audio) | 2 | 0 | 0 | 2 |
...
**DEEPSEC-1-PT-1** (two machines). Closed PASS history lives only in
[completed-work.md](./completed-work.md).
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | GAMEPAD-FRIENDS-SEATED-PT-1 — pad moves one Friends-lobby control per press `[solo]` | **Owed: Wyatt playtest — GAMEPAD-FRIENDS-SEATED-PT-1 — each pad press moves one seated-lobby control, and focus never lands on mute.** Parent **GAMEPAD-FRIENDS-SEATED-1** (`747e67d`, Worker `ef2a7550`). Retest after lever 2. Hard-refresh prod first. Hold-stick tour inside the lobby is a note, not FAIL.<br>1. Hard-refresh prod. Open FRIENDS. Seat with a controller.<br>2. Use D-pad, then left stick, to move among COPY, difficulty (host only), READY, and LEAVE. Stop on READY. Press A. Press B and confirm you did not jump to mute or the main menu.<br>3. FAIL if focus hops to mute, audio, or the menu behind the lobby, or if one press skips more than one lobby control. PASS if each press moves one lobby control. |
