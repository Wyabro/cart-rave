...
| 🟢 [Playtest owed](#playtest-owed) | 9 | 0 | 4 | 5 |
...
| Low | LOD-PITRING-PT-1 — pit-band silhouettes stay visible driving the edge `[solo]` | **Owed: Wyatt playtest — LOD-PITRING-PT-1 — the pit-band silhouettes stay visible as you drive along the arena edge.** LOD-PITRING-1 removed the origin-anchored LOD that culled them exactly when the chase camera crossed the band; prod after hard-refresh.<br>1. Storerooms solo, drive a cart around the pit band (the gondola rows just past the floor edge).<br>2. Watch the gondolas/pallets/checkout lane as the chase camera crosses the band — they must stay rendered the whole time.<br>3. FAIL if the silhouettes pop out of existence when you get close to them (or anywhere). PASS if they stay visible the entire drive. |
