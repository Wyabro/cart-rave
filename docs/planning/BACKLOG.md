...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Classic PASS · Sundial trimesh landed (not shipped) · Storerooms remain · **CART-POP-PT-1** open until those two · **NET-LAG-1-PT-1** parked |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Classic PASS 08-20** Wave I Worker `5360f482`. Do not retouch `src/levels/arena.js` floor. **Sundial landed (not shipped):** one octagon trimesh per flat with `FIX_INTERNAL_EDGES`. Isolated r=26 24 m/s: cuboids `dvy=4.367 nY=0.919`; trimesh `pops=0`. **Storerooms:** `backroomsSupermarket.js` multiple floor colliders. Same first-ejection: supported, `preVy≈0`, tilted manifold, ~24 m/s. Pattern: one floor surface, no overlap, no radial internals. No restitution/friction/CCD/solver tune. **CART-POP-PT-1** open until Storerooms (and Sundial playtest) PASS. Lane: **Critical**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | CART-POP-PT-1 — normal driving stays planted across all arenas `[1pc]` | **Owed: Wyatt playtest — CART-POP-PT-1 — Sundial and Storerooms stay planted.** **Wyatt PASS 08-20 Classic** (Wave I `5360f482`). Parent **CART-POP-1**. Still FAIL until the other two arenas PASS.<br>1. On production with `?diag=1`, drive ~30 s on Sundial, then Storerooms; straight runs and full-lock turns, no hop or boost.<br>2. FAIL if a cart pops off the flat floor without a ram, obstacle hit, or edge contact.<br>3. PASS if ordinary driving stays planted on both remaining arenas while rams and hits still have visible lift.<br>4. Press F8 once after each arena. |
...
NPC-SELFKO-3, NPC-SELFKO-3-PT-1.
