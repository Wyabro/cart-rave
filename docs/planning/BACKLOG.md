...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Classic + Sundial PASS · Storerooms remain (new window) · **CART-POP-PT-1** open until Storerooms · **NET-LAG-1-PT-1** parked |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Classic PASS 08-20** Wave I Worker `5360f482`. **Sundial PASS 08-20** prod `bb29c13b` Worker `3f3e5fbd` (VERIFY_OK `index-DPVRIrNw.js`). Do not retouch Classic or Sundial floors. **Storerooms (new window):** `backroomsSupermarket.js` multiple floor colliders. Same first-ejection: supported, `preVy≈0`, tilted manifold, ~24 m/s. Pattern: one floor surface, no overlap, no radial internals. No restitution/friction/CCD/solver tune. **CART-POP-PT-1** open until Storerooms PASS. Lane: **Critical**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | CART-POP-PT-1 — normal driving stays planted across all arenas `[1pc]` | **Owed: Wyatt playtest — CART-POP-PT-1 — Storerooms stay planted.** **Wyatt PASS 08-20 Classic** (Wave I `5360f482`) **and Sundial** (prod `bb29c13b`). Parent **CART-POP-1**. Still FAIL until Storerooms PASS.<br>1. On production with `?diag=1`, drive ~30 s on Storerooms; straight runs and full-lock turns, no hop or boost.<br>2. FAIL if a cart pops off the flat floor without a ram, obstacle hit, or edge contact.<br>3. PASS if ordinary driving stays planted on Storerooms while rams and hits still have visible lift.<br>4. Press F8 once after the arena. |
