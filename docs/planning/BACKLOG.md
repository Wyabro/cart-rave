...
1. **NPC-BOOST-2** — restore visible charged NPC attacks and stop boost self-KOs. Wyatt's
   ...
   carries charged-release mode to remote clients. QA/build and explicit ship authorization remain.
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NPC-BOOST-2-PT-1 — charged and instant NPC boosts `[2pc]` | **Blocked on a new deployment.** The prior NPC-BOOST-PT-1 is **FAIL**: no visible NPC charge-ups and more NPC self-KOs.<br>1. After deployment, hard-refresh a Solo round with three NPCs. PASS if each difficulty visibly produces charge-ups in safe attacks, with Easy least frequent and Hard most frequent.<br>2. In a Friends room, view the same NPC charge and release from both clients. PASS only if both show the charge telegraph and charged burst.<br>3. On Classic, drive NPCs near the outer rim and center hole; repeat on octagon and square-hole maps. PASS only if every unsafe charge cancels and every unsafe instant boost is skipped.<br>4. FAIL on any NPC boost self-KO, boost-state desync, or reversed difficulty order. |
