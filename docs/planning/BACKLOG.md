...
| Pri | Item | Notes |
|-----|------|-------|
| High | PRE-PODIUM-1 — confetti / wilt + win-lose callout missing `[pre-ship]` | Victory confetti and defeat wilt missing in **MP and solo** pre-podium. Also missing **“{playerName} wins / loses”** (or equivalent) at round end before the podium screen. Fight-night handover already flagged MP confetti (parked 07-23); this card re-opens both modes + the name callout. Art look of wilt still under Wilting-groceries `[SHIP-1 E2]`. |
...
| High | FIGHT-VERIFY-1 — owed fight-night verification | 8 items at [fight-night-ui-handover.md:252+](./fight-night-ui-handover.md). Agent half via SHEET-1: responsive sweep 1025/1024/768/380, reduced-motion, cold-boot loading screens per arena, die-cut hover/press surfaces. Wyatt half (cannot be agent-closed): real-match HUD/results feel, two-client friends room (CHECKOUT LINE has never rendered). Include **PRE-PODIUM-1** in Wyatt half once coded. |
| High | RESULTS-ACT-1 — PLAY AGAIN / MAIN MENU render below the fold in the short-window branch | **Code landed `f42515f` 08-01** — keep PLAY AGAIN on screen in squat windows. **Owed: Wyatt eye.** Found 07-31 by `npm run podium` (FIGHT-VERIFY-1 Phase A). Was: `@media (max-height:640px)` stacked panel with overflow but actions off-screen (900×390 / 1025×600). |
...
| Medium | LOAD-PROGRESS-1 — the mode-entry meter is decorative for the entire arena build | **Found 07-31 by `npm run loadshots`** (FIGHT-VERIFY-1 Phase C), reproduced on three consecutive runs across all three arena themes. The meter's full value timeline for a cold `classicRecord` entry at 1920×1080: `0%` at 25091ms → `15%` at 25126ms → **nothing for 11.5 seconds** → `88%` at 36628ms → `94%`/`96%` in the same millisecond → `100%` at 39446ms. So the bar is frozen at 15% for ~85% of the load and then does the other 85 points in under 3 seconds. `src/levels/index.js` has exactly the three milestones that would fix this — `:96` `onProgress?.(20, "Fetching level…")`, `:112` `(60, "Building arena geometry…")`, `:154` `(90, "Physics colliders ready")` — and **none of them can fire on a cold entry.** The callback is dropped at **`src/main.js:2875-2877`**, where `bootstrapWorldCore` calls `swapLoadedLevel(resolveLevelId(...))` with no second argument, so `opts.onProgress` is `undefined` all the way down. The one line that *does* forward it, `src/levelManager.js:267` (`swapLoadedLevel(selected, { onProgress })`), is structurally unreachable in the cold path: `levelManager.js:248`'s `needsFullSwap` is false because `:208` already set `loadedLevelId = selected` during the cold swap (and `:23` pre-initialises it from stored selection anyway), and `previewNeedsFullRebuild` is only set on the menu-preview branch, which bails out when the world isn't bootstrapped (`:311-317`). The ~11.5s of silence is `bootstrap.js:462`'s `await d.ensureWorldBootstrapped(selected)` joining the in-flight promise from `:443`. Level modules emit nothing either — `classicRecord.js` / `backroomsSupermarket.js` / `zanzibarPlatform.js` / `testArena.js` never reference `onProgress`, so `initFn`'s multi-second geometry+collider build is one opaque block. **Second defect in the same area:** `setProgress` (`src/ui/loadingScreen.js:195-203`) has no monotonic clamp, so on the *warm* preview path — the one where the wiring does work — the meter reads 15 → 20 → 60 → **90 → 88** → 94 → 100, a visible backwards step, because `bootstrap.js:463` reports 88 after `loadLevel` already reported 90. Evidence: the `meter renders progress` check row prints the complete timeline for every cell; frames in `.diag-captures/loadshots/mode-*-b-mid.png`. |
...
| Low | BOOT-METER-1 — the boot splash meter reaches 100% and then jumps *backwards* to 75% | **Found 07-31 by `npm run loadshots`** (FIGHT-VERIFY-1 Phase C), reproduced on three consecutive runs. `dismissInitialBootSplash` writes the bar to 100% by setting the DOM directly (`src/ui/loadingScreen.js:331-335` — `bootFill.style.width = '100%'`, `bootPct.textContent = '100%'`) **without raising the inline boot script's own `bootProgress` closure variable** (`index.html:581`). A later real milestone then repaints the bar downward: `noteBootMilestone(75)` (`src/main.js:790`, chained off `prefetchRaveGltf()`) calls `window.__crBootFloor(75)` (`index.html:600-605`), which compares 75 against the stale `bootProgress` and happily paints **75%** over the 100%. `noteBootMilestone(90)` (`main.js:1755`) can do the same. `noteBootMilestone` guards only on the splash *element* still existing (`loadingScreen.js:372`), and the splash lives for ~620ms after the 100% write (200ms hold + 420ms fade/remove), so the window is real. Measured with the splash held open: at 1920×1080 the meter read `100% · 100% · 100% · 100% · 75%` across the five captured beats in one run and `100% · 100% · 75% · 75% · 75%` in another — the drop lands at a different beat each time because it is gated on a network fetch of the cart GLB. In production `MIN_BOOT_MS` is 1300 (returning) / 1600 (first visit) rather than DEV's 0, so whether a player actually sees the snap-back depends on whether the GLB resolves inside that ~620ms window — cosmetic, timing-dependent, but it is on the very first screen of the game. Cheapest fix is to have `dismissInitialBootSplash` raise the floor through `__crBootFloor` (or expose a setter) instead of writing the DOM behind the inline script's back. Evidence: the `crash tableau intact` check row prints `meter reads N%` for every beat. |
...
| Low | MENU-NAME-HOVER-1 — the player-name field's hover affordance never renders | **Found 07-31 by `npm run states`** (FIGHT-VERIFY-1 Phase B), reproduced on two consecutive runs; the only red the tool reports. `.cr-name-display:hover { border-color: rgba(255,255,255,0.2) }` (`src/cart-rave-menu.css:693`) is dead: `.cr-plate .cr-name-display` (`:3447-3454`) sets `padding: 0; border: 0`, and at **equal specificity (0,2,0) the later rule wins**, so the `border` shorthand's own `border-color` beats the hover rule's and the width is 0 anyway. The redesign comment right above it says the affordance was retired on purpose ("the retired rename affordance (inset pad + dashed contour) is gone — the pencil button beside it carries that job"), so the correct fix is almost certainly to **delete `:693`**, not to resurrect the border. Worth a look either way, because the element still carries `cursor: text` from `:682` while the plate rule sets `cursor: pointer`, and it now has no hover affordance at all. Confirmed by eye: `.diag-captures/states/menu--cr-name-display-{rest,hover}.png` are pixel-identical. |
