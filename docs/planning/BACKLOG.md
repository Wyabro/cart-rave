...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave D deployed — contact-location F8 needed · **CART-POP-PT-1** FAIL / remains open · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 8 playtest blockers |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave D deployed 08-20** (`b4ba0125`; Worker `b8554436-b64d-4f6f-917c-f0c120febabc`; live entry/assets/marker verified). Wave C cap-387 puts the rise in `world.step`: 133 known record-floor samples, 28 local, and matching post-control/pre-solver `vy` in 132/133. Candidate is the 16 convex record wedges near the inner ring. The new `?diag=1` trace adds cart position/radius and up to eight record-floor handle/index/manifold summaries with overflow count; it changes no physics coefficient, collider, or solver setting. **CART-POP-PT-1 remains FAIL/open.** Reproduce ordinary Cart Rave driving and F8 before a floor change. Lane: **Critical**. |
