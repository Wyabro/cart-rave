...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **NET-LAG-1-PT-1** parked · **ONBOARD-WEBP-1** loop FAIL · **ONBOARD-WEBP-PT-1** owed |
...
| High | ONBOARD-WEBP-1 — HOW TO PLAY animated WebPs fail on some PCs | **Loop lever 08-21.** Files are valid infinite WebPs (`loop_count=0`, ~2.8s). **PT FAIL 08-21:** each clip plays once, then freezes. Cause: Chromium stops looping an `<img>` WebP after canvas `drawImage` sampling. Lever: after a frame-change verdict, replace the sampled node with a never-sampled copy. Load-error / reduced-motion still swap stays. **Owed: ONBOARD-WEBP-PT-1 retest after ship.** Distinct from reduced-motion `still.webp` (ONBOARD-ART-1). Do not reopen **ONBOARD-SLIDES-1**. |
