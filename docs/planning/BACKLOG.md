...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave D contact-location trace · **CART-POP-PT-1** FAIL / remains open · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 8 playtest blockers |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave C read 08-20.** Production cap-387 (`1a0c392d`, Cart Rave) has 133 unexpected rise samples, 28 on Wyatt's local cart. All are known record-floor contacts; none are unclassified static geometry. In 132/133, post-control and pre-solver `vy` match; `world.step` creates the upward change. Local peaks: `0 → +4.40` and `−8.43 → +1.35`, with two to four record-floor contacts. Candidate is the 16 convex record wedges near the inner ring, but no radial position or individual handle is captured yet. **CART-POP-PT-1 remains FAIL/open.** Wave D must trace location and per-contact identity before a floor change. Lane: **Critical**. |
