...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave G deployed; pull support timeline · **CART-POP-PT-1** FAIL / remains open · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 8 playtest blockers |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave G deployed 08-20.** Wave F caps 392–393: controls, restitution, wedge seams, and pitch/roll are not sufficient causes. Upright/no-spin carts reach `−8.718 → +1.43` at one flat-floor contact on the round-cuboid lower front corner; tangent impulse, solver friction, and solver restitution are zero. Wave G records the last 60 pre-solver support frames before each rise; Worker `39124619` is live with the entry, 28 referenced assets, and lazy chunk marker verified. **CART-POP-PT-1 remains FAIL/open.** Pull a new F8 capture; isolate the support-loss source before a physics change. Lane: **Critical**. |
