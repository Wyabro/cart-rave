...
| Low | GAMEPAD-DIRECT-ENTRY-PT-1 — direct controller name + room-code entry `[solo]` | **Owed: Wyatt playtest — GAMEPAD-DIRECT-ENTRY-PT-1 — the visible pencil and room-code box open a usable controller text dialog.** Parent **GAMEPAD-DIRECT-ENTRY-1** removes the duplicate lower-left controls after the earlier route failed.<br>1. On desktop with a controller, select the visible pencil by the cart name, press A, and enter a name with the D-pad and A. Press B once and confirm the dialog closes and focus returns to the pencil. Reopen it and confirm the name.<br>2. Select the visible ROOM CODE field, press A, and enter a code. Confirm the dialog accepts controller-grid input and a physical or Steam keyboard can type in the focused text field. Press B once and confirm focus returns to ROOM CODE.<br>3. At 1280×800 and on Steam Deck, FAIL if either duplicate lower-left button appears, the dialog does not appear, input freezes, B does not close it, focus reaches behind it, or phone/tablet touch behavior changes. PASS if both real controls are clear and reliable. |
