...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave E deployed; orientation capture due · **CART-POP-PT-1** FAIL / remains open · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 8 playtest blockers |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave E diagnostic deployed 08-20; caps 389–391 pulled.** Wave D cap-388 shows wedge seams are not required: four single-floor supports have the highest mean rise (+5.42), peak `−8.64 → +2.41`. Wave C rules out control code and restitution. Wave E: tilt/spin can amplify solver rises (`+15.67`, `+16.06`), but upright/no-pre-spin still reaches `+11.01`; it is not sufficient as root cause. **CART-POP-PT-1 remains FAIL/open.** Reproduce ordinary Cart Rave driving and F8 before a physics change. Lane: **Critical**. |
