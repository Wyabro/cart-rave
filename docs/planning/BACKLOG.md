...
| Low | KBM-TOAST-1 | `.cr-kbm-toast` / `-toast-text` / `-toast-close` (cart-rave-menu.css:130-206) has no JS reference anywhere in `src/` | **Filed 08-06** — found while auditing `text-transform: uppercase` usage (unrelated question). No `document.createElement`/`classList`/template literal anywhere in `src/` sets `cr-kbm-toast` or its children, and `Select-String -Path src -Pattern kbm -Recurse` only hits this one CSS file — looks like orphaned CSS from a removed keyboard/mouse-switch toast. Confirm with a full-text search before deleting (dynamic class construction elsewhere could still reference it), then drop the three rules and their `@media` overrides. Not a ship blocker — dead weight, not a bug. **⚠️ Same subject as STATES-DEAD-1 (UI/UX, Low) — run them as ONE card, 08-06 audit.** That card's `npm run states` sweep already found `.cr-kbm-toast-close` as its subject (2), and its subject (1) `.cr-touch-btn` is the immediate neighbour in the same block of `cart-rave-menu.css`. Both re-verified with a repo-wide search 08-06. Picking either alone edits lines the other one owns. |
