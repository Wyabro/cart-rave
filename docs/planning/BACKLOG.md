...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave C diagnostic · **CART-POP-PT-1** FAIL / remains open · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 8 playtest blockers |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave B FAIL → Wave C diagnostic 08-20.** Production cap-386 verifies the shared Min lever landed (effective restitution 0.05) but ordinary Cart Rave driving is unchanged or worse: 73 unexpected rises, all floor-only; 71 start in the solver step; peak `deltaVy +15.41`. Restitution is ruled out as the main cause. Next probe separates post-control/pre-solver `vy` from solver `deltaVy` and counts known record-floor versus unclassified static contacts. **CART-POP-PT-1 remains FAIL/open.** Do not tune another coefficient before that trace. Lane: **Critical**. |
