...
| **3** — WYATT LANE (blocked on you) | 👤 ongoing | SHIP-1 D-tier cut/keep |
| **4** — PERF RESIDUAL (measure-first) | 🟡 queued | WARM-SOLO-1 · PERF-WATCH-1 · NET-PERF-1 |
| **5** — SWEEP (cheap Lows) | 🟡 queued | ENG-LOW-SWEEP-2 closed 08-13 — MOTION-A11Y-1 · COUNTDOWN-QUICKPLAY-1 · COUNTDOWN-LEAK-1 remain (browser-gated) |
...
| 🟢 [Playtest owed](#playtest-owed) | 14 | 0 | 3 | 11 |
| [UI / UX](#ui--ux) | 3 | 0 | 1 | 2 |
...
| Low | RUMBLE-STRENGTH-1 — Enable controller rumble from Settings *(pre-ship 07-19)* | **Approved scope:** add a general `ENABLE CONTROLLER RUMBLE` Settings action. Xbox, PS5, and Steam Deck are first-class targets through standard Gamepad rumble; use a narrow, user-permission WebHID fallback only for USB DualSense `054c:0ce6` when that API exposes no actuator. Show connect, enable, on, and honest unsupported states; persist the setting; preserve gameplay rumble and input. Controller-only menu focus/confirm pulses are lower priority than gameplay rumble. No adaptive triggers or generic Xbox WebHID path. **Files:** `src/haptics.js`, `src/ui/gamepadNav.js`, `src/ui/cart-rave-menu.js`, Settings markup/styles, haptics/nav tests. **Asserts:** standard pads need no setup; the PS5 permission button causes a test pulse only after approval; all three targets retain hit/KO feedback; unsupported browser/device fails safely; keyboard/mouse menu use stays silent. **Risk:** WebHID is Chromium + secure-context + user-permission gated. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | ANIM-BUGS-PT-1 — menu entrance + animation lifecycle `[solo]` | **Owed: Wyatt playtest — ANIM-BUGS-PT-1 — menu title/commands stagger in, dismiss/press/fade stay safe, and cart scale returns after boost/impact.** **Unshipped:** use `npm run dev:local` until the next ship; then repeat on production after a hard refresh.<br>1. Hard-refresh the menu. PASS if CART/CLASH title words and command rows stagger in; FAIL if they stay blank then pop late with no stagger.<br>2. Open/close Settings twice fast and Esc twice fast; interrupt Esc controls during fade-in. PASS if overlays hide cleanly and buttons stay clickable; FAIL if a panel sticks or a control ignores clicks afterward.<br>3. Mouse and touch: press a menu command, drag off, release. PASS if scale returns to rest; FAIL if it stays small. Repeat on one HUD wired button if available.<br>4. With howtoSeen cleared (or a fresh profile), confirm HOW TO PLAY attracts; leave and return to menu. PASS if attract can run again; FAIL if it never restarts.<br>5. In a Solo round, boost then collide (or spam boost). PASS if cart scale ends at normal size; FAIL if it stays swollen or squashed. |
| Low | BOOST-SFX-RESPAWN-PT-1 — charge-up sound survives other carts' respawns `[solo]` | **Owed: Wyatt playtest — BOOST-SFX-RESPAWN-PT-1 — the boost charge-up sound keeps playing until release, even when an NPC respawns mid-charge.** **Deployed:** `ef6e7c4` / Worker `32b9807a-c6d4-41b8-af69-15d89c40366c`; hard-refresh production first.<br>1. Start a Solo round with 3 NPCs. Hold BOOST to charge and, while holding, ram an NPC off the arena. PASS if the charge-up sound keeps playing through the NPC's respawn (~1 s later) until you release; FAIL if the sound cuts out early while the bar is still filling.<br>2. Release at full charge. PASS if the BOOST sound fires on release; FAIL if the boost fires with no charge-up sound, or the charge sound restarts. |
| Low | RUMBLE-STRENGTH-PT-1 — controller rumble works on Xbox, PS5, and Steam Deck `[solo]` | **Owed: Wyatt playtest — RUMBLE-STRENGTH-PT-1 — Settings enables controller rumble, gives one short test pulse, and keeps gameplay feedback stronger than menu feedback.** **Deployed:** `682891e` / Worker `82e8a360-b185-403a-8a66-2757f7aba40d`; hard-refresh production first.<br>1. With an Xbox controller in Chrome, open Settings and enable controller rumble. PASS if it gives one short test pulse, says ON, and stays ON after reload; FAIL if it asks for HID permission or says ON without a pulse.<br>2. With the PS5 controller connected by USB, open Settings. If standard rumble is ready, enable it; if the permission state appears, approve only the PS5 picker. PASS if the test pulse, menu focus/confirm taps, and one hit or KO rumble work; FAIL if an unrelated device is offered, the state lies, or no gameplay rumble occurs.<br>3. In the Steam Deck browser, enable rumble and navigate the menu, then play one round. PASS if the Deck gets the same test/menu/gameplay pattern; FAIL if it is treated as unsupported while the browser exposes a standard actuator.<br>4. Turn rumble off, then use keyboard or mouse in the menu. PASS if controller rumble stops and phone/menu vibration does not occur; FAIL if a menu tap overrides a stronger hit or KO rumble. |
| Low | GAMEPAD-NAV-REPEAT-PT-1 — held controller menu navigation `[solo]` | **Owed: Wyatt playtest — GAMEPAD-NAV-REPEAT-PT-1 — holding a D-pad or left-stick direction cycles menu controls without accidental diagonal skips or a rapid rumble buzz.** **Deployed:** `34518ca` / Worker `54ce3bb3-3cfd-4d2f-9ea3-c9e671f5c7db`; hard-refresh production first.<br>1. Connect an Xbox controller, PS5 USB controller, or Steam Deck. Open Settings and hold D-pad down (then the left stick down). PASS if focus moves once immediately, pauses about 0.3 s, then moves at a readable steady rate; FAIL if it only moves once, starts slowly, or skips two controls at once.<br>2. Hold the stick diagonally. PASS if each repeat moves one predictable direction only; FAIL if focus jitters, alternates, or double-steps.<br>3. Hold a direction, then open or close an overlay. PASS if focus starts in that layer and does not carry an immediate extra move across the layer change; FAIL if it jumps behind the overlay.<br>4. Focus a Settings or Customize slider and hold left or right. PASS if it makes one normal 5% nudge only; FAIL if it keeps changing the value. |
| Low | KO-DOOMED-PT-1 — local KO edge pulse + shockwave `[solo]` | **Owed: Wyatt playtest — KO-DOOMED-PT-1 — every local KO gives a medium red edge pulse and one centered shockwave when your cart shatters.** **Unshipped:** use `npm run dev:local` until the next ship; then repeat on production after a hard refresh.<br>1. Start a Solo round and deliberately fall off the arena, including the center hole where available. PASS if the red edge pulse and centered shockwave appear once with your cart shatter; FAIL if either is missing, repeats, or appears before the shatter.<br>2. Let an NPC ram you into the void. PASS if the same feedback appears once; FAIL if it only works for self-falls.<br>3. KO an NPC without falling yourself. FAIL if you get the red victim feedback; PASS if only the normal attacker KO feedback appears.<br>4. In Settings, set graphics quality Low or turn post-FX off if available. Repeat one local fall. FAIL if the feedback disappears. |
| Low | CHAL-SHELF-FIT-PT-1 — portrait challenge shelf shows six cards `[solo]` | **Owed: Wyatt playtest — CHAL-SHELF-FIT-PT-1 — the Challenges screen shows four Daily and two Weekly cards, their headings, BACK, and DONE together at 360 × 640 portrait.** **Unshipped:** use `npm run dev:local` until the next ship; then repeat on production after a hard refresh.<br>1. On a 360 × 640 portrait viewport, open Challenges. PASS if all six cards, DAILY and WEEKLY headings, BACK, and DONE are visible without scrolling.<br>2. Check the longest visible challenge name and a completed card if one is available. FAIL if any name, badge or DONE stamp, description, progress bar, or count is clipped or overlaps.<br>3. Open Challenges on desktop. PASS if the existing two-column desktop shelf appearance is unchanged. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | ANIM-BUGS-1 — animations.js entrance + lifecycle bugs | **Levers 1–6 landed** on `cart-clash` (HEAD includes `51f33b0`); QA 7/7. Multi-target menu enter + stagger, dismiss settle, fadeIn pointer cleanup, pointer capture (shared + menu), HOW TO attract clear on cancel, cart pulse snap + dead joystick delete. Human check: **ANIM-BUGS-PT-1**. Unshipped until Wyatt says ship it. |
| Low | CHAL-SHELF-FIT-1 — challenge shelf fits portrait phone without scrolling | Four daily and two weekly challenge cards, headings, and DONE/BACK controls must fit on one portrait-phone screen without scrolling while staying readable. Scope is portrait phones only; desktop layout and appearance must remain unchanged. No challenge data, rotation, progress, or interaction changes. |
| Low | GAMEPAD-NAV-REPEAT-1 — held controller menu navigation | **Approved scope:** controller D-pad or left stick moves once immediately, waits 300 ms, then repeats every 100 ms. Resolve one direction only: D-pad wins; stick uses its strongest entry axis with 0.55 enter / 0.35 release hysteresis. Reset held input when the nav scope changes, navigation becomes inactive, or the controller disconnects. Keep keyboard navigation, A/B/LB/RB, and left/right slider behavior unchanged; a held slider nudge remains one 5% step. Limit controller focus rumble to one pulse per 200 ms. **Files:** `src/ui/gamepadNav.js`, `src/haptics.js`, nav/haptics tests. **Asserts:** no diagonal double-step, no drift restart, no post-overlay repeat leak, no repeat burst after a stalled frame, and no rapid rumble buzz. **Risk:** browser gamepad reports vary by controller. |
