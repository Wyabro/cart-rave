...
| **2** — PRE-SHIP (before public post) | 🟡 queued | Definition: **CART-COLOR-DEPTH-1**. Then ONBOARD-ART-1 · MENU-CART-1 · VOICE-BUS-1 · CHUNK-DEFER-1 · NPC-BOOST-1 (measure) · UI-FRAME-1 + ESC-PANEL-1 · PAD-MENU-1 · KILLFEED-PHONE-1 · ARENA-BUMPER-HINT-1 (after product call) |
...
5. **CHUNK-DEFER-1** — dynamic-import deferral of netcode / bootstrap / cartRaveGltf off the cold menu initial set (successor residual from closed CHUNK-MEMBER-1).
6. **NPC-BOOST-1** — measure session only; retune is a separate ack.
7. **UI-FRAME-1 + ESC scoring panel** `[SHIP-1 E1]` look pair.
8. **Controller menu navigation polish** *(pre-ship 07-19 residual)*.
9. **KILLFEED-PHONE-1** — kill feed awkward/overlapping on phone.
10. **ARENA-BUMPER-HINT-1** — menu advertises LB/RB arena; nothing reads bumpers. Product call first (wire or drop the hint).
