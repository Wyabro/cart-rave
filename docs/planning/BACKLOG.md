...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 19 | 0 | 11 | 7 (+1 partial) |
| [Art](#art) | 16 | 2 | 4 | 10 |
| [Audio](#audio) | 5 | 0 | 3 | 2 |
...
do." **As of the 08-06 export it is empty of actionable cards** — the section holds only
SHARD-PT-2, which is deferred to launch day, and AQ-RING-CLEAR-1, which is a reserve lever rather
than a check.
...
| Pri | Item | Notes |
|-----|------|-------|
| High | ONBOARD-ATTRACT-1 — HOW TO PLAY should invite a first-time player, not open itself on them | **Filed 08-06 from Wyatt's note on the ONBOARD-SLIDES-PT-2 PASS** — *"i'm afraid that if the how to play thing is the first thing they see they might be confused. maybe instead of forcing them to it, HOW TO PLAY should glow and move more on first play to direct them to it."* The mechanism the card checked (Enter pages instead of closing) works; **this is the auto-open policy itself**, one level up. Today `maybeAutoOpenHowTo()` ([cart-rave-menu.js:1318](../../src/cart-rave-menu.js:1318)) arms on the first menu show when `howtoSeen` is unset and no `?room=` param, then opens the overlay 600 ms later. Replace that with an attract state on the HOW TO PLAY button — glow/pulse/nudge while unseen, stopping the moment it is opened or the player starts a match. **Keep three things from the current design when it changes:** the `?room=` bail (a player following a friend's link must never be interrupted), the "arming writes nothing" rule from ONBOARD-FLAG-1 (`howtoSeen` is stamped when the overlay actually shows, never on intent), and `disarmHowToAutoOpen()`'s entering-a-match path. Respect `prefers-reduced-motion` for the pulse — see MOTION-A11Y-1. |
| High | ONBOARD-SIZE-1 — the how-to slide arrows and body text are too small | **Filed 08-06 from Wyatt's note on the ONBOARD-SLIDES-PT-1 PASS** — *"the forward and back buttons are too small by the way, same with the card text — it should be large and simple."* The deck reads and pages correctly (that is what PT-1 passed); this is the type-and-target-size pass over `.cr-howto-*` in [cart-rave-menu.css](../../src/cart-rave-menu.css). Two separate asks in the same note: the prev/next arrows want a real touch target (≥44 px, the same bar TOUCH-HOVER-1 used), and the card body wants to be bigger and plainer — this is the screen a brand-new player reads once, so favour few large words over dense copy. **Coordinate with ONBOARD-ART-1** — the art slots reserve a 28 rem column on four slides, so growing the text without knowing the final two-column widths risks doing the work twice; if ONBOARD-ART-1 is close, do the type pass after the slots turn on. Phone was already judged separately and passed (ONBOARD-SLIDES-PT-3), so this is not a mobile-only fix. **Implemented 08-07, committed `b893eba` (+ scope-correction `7f8b870`, see below) — CSS sizing only, no copy change (Wyatt's call): arrows 2rem→2.75rem/44px (`cr-howto-page`), body 13px→18px + brighter (0.6→0.72 alpha), name/chip text bumped to match, deck floor unchanged (nothing grew past 23rem). `npm run qa` 7/7. **Not deployed** — needs `npm run ship` and a Wyatt playtest pass on prod before this closes; do not add an `Owed: Wyatt playtest` row until then. Screenshot/visual confirmation and a real Tab-key focus-ring check were blocked this session by the Browser-pane panel not being displayed (computed-style values were verified instead — exact rem/color/line-height matches, confirmed by direct DOM inspection). **Unrelated finding while committing:** `cart-rave-menu.css` and `tests/onboardFirstRun.test.js` already carried uncommitted ONBOARD-ATTRACT-1 work (the `.cr-cmd--howto-attract` glow/pulse CSS + its matching test rewrite) sitting alongside this edit — that's a different card (row above) and was deliberately left untouched/uncommitted for whoever owns it. Also: `maybeAutoOpenHowTo`/`howtoAutoOpenArmed`, which the ONBOARD-ATTRACT-1 row above cites as the current mechanism, no longer exist in `cart-rave-menu.js` — replaced by `applyHowToAttract`/`clearHowToAttract`, committed under `463e389` (an unrelated-sounding message, "fix Settings music fill palette + muted keyboard volume steps"). That row's "todo" framing may be stale; worth a look before anyone re-implements it.** |
...
FRIENDS-LEVEL-1, FRIENDS-LEVEL-PT-1,
