...
| High | MENU-CART-FOLLOW-1 — menu cart preview follows the cursor slightly | **Landed and deployed 08-20** in `de3cbdca` (Worker `c28a585a-7ad7-424c-8d08-4e2060e7742b`; VERIFY_OK `index-Bf1l9Pph.js` / `cartPreview-DFpDtf1Q.js`). Desktop Medium/High main-menu preview maps the full-menu mouse position to at most ±5° yaw / ±2° lean, follows in ~180 ms, and returns in ~350 ms. One presentation-pose compositor blends cursor life out for the fake ram and back in on recovery; reduced motion, touch, mobile, Low, suspend, blur, and release reset it. Customize stays untouched. **PT-1 found vertical movement reversed; the one-line sign correction is local, uncommitted, and full-QA green.** **Owed: MENU-CART-FOLLOW-PT-1.** Do not reopen **MENU-CART-1**. |
...
| Low | MENU-CART-FOLLOW-PT-1 — menu cart cursor life is subtle and isolated `[1pc]` | **Owed: Wyatt playtest — MENU-CART-FOLLOW-PT-1 — the desktop menu cart lightly follows the cursor without fighting its fake ram.** Parent **MENU-CART-FOLLOW-1**. Not deployed — after commit use `npm run dev:local`; after ship, hard-refresh prod.<br>1. At desktop Medium/High quality, move the mouse from the centre to each menu corner. FAIL if the cart chases, snaps, translates, or rotates more than a subtle lean; PASS if it lightly turns toward the cursor.<br>2. Leave the menu and Alt-Tab out/in. FAIL if a stale tilted pose returns; PASS if the cart settles smoothly to the hero pose.<br>3. Wait through one fake ram. FAIL if cursor motion and the feint jitter or snap; PASS if the ram owns the pose and the cursor life returns smoothly.<br>4. Open and close Customize. FAIL if its preview tracks the cursor, or if the menu cart resumes tilted; PASS if Customize is unchanged and the menu cart rests on return.<br>5. Enable reduced motion, set Low, and use a ≤1024 px viewport. FAIL if the effect or cart appears; PASS if the cart is static/absent as before. |
