...
| Pri | Item | Notes |
|-----|------|-------|
| Low | BOOST-SFX-RESPAWN-PT-1 — charge-up sound survives other carts' respawns `[solo]` | **Owed: Wyatt playtest — BOOST-SFX-RESPAWN-PT-1 — the boost charge-up sound keeps playing until release, even when an NPC respawns mid-charge.** **Unshipped:** use `npm run dev:local` until the next ship; then repeat on production after a hard refresh.<br>1. Start a Solo round with 3 NPCs. Hold BOOST to charge and, while holding, ram an NPC off the arena. PASS if the charge-up sound keeps playing through the NPC's respawn (~1 s later) until you release; FAIL if the sound cuts out early while the bar is still filling.<br>2. Release at full charge. PASS if the BOOST sound fires on release; FAIL if the boost fires with no charge-up sound, or the charge sound restarts. |
