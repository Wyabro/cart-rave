...
| High | **HOST-ROLE-1 — weak host poisons every peer** | Run-5/6 F8: weak host poisons peers. **Lever 3 coded unpushed (2026-07-20):** lobby host-quality rebalance (`hostScore` on join, migrate if preferred ≥ current+20, toast `reason:host_quality`). Not a weak-host ban. Needs ship + dual-client lobby retest. |
