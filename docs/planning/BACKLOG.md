...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 queued | **NET-LAG-1** first · 9 playtest blockers (do not playtest until this block drains) |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NET-LAG-1 — Friends/QP lag + rubber-band before playtest | **Filed 08-19 from pre-playtest multiplayer.** Noticeable lag and rubber-banding in Friends, possibly Quickplay too. F8 captures on both machines. **Measure first** — correlate the captures and name the cause (transport, snapshot timing, interpolation/reconciliation, host perf, packet loss/jitter) before any netcode retune. Sibling **NET-PERF-1** (rewind-replay residual): absorb into it only if evidence points there; do not duplicate that lever. Do not reopen **NET-1** · **NET-2** · **NET-MIG-3**. Lane when picked: **Critical**. |
| High | CART-POP-1 — carts pop off the floor in normal driving | **Filed 08-19.** During ordinary driving, carts briefly pop/bounce off the ground with no jump and no obvious obstacle. Seen on all three arenas and in multiple modes — investigate shared cart physics, floor/collider contact, or suspension, not a level-specific lever. Measure first. Lane when picked: **Critical**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NPC-SELFKO-3 — measure and reduce NPC self-KOs on all three arenas | **Filed 08-19.** NPCs self-KO too often on all three arenas. First measure the actual self-KO rate per level and name the main causes (edge driving, hazards, nav errors, vortexes). Then tune AI/hazard avoidance. Do not blindly cut aggression or make NPCs overly cautious. New evidence — do not reopen **AI-ARENA-SELFKO-1** or **STOREROOMS-NPC-SELFKO-2**. |
