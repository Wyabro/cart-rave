...
| Low | SHARE-CARD-PT-1 — invite link shows the new arena lockup `[1pc]` | **Owed: Wyatt playtest — SHARE-CARD-PT-1 — paste cartclash.lol after ship and read CART CLASH on the preview.** Parent **SHARE-CARD-1**. Prod after ship.<br>1. After ship, paste `https://cartclash.lol/` into Discord.<br>2. FAIL if the preview is the old blank orange splat, or if CART / CLASH is unreadable at embed size.<br>3. PASS if the preview is the neon arena lockup with CART CLASH and the four carts. |
