...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **NET-LAG-1-PT-1** parked · **ONBOARD-WEBP-PT-1** on prod `14658bf8` |
...
| High | ONBOARD-WEBP-1 — HOW TO PLAY animated WebPs fail on some PCs | **Shipped 08-23** `14658bf8` Worker `e14acbd4`. VERIFY_OK `index-DiFdiFls.js`; live entry has `decoder-loop`. Files are valid infinite WebPs (loop=0, 2.8s). Chromium `<img>` can play once then freeze; unique `onboardLoop` remounts did not hold. Lever: paint frames on a canvas with `ImageDecoder`. Browsers without that API keep a plain `<img>`. Decode/load failure → still. **Owed: ONBOARD-WEBP-PT-1 on prod (hard-refresh).** Distinct from reduced-motion `still.webp` (ONBOARD-ART-1). Do not reopen **ONBOARD-SLIDES-1**. |
