...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **RESTART-ROUND-1** · **PAUSE-SLIDER-DELAY-1** · **MENU-SHORTWIN-1** · **CG-ZIP-1** · **CG-COVERS-1** |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 16 | 5 | 4 | 6 (+1 partial) |
...
more engineering. Open owed checks: **SOFTGL-DISMISS-PT-1** (local `?forcegpu=sw`) ·
...
Landed 09-01 — **SOFTGL-DISMISS-1** (software-GL modal one-shot per tab session); owed **SOFTGL-DISMISS-PT-1**.
1. **RESTART-ROUND-1** — pause RESTART ROUND must replay the current round, not RD n+1.
2. **PAUSE-SLIDER-DELAY-1** — pause overlay empty ~1 s before sliders show.
3. **MENU-SHORTWIN-1** — menu readable at CrazyGames iframe **1077 × 606**.
4. **CG-ZIP-1** — `dist/` zip, `index.html` at root, ≤ 50 MB, no SDK, desktop-only listing.
5. **CG-COVERS-1** — three covers + two silent videos (Wyatt art; required to upload).
6. **NET-LAG-1** — Friends/QP lag + rubber-band (F8 both machines; measure first).
...
| Pri | Item | Notes |
|-----|------|-------|
| High | RESTART-ROUND-1 — pause RESTART ROUND advances RD 1 → RD 2 | **Filed 09-01 from Wyatt prod playtest** (`d16fd523`). Button calls `onHostPlayAgainClick` ([pauseOverlay.js](../../src/ui/pauseOverlay.js) → [roundLifecycle.js](../../src/orchestration/roundLifecycle.js)), which is rematch: `startCountdown` writes a new running `startedAtMs`. **RD-COUNTER-1** then increments. Player expect: same round (clock, scores, RD n). Do not reuse rematch. Do not reopen **RD-COUNTER-1**. |
| High | PAUSE-SLIDER-DELAY-1 — pause overlay empty ~1 s before sliders show | **Filed 09-01 from Wyatt prod playtest** (`d16fd523`). High 09-01 (Wyatt). `show()` → `resetEscOverlayAnimState` zeros `.escSections` opacity, then staggered `animateMenuReveal`. Audio sliders live in those sections. Done: sliders visible on first paint of pause (skip or shorten the opacity-0 entrance on the audio card). |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | SOFTGL-DISMISS-PT-1 — software-GL modal stays gone after PLAY ANYWAY `[solo]` | **Owed: Wyatt playtest — SOFTGL-DISMISS-PT-1 — after PLAY ANYWAY, reload does not bring the software-GL warning back.** Parent **SOFTGL-DISMISS-1**. Use `npm run dev:local` with `?forcegpu=sw`. Prod `?forcegpu=sw` does not show this modal. Keep that flag on reload and on the new tab. Skip if the modal never appears.<br>1. Load with `?forcegpu=sw`. See the software-mode warning. Click PLAY ANYWAY.<br>2. Reload the same tab. FAIL if the warning returns.<br>3. Open a new tab to the same URL. PASS if the warning shows again. |
