...
[completed-work.md](./completed-work.md)). **HARNESS-NULL-1 ✅ CLOSED 08-06** · **remaining:**
HARNESS-FRIENDS-1 · HARNESS-FREEZE-1 (same `netharness.mjs` scenario lane, one tooling commit
each — take them in the same sitting). **HOOK-COMMENT-1 ✅ CLOSED 08-06** (one-line stale
comment) · **CC-ESC-1 ✅ CLOSED 08-06**. **ARCH-DRIFT-1 removed — closed**, shipped `91b39aa`.
...
| Medium | HARNESS-FREEZE-1 — netharness has no host-freeze / migration scenario | **Filed 08-06** from the same review, and the *only* one of its three E2E suggestions that is a genuine gap. `tools/netharness.mjs` covers `spawnlock`, `hostReload` (mid-round host tab reload, A6b), `teardownRejoin`, `shardOverflow` and `mpIntegration` — so **quickplay shard hops are already covered** (`shardOverflow`, QUICKPLAY-SHARD-1) and a host *reload* is covered. What is **not** covered is the host going quiet without dying: tab hidden / throttled rAF, then recovery — the exact case HOST-TAB-1 shipped against and the one that produces `host_send_gap`. A weak→strong host **migration** scenario is likewise manual-only; `applyHostMigration` has 20 unit cases in [hostMigration.test.js](../../tests/hostMigration.test.js) but nothing drives two real browsers through the handoff. **Scope if picked up: one scenario, not two** — host hide → assert joiner freeze arms and no ghost colliders → host visible → assert snapshots resume and the freeze clears. Migration is a second card if the first proves stable. **Note the hidden-tab rAF/WAAPI trap** (dev quirks): the harness must throttle via CDP page visibility, not by pausing the loop from inside the page, or it will test its own stub. **`tools/` — frozen during a game card;** needs its own tooling commit and an ack. Extends the NET-SIM-1 lane alongside HARNESS-FRIENDS-1. |
