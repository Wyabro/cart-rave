...
| **5** — SWEEP (cheap Lows) | 🟡 queued | MOTION-A11Y-1 · **PLAYREADY-RESET-FLAKE-1** · 08-16 audit Lows |
...
2. **PLAYREADY-RESET-FLAKE-1** — party-do playReady-reset test flakes under suite load (400 ms ceiling vs `connectAndSeat`).
3. 08-16 audit Lows, top first: **HOST-PRESENT-ORDER-1** · **ZOMBIE-ROOM-RESET-1** · **DEMOTE-COUNTDOWN-1**.
4. **MENU-SHORTWIN-1** — measured 08-19; the menu command list already overlaps the hint bar below ~618px viewport height.
...
| Low | PLAYREADY-RESET-FLAKE-1 — party-do `resets playReady timeout when a new human seats mid-wait` flakes under suite load | **Filed 08-19 during QP-PLAYING-1 qa.** `tests/party-do/cartRaveServer.test.js` sets the playReady ceiling to 400 ms, sleeps 280 ms, then `connectAndSeat`s a second human and asserts `gameStart` has not fired. Under full `npm run qa` / full party-do file load, that join can take >120 ms so the first ceiling fires during seat and the assert sees `gameStart`. Passes in isolation (`vitest -t "resets playReady timeout"`). Not a production playReady bug. Do not "fix" by widening sleeps unless the test still proves the reset; prefer a deterministic clock or an event wait. |
...
SPILL-DOUBLE-VFX-PT-1, FEEDBACK-1, FEEDBACK-PT-1, QP-PLAYING-1.
