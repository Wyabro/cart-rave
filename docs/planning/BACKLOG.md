...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 playtest | **RAPIER-MAJOR-PT-1** on `npm run dev:local` |
| **2** — PRE-SHIP (before public post) | ✅ clear | NPC-BOOST cluster closed 08-11 |
| **3** — WYATT LANE (blocked on you) | 👤 ongoing | **RAPIER-MAJOR-PT-1** · ANNOUNCER-RERECORD-1 · SHIP-1 D-tier cut/keep |
...
| 🟢 [Playtest owed](#playtest-owed) | 9 | 0 | 7 | 2 |
...
| Medium | RAPIER-MAJOR-1 — `@dimforge/rapier3d` 0.19 → 0.20 | **Code landed, playtest owed.** Both packages at `0.20.0` (wraps Rust 0.35). No knob retune. SIMD stays opt-in. Playtest: **RAPIER-MAJOR-PT-1** (solo, `npm run dev:local`) · **RAPIER-MAJOR-PT-2** (live MP, after ship). Do not close until both PASS. |
...
| Medium | RAPIER-MAJOR-PT-1 — solo drive + hop + KO + pit still feel like 0.19 `[solo]` | **Owed: Wyatt playtest — RAPIER-MAJOR-PT-1 — solo drive, hop, KO, and pit feel like 0.19.** Not on prod. Run `npm run dev:local`. Parent RAPIER-MAJOR-1. Physics 0.20 wraps Rust 0.35 — same knobs, new solver.<br>1. Menu → Solo → Cart Rave. Spawn. Drive a lap. Hop on flat ground. Charge-boost.<br>2. Ram an NPC to KO. Watch bounce and respawn. Drive over the center hole. Spill groceries.<br>3. Hit a pit stave at speed. FAIL if you sink through, hop dies, air-hops return, or KO bounce is wrong. PASS if drive, hop, KO, and pit match 0.19. |
| Medium | RAPIER-MAJOR-PT-2 — live host+joiner drive and KO `[2pc]` | **Owed: Wyatt playtest — RAPIER-MAJOR-PT-2 — joiner drive and a KO on the joiner still match the host.** Two browsers, Friends room. After ship + hard-refresh. Parent RAPIER-MAJOR-1.<br>1. Host and joiner both drive. FAIL if the joiner rubber-bands or cannot steer.<br>2. Host KOs the joiner. FAIL if the joiner does not shatter or respawn. PASS if both screens agree. |
...
PA-COMBO-1, PA-COMBO-PT-1, STORE-1-PT-1, STORE-MUSIC-PT-1.
