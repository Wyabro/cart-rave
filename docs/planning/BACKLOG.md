...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 playtest | **PA-COMBO-PT-1** on prod after hard-refresh |
| **2** — PRE-SHIP (before public post) | ✅ clear | NPC-BOOST cluster closed 08-11 |
| **3** — WYATT LANE (blocked on you) | 👤 ongoing | **PA-COMBO-PT-1** · ANNOUNCER-RERECORD-1 · SHIP-1 D-tier cut/keep |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 21 | 0 | 5 | 15 (+1 partial) |
| [Art](#art) | 6 | 0 | 0 | 6 |
| [Audio](#audio) | 3 | 0 | 2 | 1 |
| [Design / Gameplay](#design--gameplay) | 7 | 0 | 2 | 5 |
| 🟢 [Playtest owed](#playtest-owed) | 16 | 0 | 13 | 3 |
...
| Low | BINARY-F32NAME-1 — misnamed `encodeF32` | It returns values unchanged and is also used for the Float64 `tHost`. Rename to `toFiniteNumber`. |
| Low | ROUND-CLOCKDOMAIN-1 — podium MAX mixes clock domains | `now - runningAnchor - paused` mixes server-domain and host-domain terms. Rename/split `pausedWallMs` so the domains are explicit. |
| Low | CONSOLE-HI-1 — `%cHI :D` console line in prod | Decide keep vs gate behind DEV in `src/main.js`. |
...
| Medium | RAPIER-MAJOR-1 — `@dimforge/rapier3d` 0.19 → 0.20 | **Filed 08-12 from the dependency sweep.** `npm update` deliberately leaves it behind (caret range). Physics engine major — behavior-affecting: bump both `rapier3d` and `rapier3d-simd` together, run `npm run qa`, then a live multiplayer playtest (host-authoritative invariant untouched — same WASM stepping, verify no feel/KO regression). Watch Rapier 0.20 changelog for API breaks before starting. |
| Low | DEPS-MAJOR-1 — tooling majors: `sharp` 0.35 + `@cloudflare/vitest-pool-workers` 0.21 | **Filed 08-12 from the dependency sweep.** Dev-only, no player-visible behavior. `sharp` 0.34 → 0.35 also clears the libvips CVEs (`npm audit` high). vitest-pool-workers 0.18 → 0.21 pulls miniflare/wrangler/undici CVE fixes (2 high, 3 moderate today). TS 7 is owned by **TS-1** — not this card. Bump + `npm run qa`. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | PA-COMBO-1 — Savage/carnage PA lines actually play | Deployed 08-12 `c0a15308`. Dropped combo upgrades no longer burn the tier; combo lines queue on a busy channel. Playtest owed: **PA-COMBO-PT-1**. |
...
| Low | STORE-1-PT-1 — solo round still starts, ends, and rematches `[solo]` | **Owed: Wyatt playtest — STORE-1-PT-1 — a solo round still starts, scores, and rematches.** Not on prod. Run `npm run dev:local`. Parent STORE-1 deleted `src/gameState.js` locally (`04eba81`).<br>1. Menu → Solo → play. Watch 3-2-1. Drive. KO one cart. Podium shows.<br>2. Start another solo. Quit to menu mid-round. Lobby is clean. No leftover countdown digits.<br>3. Play again, then rematch. Countdown starts. Scores are 0.<br>4. After a KO, restart. The combo badge stays off during 3-2-1.<br>FAIL if a phase sticks, a score is wrong, or the Rampage badge shows on countdown. |
...
| Medium | CONN-TRACK-LEAK-PT-2 — same-clientId reconnect keeps one seat `[1pc]` | **Owed: Wyatt playtest — CONN-TRACK-LEAK-PT-2 — a hard refresh reclaims your seat instead of leaving a ghost.** CONN-TRACK-LEAK-1 (`9439cd2`, deployed `5ae6f69b`) refactored the server's ghost-exorcism teardown; this checks it live on prod.<br>1. Join a Friends room, seat, note your name and color.<br>2. Hard-refresh the tab.<br>3. FAIL if you land as a second human (old ghost seat still yours) or the room is hostless. PASS if you reclaim your seat and the lobby shows one human. |
| Medium | CARGO-BAY-INSTANCE-PT-1 — solo cargo still fills in order `[solo]` | **Owed: Wyatt playtest — CARGO-BAY-INSTANCE-PT-1 — a solo cart's cargo bays still fill in order as you score.** Parent CARGO-BAY-INSTANCE-1 (`9e86382`) switched bays to InstancedMesh. Not a look card — this is fill order.<br>1. Solo on prod (hard-refresh). Play long enough for the bays to fill.<br>2. Watch the groceries appear in the same front-to-back order as before.<br>3. FAIL if bays stay empty, fill in a random order, or flicker every frame. PASS if they fill in order and stay put. |
| Medium | CARGO-BAY-INSTANCE-PT-2 — spill and shatter still rebuild cargo `[solo]` | **Owed: Wyatt playtest — CARGO-BAY-INSTANCE-PT-2 — cargo hides on a spill and comes back after rebuild.** Same parent as PT-1; this is the hide/rebuild path, not fill order.<br>1. Solo on prod. Get some cargo in the bays.<br>2. Take a hit that spills, or shatter if you can.<br>3. FAIL if spilled groceries stay stuck in the bay, or a respawn/rebuild leaves the bay empty or doubled. PASS if cargo hides on the spill and the bay looks right after rebuild. |
...
| Medium | STORE-MUSIC-PT-1 — Storerooms plays the two new songs `[solo]` | **Owed: Wyatt playtest — STORE-MUSIC-PT-1 — The Storerooms plays the two new songs, not the old one.** Parent STORE-MUSIC-1. Prod after hard-refresh.<br>1. Menu → Solo → The Storerooms. Start the round. Listen to the music.<br>2. Rematch, or wait for the next song.<br>3. FAIL if you hear the old Storerooms track, silence, or a Cart Rave / Sundial song. PASS if both new songs play. |
| Medium | PA-COMBO-PT-1 — savage and carnage PA lines play on a chain `[solo]` | **Owed: Wyatt playtest — PA-COMBO-PT-1 — a two-KO chain speaks SAVAGE, not only RAMPAGE.** Parent PA-COMBO-1. Prod after hard-refresh. Deployed `c0a15308`.<br>1. Menu → Solo → play. KO one cart. Hear first spill or rampage.<br>2. Inside 5 seconds, ram and KO a second cart. The coupon says SAVAGE. The PA says SAVAGE or OUT OF CONTROL.<br>3. Inside 5 seconds, KO a third. The coupon says CARNAGE. The PA says CARNAGE or TOTAL CARNAGE.<br>FAIL if the coupon hits SAVAGE or CARNAGE and the PA stays on RAMPAGE or stays quiet. FAIL if two quick KOs never leave RAMPAGE on the coupon. PASS if the PA name matches the coupon. |
| Medium | NPC-BOOTH-TARGET-PT-1 — NPCs do not self-KO chasing a booth sitter `[solo]` | **Owed: Wyatt playtest — NPC-BOOTH-TARGET-PT-1 — NPCs stay on the floor while you sit on a spawn booth.** Parent NPC-BOOTH-TARGET-1. Prod after hard-refresh. Deployed `2fa4b2e4`.<br>1. Menu → Solo → play. Stay on your spawn booth. FAIL if an NPC drives off the floor or into the void toward you. PASS if they stay on the floor.<br>2. Jump off, die, respawn. Sit on the booth again. FAIL if NPCs chase you across the void before you land. PASS if they wait, then chase after you land.<br>3. Tie the score and enter Sudden Death. Sit on the booth. FAIL if the NPC self-KOs chasing you. PASS if it stays on the floor. |
| Medium | NPC-TYPE-DRAW-PT-1 — solo NPCs are three types; omit rotates on refresh `[solo]` | **Owed: Wyatt playtest — NPC-TYPE-DRAW-PT-1 — solo never fields two of one type, and the omitted type changes only after a hard-refresh.** Parent NPC-TYPE-DRAW-1. Prod after hard-refresh. Deployed `7aa16db4`.<br>1. Menu → Solo → play. Look at the three NPC emblems. FAIL if two match. Note the missing type.<br>2. Rematch without refresh. FAIL if the missing type changed. PASS if the same trio returns.<br>3. Hard-refresh. Solo again. FAIL if the missing type is the same as step 1, or if two emblems match. PASS if the omitted type changed and the three types are still distinct. |
| Medium | NPC-TYPE-DRAW-PT-2 — quickplay solo fields three distinct types `[solo]` | **Owed: Wyatt playtest — NPC-TYPE-DRAW-PT-2 — a one-human Quickplay room fields three different NPC types.** Parent NPC-TYPE-DRAW-1. Prod after hard-refresh. Deployed `7aa16db4`.<br>1. Menu → Quickplay. Sit and look at the three NPC emblems.<br>2. FAIL if two emblems match. PASS if you see three different types. |
