...
| **B** — pre-ship batch (16 items, best-first) | 🟡 in progress | #1 **UI-SCALE-1** (both passes) + **TOUCH-HOVER-1** ✅ CLOSED PASS 08-06; #2 **ONBOARD-SLIDES-1** is next |
...
| [UI / UX](#ui--ux) | 15 | 3 | 7 | 5 |
| [Tech Debt](#tech-debt) | 14 | 0 | 5 | 9 |

**86 open rows total** (08-06: 5 playtest owed rows closed PASS, TOUCH-HOVER-1's UI/UX stub
closed with it, 2 new UI/UX nits filed — KILLFEED-PHONE-1, ORIENT-TOAST-Z-1). Everything in
...
| Pri | Item | Notes |
|-----|------|-------|
| High | *(NET-LOOK-ACC-1 ✅ CLOSED PASS 08-06 — [completed-work.md](./completed-work.md))* | |
...
| High | ONBOARD-SLIDES-1 — how-to-play as click-through visual slides `[pre-ship]` | ⏳ **L1 SHIPPED 08-06 (`4c89e04`), UNPUSHED AND NOT DEPLOYED — L2 (the art) is blocked on an art source, see below.** L1 replaced the wall of text with a deck, **grown to eight slides 08-06 on Wyatt's call** (`ad8f890`): AISLE 1 DRIVE (controls, re-rendered per keyboard/gamepad/touch) · 2 BOOST · 3 RAM THEM OUT · 4 READ THE HUD · 5 CHAIN COMBOS · 6 THE LIVING STORE · 7 PRICE CHECK · 8 THE BOTS. Nothing dropped from the old screen. **Eight is safe because of ordering, not count** — the game is fully taught by AISLE 4, so 5–8 are depth a first-run player can bail on without losing the rules. **BOOST's copy carries a standing lesson (Wyatt, 08-06): verified-against-code is not the same as true-for-players.** The first draft was checked against [simulation.js:509-513](../../src/simulation.js:509) and stated the sub-100 ms silent cancel as a rule. Accurate, and wrong to ship — a deliberate tap lands *above* that threshold, so the dead zone is an accidental-brush filter nobody feels, and naming it would teach players to distrust tapping, which is a real move (a tap fires an instant burst at the 0.3 floor; holding scales it to 1.0). The shipped copy is the choice the player actually makes — **tap for a quick shove, hold for a big hit** — and the chips name that choice rather than the ms constants behind it. Apply the same test to any future mechanic copy. AISLE 4 carries the one **uncropped** still with callouts in percent coordinates; **those positions are provisional against the placeholder and need a pass when the real still lands.** The primary button **morphs** `NEXT ▸`→`LET'S ROLL` rather than closing, because `howtoSeen` is still stamped on open (ONBOARD-FLAG-1) and a closing focused default would have made "see 1 rule of 5, never asked again" the first-run happy path. No wrap-around, dots are non-focusable spans, no new keyboard handler (gamepadNav already owns the arrows). `npm run qa` 7/7 (1563 tests, +4); `npm run states` FAIL set byte-identical to a stash-measured pre-change baseline. **Absorbed 08-05: ONBOARD-1.** <br>**L2 BLOCKED — the plan's capture premise was falsified.** **Four** slides now ship a styled placeholder panel where a still goes — DRIVE, BOOST, RAM THEM OUT (all cropped 16:10) and READ THE HUD (**uncropped 16:9, HUD deliberately in frame** — it is the subject). `npm run shoot` cannot produce them: it renders the **attract state, so the arena is empty of carts**, it bakes the software-mode modal into the frame, and it runs on SwiftShader (see **SHOOT-SOFTGL-1**). A scratch rig that drives a real solo round with real GPU flags *does* produce the right look (verified: RTX 4090, 60 fps, correct dark neon), but framing is non-deterministic — the chase cam follows the local cart and NPC positions differ every run — so getting three *good* stills is a sample-and-hand-pick job, not a scripted one. **Wyatt is the art director and owns a 4090; three of his own screenshots are minutes of his time versus an hour of uncertain agent iteration.** Decide the art source before spending more: (a) Wyatt supplies 3 stills, agent crops/encodes/wires; (b) agent iterates the scratch rig; (c) drop the art slot and ship 5 text/chip slides, art as a follow-up. **Do not deploy L1 alone** — placeholder panels on prod. |
...
| High | *(TOUCH-HOVER-1 ✅ CLOSED PASS 08-06 — TOUCH-HOVER-PT-1, [completed-work.md](./completed-work.md))* | |
