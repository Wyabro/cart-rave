...
| **3** — WYATT LANE (blocked on you) | 👤 ongoing | SHIP-1 D-tier · **WARM-QP-ROTATE-PT-1** (after ship) · **BOOST-SFX-NONHOST-PT-1** |
...
- **BOOST-SFX-NONHOST-PT-1** — non-host boost whoosh when host `b` converts a live charge `[2pc]`.
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | BOOST-SFX-NONHOST-PT-1 — non-host boost whoosh when host `b` converts a live charge `[2pc]` | **Owed: Wyatt playtest — BOOST-SFX-NONHOST-PT-1 — the boost whoosh plays when a non-host fire is confirmed by the host.** Parent **BOOST-SFX-NONHOST-1**. Join as non-host. Hard-refresh first. Code is pushed; play on prod after ship.<br>1. Charge, then release. Hear the whoosh every fire.<br>2. If you can hitch mid-charge, hear the whoosh when the cart still boosts.<br>3. FAIL if a fire is silent. PASS if every fire whooshes and other SFX still play. |
...
ANIM-BUGS-1, ANIM-BUGS-PT-1, BOOST-SFX-RESPAWN-1, BOOST-SFX-RESPAWN-PT-1, BOOST-SFX-NONHOST-1, KO-DOOMED-PT-1,
