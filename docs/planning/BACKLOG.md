...
| 🟢 [Playtest owed](#playtest-owed) | 12 | 0 | 9 | 3 |
...
| Medium | PA-QUIET-PT-1 — one line per KO, 10 SECONDS cuts through `[solo]` | **Owed: Wyatt playtest — PA-QUIET-1 — one line per KO, 10 SECONDS cuts through.** Not on prod until ship. Run `npm run dev:local`.<br>1. Cause a multi-KO pileup. Confirm one line, no chain.<br>2. Cause a critical-hit first KO. Confirm FIRST BLOOD. Confirm CRITICAL HIT does not play.<br>3. Mid-round, KO the leader with no combo. Confirm LEADER DOWN plays.<br>4. At 10 s, start a line then wait for the clock. Confirm 10 SECONDS cuts in.<br>5. Confirm countdown / GO still cut through. |
