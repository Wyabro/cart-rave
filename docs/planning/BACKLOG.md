...
| High | ONBOARD-WEBP-1 — HOW TO PLAY animated WebPs fail on some PCs | **Landed 08-20** `51df06af` (Worker `819ad9ca`). Files are valid infinite WebPs (`loop_count=0`, ~2.8s). Visible-slide mount + 5-frame canvas sample. **PT FAIL 08-21:** each clip plays once, then freezes; it must loop while the slide is open. **Owed: ONBOARD-WEBP-PT-1 retest after the loop fix ships.** Distinct from reduced-motion `still.webp` (ONBOARD-ART-1). Do not reopen **ONBOARD-SLIDES-1**. |
