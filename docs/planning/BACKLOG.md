...
| Medium | PATTERNS-UI-4-PT-1 — medium readable cart pattern scale `[solo]` | **Code ready locally: QA 7/7 (190 files / 2,114 tests). Owed: Wyatt playtest — PATTERNS-UI-4-PT-1 — Diamond stays unchanged, while Maze, Honeycomb, and Cubes are smaller than UI-3 but remain readable and seamless.** Parent **PATTERNS-UI-4**. **PATTERNS-UI-3-PT-1 FAIL 08-15:** Diamond passed; the other three were too large.<br>1. In local dev, open CUSTOMIZE → PATTERN and select Pink.<br>2. Select Maze. PASS if six smaller turns span the cart texture and it still reads as a maze.<br>3. Select Honeycomb and Isometric Cubes. Rotate through front, side, and rear views. PASS if their repeated edges connect cleanly and Cubes still shows three cube faces. Select Diamond Weave. PASS if it is unchanged.<br>4. Repeat steps 2–3 with Blue, then start a solo round. PASS if every pattern stays stable while moving and after a KO rebuild.<br>5. After `ship it`, hard-refresh production and repeat steps 1–4. |
