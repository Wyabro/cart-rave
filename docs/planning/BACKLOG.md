...
| Pri | Item | Notes |
|-----|------|-------|
| High | RESTART-ROUND-1 — pause RESTART ROUND advances RD 1 → RD 2 | **Lever landed** (this commit). Pause RESTART ROUND calls `onHostRestartRoundClick`, not rematch. HUD `retainCurrentRound()` keeps RD n on the new clock anchor. Solo / testdrive only. Owed: **RESTART-ROUND-PT-1**. Do not reuse rematch. Do not reopen **RD-COUNTER-1**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | RESTART-ROUND-PT-1 — solo pause RESTART ROUND stays RD n | **Owed: Wyatt playtest — RESTART-ROUND-PT-1 — pause RESTART ROUND keeps RD 1.** Parent **RESTART-ROUND-1**. `npm run dev:local`, or prod after ship.<br>1. Solo match. Play until the HUD shows RD 1 and the clock is not full. Open pause.<br>2. Press RESTART ROUND. Wait for GO.<br>3. FAIL if the HUD shows RD 2, if the clock keeps the old remaining time, if scores are not 0, or if Sudden Death is still on.<br>4. PASS if the HUD still shows RD 1, the clock is a full round, scores are 0, and Sudden Death is off. |
