...
| High | ANLX-ATTRACT-1 — attract mode is being counted as real matches | **Do this BEFORE the pre-tester `DELETE /api/analytics`, or the reset buys nothing.** Prod DO (07-30) holds 20,000 rows — at ring cap, already evicting — of which `match_started=9721` / `match_ended=9548` come from only **263 sessions / 8 clients**: ~37 matches per session averaging **1,828 ms** when a real round is 150 s, with `resultSplit loss=9330 / win=9`. Mechanism located, not guessed: `gameplayAnalytics.js:100-125` subscribes to `gameStore.roundPhase` and emits `match_started` on **any** →RUNNING and `match_ended` on RUNNING→PODIUM, with no gate for attract mode / harness / `?diag`. The menu attract demo drives the same store, so every attract cycle books a ~1.8 s "match". Consequence: every gameplay aggregate (`avgKos`, `matchesByArena`, result split, quit funnel) is currently meaningless, and real tester data would be buried the same way. Cheapest test: sit on the prod menu in attract and watch `/api/analytics` beacons (or the DEV consoleSink batches) — no round played, events still fire. Fix direction: gate the subscription on a real session (attract flag / `isAttract`), not on build type. |
