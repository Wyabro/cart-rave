...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 21 | 0 | 4 | 16 (+1 partial) |
| [Art](#art) | 6 | 0 | 0 | 6 |
| [Audio](#audio) | 2 | 0 | 1 | 1 |
...
| Low | CHAL-PODIUM-DEDUPE-1 — podium round-event dedupe key only set when `startedAtMs > 0` | **Filed 08-13 from the challenge-stack audit.** `recordPodiumStats` (roundLifecycle.js:110-124) skips the `startedAtMs:winKey` dedupe latch when `startedAtMs` is falsy — a redelivered MSG.round on that theoretical edge would double-credit ROUND_*. Real rounds always set it; guard-depth nit. |
...
CHAL-DEAD-EXPORT-1, ZAN-REACTIVE-ALLOC-1.
