...
| Pri | Item | Notes |
|-----|------|-------|
| High | FRIENDS-LEVEL-1 — arena picked on the FRIENDS menu is not the arena the private lobby opens on | **Filed 08-06 from Wyatt's note on the DIFF-FRIENDS-PT-1 PASS** — *"i had the storerooms selected but the friends lobby went to cart rave. did something break?"* The card itself passed; this is a different mechanism and gets its own row. **Investigate before asserting a bug** — first question is whether a fresh private room is *meant* to open on the default arena and take the pick only at round start. **Strong prior that it is the DIFF-FRIENDS-1 shape all over again:** the server stamps a room level and broadcasts `levelId` on hello/round, and the client latches it via `adoptAuthoritativeRoomLevel` ([netcode.js:3399](../../src/netcode.js:3399), called from `:2914` and `:3263`) — while the only host-side push, `adoptRoomLevelAsHost` ([netcode.js:3431](../../src/netcode.js:3431)), is called from exactly one place, `roundLifecycle.js:1071`, at **arena rotation**, not at room create/enter. So the menu pick has no path to win the hello, exactly as the difficulty pick had none before `adoptFriendsHostAiDifficultyFromStore()`. If confirmed, the fix mirrors that one; **do not fold it into DIFF-FRIENDS-1**, which is shipped and passed. Repro: FRIENDS → pick THE STOREROOMS → create private room → read the lobby arena. |
