...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave B candidate · **CART-POP-PT-1** owed after deploy · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 8 playtest blockers |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave B candidate 08-20.** Three production captures show ordinary floor rebounds: pre-step `vy -8.72` → `+1.57` in one physics step, generally floor-only. Cart Rave / Storerooms pair the cart 0.30 and floor 0.05 into 0.175; the shared cart collider now uses restitution Min, so a floor's authored 0.05 wins without an arena-specific lever. **Owed: CART-POP-PT-1 after deploy.** Lane: **Critical**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | CART-POP-PT-1 — normal driving stays planted across all arenas `[1pc]` | **Owed: Wyatt playtest — CART-POP-PT-1 — carts stay planted during ordinary driving on Cart Rave, Sundial Station, and The Storerooms.** Parent **CART-POP-1**. Pushed but not yet deployed.<br>1. On production with `?diag=1`, drive normally for about 30 seconds on each arena; include straight runs and full-lock turns, without hop or boost.<br>2. FAIL if a cart pops or bounces off the flat floor without a ram, obstacle hit, or edge contact.<br>3. PASS if ordinary driving stays planted on all three arenas while rams and obstacle hits still have visible lift.<br>4. Press F8 once after each arena. |
...
| Low | MENU-MUSIC-2C-PT-1 — one menu song on title boot `[1pc]` | **Owed: Wyatt playtest — MENU-MUSIC-2C-PT-1 — cold boot into the main menu plays exactly one song.** Parent **MENU-MUSIC-2C**. Prod after ship (hard-refresh).<br>1. Hard-refresh the title screen on prod (`https://www.cartclash.lol/`). Wait until the splash is gone.<br>2. FAIL if you hear both menu songs at once, or one song stacked on itself.<br>3. PASS if you hear one song, then the other after the first ends.<br>4. Quit a match back to the menu and listen again. Same FAIL/PASS. |
