...
**QP-ROTATE-PT-1** (two machines) · **SHARD-PT-2** (launch day) · **SUNGLASSES-OBSIDIAN-PT-1**
(solo cosmetic). Closed PASS history lives only in
[completed-work.md](./completed-work.md).
...
- **DEEPSEC-2** — parked DeepSec residuals (token rotate + DO limiters). Do not pick while DEEPSEC-1 is open.
...
| Low | GAMEPAD-MENU-ROUTES-PT-1 — controller pause-menu navigation `[solo]` | **Owed: Wyatt playtest — GAMEPAD-MENU-ROUTES-PT-1 — the in-game pause menu is fully controllable with a controller.** Parent **UI-INPUT-LIFECYCLE-1** is deployed in Worker version `70bf742a-0b6c-4214-be27-4697d950fbcc`.<br>1. Start a solo round with a controller. Press START and confirm RESUME has the focus ring immediately.<br>2. Use D-pad or left stick to reach audio sliders, quality, RESTART ROUND, and MAIN MENU. Adjust one slider, then use A to resume. Pause again and use B to resume from another control.<br>3. FAIL if focus does not move, A or B does nothing, focus reaches gameplay or the main menu behind the pause screen, or resuming causes a boost/hop. PASS if every visible pause control is reachable and gameplay needs a fresh action press after resume. |
| Low | PAUSE-CHARGE-SFX-PT-1 — pause cancels charge-up audio `[solo]` | **Owed: Wyatt playtest — PAUSE-CHARGE-SFX-PT-1 — pausing never leaves the charge-up sound looping.** Parent **UI-INPUT-LIFECYCLE-1** is deployed in Worker version `70bf742a-0b6c-4214-be27-4697d950fbcc`.<br>1. Start a solo round. Hold boost until the charge-up sound is clear, then open the pause menu while charge is active.<br>2. Stay paused for five seconds, resume with A, release all controls, then perform one fresh boost press.<br>3. FAIL if charge-up repeats or continues while paused, continues after resume, or the resume press fires a boost. PASS if pause stops the old charge and only the fresh boost starts a new charge sound. |
...
| Low | SUNGLASSES-OBSIDIAN-PT-1 — Obsidian shades read dark, not silver `[solo]` | **Owed: Wyatt playtest — SUNGLASSES-OBSIDIAN-PT-1 — the Obsidian mirror finish reads as dark black glass, clearly distinct from the default Silver.** Parent **SUNGLASSES-OBSIDIAN-1** (`fe91388`, deployed 08-14) darkened the obsidian gradient core/mid (was near-white `#effcff`, nearly identical to Silver's `#f4f7ff`) to dark slate; menu chip swatch and in-game visor both derive from the same stops.<br>1. Hard-refresh production and open CUSTOMIZE → SUNGLASSES.<br>2. Select Obsidian and Silver in turn; compare the visor on the preview cart and the menu chips.<br>3. FAIL if Obsidian still reads bright/silver like the default. PASS if it reads as dark black glass with a subtle cool glint. |
