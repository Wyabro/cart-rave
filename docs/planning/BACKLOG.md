...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **NET-LAG-1-PT-1** parked · **FRIENDS-ROTATE-PT-1** owed · **ONBOARD-JUMP-1** queued |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 16 | 4 | 4 | 7 (+1 partial) |
| [Design / Gameplay](#design--gameplay) | 3 | 1 | 2 | 0 |
| 🟢 [Playtest owed](#playtest-owed) | 9 | 0 | 0 | 9 |
...
2. **FRIENDS-ROTATE-1** — Friends rematch rotates arenas, synced.
3. **ONBOARD-JUMP-1** — HOW TO PLAY matches jump+boost.
4. **ONBOARD-WEBP-1** — HOW TO PLAY WebP playback + fallback.
5. **MENU-CART-FOLLOW-1** — subtle menu-cart cursor parallax.
Landed 08-21 — **SHARE-CARD-1** (OG share card 1200×630 arena lockup). **SHARE-CARD-PT-1** owed after ship.
...
| High | ONBOARD-WEBP-1 — HOW TO PLAY animated WebPs fail on some PCs | **Landed and deployed 08-20** in commit `51df06af` (Worker `819ad9ca-ce02-46d0-aff4-c1523921e8cb`). The source files are valid infinite WebPs; the old F8 report was static despite Windows animation effects being on. Motion now mounts only on the visible slide and samples five tiny frames over ~700 ms. A frame change keeps motion; no change, a load error, or a sample error uses the paired still. Navigation and hidden tabs do not produce a verdict. Local in-app-browser repro fell back cleanly on all five art slides. **Owed: ONBOARD-WEBP-PT-1 on the brother's F8 machine.** Distinct from reduced-motion `still.webp` (ONBOARD-ART-1). Do not reopen **ONBOARD-SLIDES-1**. |
| High | MENU-CART-FOLLOW-1 — menu cart preview follows the cursor slightly | **Landed and deployed 08-20** in `de3cbdca`; vertical direction corrected in `ba5a62b7` (Worker `79cc4526-d2df-44a3-918c-8f676299d1b7`; VERIFY_OK `index-BlUL-a0f.js` / `cartPreview-C8wWRAJu.js`). Desktop Medium/High main-menu preview maps the full-menu mouse position to at most ±5° yaw / ±2° lean, follows in ~180 ms, and returns in ~350 ms. One presentation-pose compositor blends cursor life out for the fake ram and back in on recovery; reduced motion, touch, mobile, Low, suspend, blur, and release reset it. Customize stays untouched. **PT-1 found and this deployment corrected vertical movement.** **Owed: MENU-CART-FOLLOW-PT-1.** Do not reopen **MENU-CART-1**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | ONBOARD-JUMP-1 — HOW TO PLAY matches jump+boost | **Filed 08-19.** Live input now fires jump + boost together. HOW TO PLAY still teaches them as separate (`HOWTO_CONTROLS`: SHIFT=BOOST, SPACE=HOP; AISLE 2 is BOOST-only). Update instructions and any control graphics/text so keyboard, gamepad, and touch match the live scheme. Do not reopen **ONBOARD-SLIDES-1**. |
...
| Low | FRIENDS-ROTATE-PT-1 — Friends rematch rotates arenas in sync `[2pc]` | **Owed: Wyatt playtest — FRIENDS-ROTATE-PT-1 — after a Friends match, PLAY AGAIN lands every client on the same next arena.** Parent **FRIENDS-ROTATE-1**. Prod `1a740c79` (hard-refresh) with `?diag=1` on both machines.<br>1. Friends room, two machines, F8 both. Play a match, then PLAY AGAIN.<br>2. FAIL if the two clients load different arenas, or if GO starts while the swap is still running, or if rotation plus warmup stays hidden more than about 10s.<br>3. PLAY AGAIN again. PASS if both wrap the same way (classicRecord → backrooms → zanzibar → classicRecord) and `arena/rotation_finished` target matches on both.<br>4. One Quickplay rematch on the same build. FAIL if QP rotation regresses.<br>5. Press F8 on both machines after the Friends wrap. |
| Low | ONBOARD-JUMP-PT-1 — gamepad jump/boost match HOW TO PLAY `[1pc]` | **Owed: Wyatt playtest — ONBOARD-JUMP-PT-1 — A hops and RT boosts, and every chart says so.** Parent **ONBOARD-JUMP-1**. Not deployed — `npm run dev` with a gamepad (or prod after ship).<br>1. Open HOW TO PLAY and land on AISLE 1 — chips read RT BOOST / A HOP.<br>2. Start a round. Press A — the cart hops. Hold RT — the cart boosts/charges.<br>3. FAIL if A boosts or RT hops, or if any chart (HOW TO PLAY, Settings, pause CONTROLS, main menu) disagrees with what the pad does.<br>4. Pause mid-round and check the CONTROLS card; check Settings > CONTROLS — RT/B boost, A/LT hop. |
| Low | ONBOARD-WEBP-PT-1 — HOW TO PLAY media moves or falls back cleanly `[1pc]` | **Owed: Wyatt playtest — ONBOARD-WEBP-PT-1 — the HOW TO PLAY pictures move when supported and stay clean when they cannot.** Parent **ONBOARD-WEBP-1**. Prod `https://www.cartclash.lol/` after a hard-refresh with `?diag=1`.<br>1. Open HOW TO PLAY. Page through DRIVE, BOOST, RAM THEM OUT, READ THE HUD, and THE LIVING STORE.<br>2. FAIL if any picture is blank, broken, jumps in size, or locks the menu. A clean still picture is an acceptable fallback when motion cannot play.<br>3. PASS if each picture either moves or changes to a clean matching still within about a second; the teaching text and controls stay readable.<br>4. Press F8 after the five picture slides. |
| Low | MENU-CART-FOLLOW-PT-1 — menu cart cursor life is subtle and isolated `[1pc]` | **Owed: Wyatt playtest — MENU-CART-FOLLOW-PT-1 — the desktop menu cart lightly follows the cursor without fighting its fake ram.** Parent **MENU-CART-FOLLOW-1**. Deployed: hard-refresh `https://www.cartclash.lol/`.<br>1. At desktop Medium/High quality, move the mouse from the centre to each menu corner. FAIL if the cart chases, snaps, translates, or rotates more than a subtle lean; PASS if it lightly turns toward the cursor. Confirm up and down now move in the expected vertical direction.<br>2. Leave the menu and Alt-Tab out/in. FAIL if a stale tilted pose returns; PASS if the cart settles smoothly to the hero pose.<br>3. Wait through one fake ram. FAIL if cursor motion and the feint jitter or snap; PASS if the ram owns the pose and the cursor life returns smoothly.<br>4. Open and close Customize. FAIL if its preview tracks the cursor, or if the menu cart resumes tilted; PASS if Customize is unchanged and the menu cart rests on return.<br>5. Enable reduced motion, set Low, and use a ≤1024 px viewport. FAIL if the effect or cart appears; PASS if the cart is static/absent as before. |
| Low | SHARE-CARD-PT-1 — invite link shows the new arena lockup `[1pc]` | **Owed: Wyatt playtest — SHARE-CARD-PT-1 — paste cartclash.lol and read CART CLASH on the preview.** Parent **SHARE-CARD-1**. Prod `dfda0c21` Worker `d4738242` (hard-refresh / new Discord message).<br>1. Paste `https://cartclash.lol/` into Discord (a new message, not an old unfurl).<br>2. FAIL if the preview is the old blank orange splat, or if CART / CLASH is unreadable at embed size.<br>3. PASS if the preview is the neon arena lockup with CART CLASH and the four carts. |
...
BOOT-TBT-1, LAST-STANDING-DEAD-1, DEEPSEC-1, NPC-BOOTH-TARGET-1, NIGHT-SHIFT-BLOCKOUT-1, NIGHT-SHIFT-CITY-1, CART-MODEL-1, SHADES-MAT-1, MENU-CART-1, FRIENDS-JOIN-LAYOUT-1, FRIENDS-LEVEL-1, FRIENDS-LEVEL-PT-1, ONBOARD-ATTRACT-1,
ONBOARD-ART-1,
...
NPC-SELFKO-3, NPC-SELFKO-3-PT-1, CART-POP-1, CART-POP-PT-1, SHARE-CARD-1.
