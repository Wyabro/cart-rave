...
| ✅ | VFX-1 endgame: display-referred bloom promoted to default | **Closed 07-17** (`adea4bf`) — all-arena default; the flickery HDR split survives only behind `?bloompipe=hdr`. Remaining (optional): real-HW `?blackmon=1` taste pass, then delete the legacy fork paths. |
