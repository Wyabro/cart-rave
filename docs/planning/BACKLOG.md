...
| Done | COUNTDOWN-WARM-1 — round-start shader/composer stall `[SHIP-1 A1]` | Fly-over camera hard-cuts to a never-warmed wide/high orbit right at countdown start. Fixed with a second hidden warm-up pass from that framing (`camera.js` + `main.js warmupActiveSceneShaders`, 07-21). Real but distinct from COUNTDOWN-SYNC-1 below — this was frame-stutter, not beat timing. |
| 🟡 Partial | COUNTDOWN-SYNC-1 — countdown beat desync / "skips" `[SHIP-1 A1]` | The actual complaint: 3/2/1/GO beats fire but wildly unevenly spaced — non-host measured the 1→GO gap collapsing to ~90ms (should be ~1200ms), twice, across two rounds. Root cause: `hud.js updateStatus()` computes the shown digit by polling each frame, but GO fires on an edge-detected phase transition independent of it — a stall spanning the last digit-window lets phase flip to `running` before the digit branch ever observes n===1, skipping it. Fixed (07-21): retroactively fire the missed "1" beat, staggered 220ms before GO, generation/phase-guarded. Presentational only (gameplay unlock already gated on phase, not this banner). Needs Wyatt's playtest — could not force a real stall to verify the catch-up path headlessly. |
