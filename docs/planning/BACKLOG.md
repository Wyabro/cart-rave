...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 playtest | **PA-COMBO-PT-1** on `npm run dev:local` |
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | PA-COMBO-1 — Savage/carnage PA lines actually play | Landed 08-12 (`ff8175b`). Dropped combo upgrades no longer burn the tier; combo lines queue on a busy channel. Playtest owed: **PA-COMBO-PT-1**. |
