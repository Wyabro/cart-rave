...
| **2** — PRE-SHIP (before public post) | 🟡 queued | **COUNTDOWN-HOST-STAMP-1** · SD-SPECTATOR-CHARGE-1 (**REMOTE-INPUT-STALE-1** landed, PT owed; **NPC-ABORT-BURST-1** landed, PT owed; **SD-SCORE-STALE-1** Wyatt PASS; **THOST-CEILING-1** Wyatt PASS; **GAMEPAD-FREEZE-1** · **ZOMBIE-HOST-PICK-1** Wyatt PASS) |
...
(see `## Playtest owed`).
1. **COUNTDOWN-HOST-STAMP-1** — first non-host 3-2-1 uses host start while host clock is unlocked.
...
- Structure debt after multiplayer is proven: **DIR-1** · **CB-SEAM-2** · **BUNDLE-KEY-1** · **LATE-BIND-1** · **COMPOSER-ORDER-1** · **GLTF-1** · **DUAL-1** · **TS-1** · **TOOL-1** · **BACKLOG-GATE-2** · **PARTY-SERIALIZE-1** · **HOWLER-UPGRADE-1**.
...
| Medium | COUNTDOWN-HOST-STAMP-1 — first non-host 3-2-1 hangs on 3 then slams 2+1 `[2pc]` | **Filed 08-16 from THOST-CEILING-PT-1 F8s (cap-367 · cap-368).** Not a THOST fail: guest `snapGapMaxMs` was 57 / 31. HUD digits use host-clock offset. First countdown still has `hostClock.samples === 0`. `host_round` then overwrites the `gameStart` local start with a host-domain `countdownStartedAtMs`. Guest timeOrigin is ~1.8 s behind → announcer `3` at +10 ms, `2` at +2.9 s, `1` at +3.5 s. Same-session rematch (cap-369) is spaced right once the offset is locked. Candidate lever: do not apply `host_round` / hello `countdownStartedAtMs` while `hostClock.samples === 0`. Do not add `tHost` to `host_round` on the first lever (needs a Worker deploy). Mid-join during countdown is out of that lever. Not a reopen of COUNTDOWN-SYNC-1 / COUNTDOWN-QUICKPLAY-1 / THOST-CEILING-1. |
