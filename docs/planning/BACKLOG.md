...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **NET-LAG-1-PT-1** parked · **ONBOARD-WEBP-1** canvas decoder loop · **ONBOARD-WEBP-PT-1** after ship |
...
2. **ONBOARD-WEBP-1** — HOW TO PLAY WebP playback must loop (PT FAIL 08-21 and 08-23).
...
| High | ONBOARD-WEBP-1 — HOW TO PLAY animated WebPs fail on some PCs | **Wave 3 08-23.** PT FAIL on prod `310a5f86` (query remount). Files are valid infinite WebPs (loop=0, 2.8s). Chromium `<img>` can play once then freeze; unique `onboardLoop` remounts did not hold. Lever: paint frames on a canvas with `ImageDecoder` (own the loop). Browsers without that API keep a plain `<img>`. Decode/load failure → still. **Owed: ONBOARD-WEBP-PT-1 after ship (hard-refresh).** Distinct from reduced-motion `still.webp` (ONBOARD-ART-1). Do not reopen **ONBOARD-SLIDES-1**. |
