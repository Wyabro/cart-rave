...
| [Design / Gameplay](#design--gameplay) | 5 | 2 | 3 | 0 |
...
Landed 09-14 — **SITE-TRUST-1** (privacy, terms, contact, Leave Feedback under Controls, branded 404); **SITE-TRUST-PT-1** owed on prod `c58c96ff`.
1. **CG-COVERS-1** — three covers + two silent videos (Wyatt art; required to upload).
2. **NET-LAG-1** — Friends/QP lag + rubber-band (F8 both machines; measure first).
3. **ONBOARD-COPY-1** — HOW TO PLAY AISLE 1+2 body matches live input mode. Landed 09-07; **ONBOARD-COPY-PT-1** owed.
...
| Pri | Item | Notes |
|-----|------|-------|
| High | SITE-TRUST-1 — public information and analytics choice | **Shipped 09-14** prod `c58c96ff` Worker `aed9d44e-7139-4af7-92ee-1625307d7147`. Critical: static routing. Publisher Redshift Studios; support redshiftstudiossupport@gmail.com; analytics on by default per Wyatt. Settings: Privacy & Support under Controls, includes Leave Feedback. All optional provider loaders respect saved off. Public pages load no game or analytics. Preserve real 404 status and API/multiplayer routes. PNG uses a reduced palette (1,137,264 to 300,645 bytes). Checks: QA 2,435/2,435, VERIFY_OK `index-_uKl7Ppw.js`. Playtest: **SITE-TRUST-PT-1**. No claim of legal compliance or human PASS. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | CG-COVERS-1 — CrazyGames cover images + hover videos | **Filed 09-01.** High 09-01 (Wyatt). Required to submit, not to build `dist/`. Sizes: landscape 1920×1080, portrait 800×1200, square 800×800. Videos: 15–20 s, no sound, landscape 16:9 1080p and portrait 2:3 1080p. Title on covers only. No "Play now", borders, or store logos. [Game covers](https://docs.crazygames.com/requirements/game-covers/). |
| High | ONBOARD-COPY-1 — HOW TO PLAY AISLE 1+2 copy matches live input | **Filed 09-07.** Chips already swap; body is frozen as "Steer with the stick" next to WASD. AISLE 1+2 body + gamepad chips (RT·B, A·LT) follow `onInputModeChange`, same as chips. **AISLE 1:** Keyboard "Steer with WASD. Press Space to hop." · Gamepad "Steer with the stick. Press A or LT to hop." · Touch "Drag the stick to steer. Tap Hop." **AISLE 2:** Keyboard "Tap Shift…" · Gamepad "Tap RT or B…" · Touch "Tap Boost…" then the shared hold/auto-fire line. No pit mention (not on every arena). Files: `index.html`, `src/ui/cart-rave-menu.js`, `tests/ui/onboardFirstRun.test.js`. Out of scope: AISLE 3–8, LOAD_TIPS, pause/settings tables. Shipped 09-08 prod `39010c93` Worker `0c962927`. Playtest: **ONBOARD-COPY-PT-1**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | SITE-TRUST-PT-1 — privacy choice and public pages `[solo]` | **Owed: Wyatt playtest — SITE-TRUST-PT-1 — privacy controls and public pages work.** Parent **SITE-TRUST-1**. Prod `c58c96ff` (Worker `aed9d44e`, hard-refresh https://www.cartclash.lol/).<br>1. Open Settings. Privacy & Support is under Controls and includes Leave Feedback. Open Privacy & Analytics. Confirm Redshift Studios and the support email.<br>2. Turn analytics off. Return to the game. Reopen the privacy page; the off choice stays saved. Turn it on again.<br>3. Check Terms, Contact, and Leave Feedback. Do not send a test email. Leave Feedback may open a new tab.<br>4. On a phone-sized window, confirm readable text, usable controls, and no extra settings scrollbar. Open an unknown path; confirm the branded 404 and return link.<br>5. PASS if the choice persists and the links work. Otherwise record the failing step. |
| Low | ONBOARD-COPY-PT-1 — HOW TO PLAY AISLE 1+2 matches the live input `[solo]` | **Owed: Wyatt playtest — ONBOARD-COPY-PT-1 — HOW TO PLAY names the buttons for the device you are using.** Parent **ONBOARD-COPY-1**. Prod `39010c93` (Worker `0c962927`, hard-refresh https://www.cartclash.lol/).<br>1. Open HOW TO PLAY on keyboard. AISLE 1 says steer with WASD and press Space to hop. AISLE 2 names Shift. FAIL if it says stick next to WASD, or mentions a pit.<br>2. Plug in a pad (or press a pad button). AISLE 1 says stick and A or LT. AISLE 2 names RT or B. Chips show RT and B for boost, A and LT for hop. FAIL if B is labeled BACK, or if copy does not change when the pad wakes.<br>3. On a phone, AISLE 1 says drag the stick and tap Hop. AISLE 2 names Boost. |
