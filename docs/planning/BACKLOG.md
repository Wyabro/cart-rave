...
| Pri | ID | Item | Notes |
|-----|----|------|-------|
| Medium | DEV-GRAPH-1 | Model-free development plan graph | **Wyatt-selected 08-11.** Build only the fail-closed control-plane prefix: clean-worktree preflight → plan artifact → review artifact → explicit human ACK. Persist atomic state and artifacts under ignored `.agent/runtime/`; bind review to the plan SHA-256; keep one persistent run lock. No model, shell, source-edit, commit, deploy, closure, or player-playtest node. DeepSeek integration waits for `LOOP-SAFETY-2`. |
