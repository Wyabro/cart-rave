...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | MENU-MUSIC-FIRST-PT-1 — menu music starts on first visit after one click | **Owed: Wyatt playtest — MENU-MUSIC-FIRST-PT-1 — first visit, one click on the menu starts the music.** Parent **MENU-MUSIC-FIRST-1**. Do not use `npm run dev` — the DEV music gate hides this. Prod after ship, or `npm run build` then `npm run preview`. Incognito / cleared site data.<br>1. Incognito window. Load the site. Wait for the menu. Do not click yet.<br>2. Click anywhere on the menu once.<br>3. FAIL if music stays silent after that click. Silent before the click is autoplay policy, not a fail.<br>4. Press F8 if silent after the click. |
