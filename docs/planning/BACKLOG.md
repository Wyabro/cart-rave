...
| High | RESULTS-PT-1 — the results screen reads as one composition `[solo]` | **Owed: Wyatt playtest — RESULTS-PT-1 — after a match, the podium and the receipt read as one screen instead of two things floating apart.** Shipped 08-06 (unpushed as of seed — use `npm run dev`, or ship first). The podium used to sit hard against the left edge with a big empty gap before the receipt; it now sits right next to it. **Nothing about what is shown changed — judge the arrangement only.**<br>1. Play a solo round to the end and let the results screen come up.<br>2. Look at the bottom half: the four rank blocks should sit just to the left of the receipt card, with a normal gap between them — not a wide empty stripe.<br>3. Check nothing is cut off at the left edge of the screen, and the blocks do not touch or overlap the receipt or the PLAY AGAIN / MAIN MENU buttons.<br>4. **If you have the ultrawide handy, look there too** — the pair should sit in the middle of the screen with the empty space split evenly either side, sitting under the centred title. (The first build parked them against the right edge; you called it, and it was changed before this card was seeded.)<br>5. PASS if the screen reads as one composition. FAIL with the window size and what looked wrong. |
