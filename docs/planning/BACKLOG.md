...
| **B** — pre-ship batch (16 items, best-first) | 🟡 in progress | #2 **ONBOARD-SLIDES-1** shipped 08-06, awaiting playtest (art split to **ONBOARD-ART-1**); #3 **RESULTS-1** is next |
...
| **F** — sweep-day batch (Lows, one commit each) | ⚪ not started | **10 items, re-ranked 08-06 into desk-only vs needs-a-look.** Desk-only first: MONTAGE-ESC-1 → RAPIER-DEFAULT-MAX-1 → STATES-DEAD-1 (+KBM-TOAST-1) → SPINDLE-COLOR-DEAD-1 → RESULTS-GLOW-1 |
| **G** — tooling-window batch | 🟡 **2 open, not drained** | **MONTAGE-ESC-1** (High — `npm run states`/`sheet`/`podium`/`loadshots` all throw) and **SHOOT-SOFTGL-1**; the 08-06 "fully drained" line counted only the cards filed *before* them |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 24 | 1 | 13 | 9 (+1 partial) |
| [Art](#art) | 16 | 2 | 6 | 8 |
| [Audio](#audio) | 5 | 0 | 3 | 2 |
| [Design / Gameplay](#design--gameplay) | 9 | 0 | 3 | 6 |
| 🟢 [Playtest owed](#playtest-owed) | 6 | 3 | 1 | 2 |
| [UI / UX](#ui--ux) | 14 | 2 | 8 | 4 |
| [Tech Debt](#tech-debt) | 18 | 1 | 6 | 11 |

**92 open rows total — recounted row-by-row 08-06, the previous 87 was drift.** Every column above
was re-derived from the tables below, not carried forward: Tech Debt was three rows and a whole
**High** short (MONTAGE-ESC-1 is High and the glance table read `0`), and Engineering/UI-UX each
carried a closed card in their High column. Four ✅-stub rows were deleted the same pass
(**NET-LOOK-ACC-1** · **LOAD-SCALE-1** · **TOUCH-HOVER-1** · **UI-SCALE-1**, whose row still said
*"Pass 2 awaiting Wyatt playtest"* after the Work order had already recorded it PASSed) — closures
live in [completed-work.md](./completed-work.md), not as placeholder rows here. Everything in
**Playtest owed** already shipped — those rows are just waiting on Wyatt's eyes, not on more
engineering. That's usually the fastest place to look for "what do I personally need to go do."
...
**F1 — desk-only (no playtest, verdict is in the diff or the test run). Do these in one sitting.**
1. **MONTAGE-ESC-1** *(Tech Debt, **High** — the only High in this batch)*. One line. It is here
   and not in a High wave because it is a `tools/` card: it needs the tooling window Block G opens,
   and it must go first because `npm run states` / `sheet` / `podium` / `loadshots` **all throw**
   until it lands, and STATES-DEAD-1 below is judged with `npm run states`.
2. **RAPIER-DEFAULT-MAX-1** — load-bearing false prose (it is the stated reason Classic's walls
   take no restitution rule), so anyone who opens a physics card before it is fixed reasons from a
   wrong model of what Rapier does. **Prose only — every collider value stays byte-identical.**
3. **STATES-DEAD-1 + KBM-TOAST-1 as one card.** They are the **same CSS**: KBM-TOAST-1 (filed
   08-06) is `.cr-kbm-toast*`, which is STATES-DEAD-1's subject **(2)** `.cr-kbm-toast-close`, and
   its neighbour `.cr-touch-btn` is subject **(1)**. Both re-verified 08-06 — zero `kbm` or
   `cr-touch-btn` hits anywhere in `src/` or `index.html`. **Do not run them as two cards**; they
   would edit the same lines of `cart-rave-menu.css` and the second would land on a stale premise.
4. **SPINDLE-COLOR-DEAD-1** — dead colour pair in Classic; delete-after-checking, no player effect.
5. **RESULTS-GLOW-1** — note-for-later; may close as "documented, no change" in one commit.
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | DIFF-FRIENDS-1 — NPC difficulty applies in Friends rooms but cannot be set there | **Filed 08-06 from Wyatt, who remembered picking difficulty in Friends — and the code agrees with him, but the UI does not.** [cart-rave-menu.js:492-494](../../src/cart-rave-menu.js:492) documents the controller as driving *"the menu context panel's row **and the Friends invite screen's host row**"*, and `allDiffButtons()` queries `.cr-diff-row .cr-diff-btn` document-wide as if two rows exist. **Only one does** — `#cr-diff-row` in the menu context panel ([index.html:736](../../index.html:736)) — and it only renders when `MENU_ITEMS[cmd].diff === true`, which is **`solo` alone** (`quickplay` and `friends` are both `diff: false`, [:1942-1946](../../src/cart-rave-menu.js:1942)). Git says the comment arrived with `de6797f` (fight-night part 4b, Friends invite chrome), which also touched `cr-diff-row` in index.html — so the row existed at some point and was lost, most likely in the Fight Night redesign. **Why it is not cosmetic:** [party/index.ts:236](../../party/index.ts:236) seeds **all four slots as `kind: "npc"`** in *every* room and humans claim seats from there, so any Friends room that is not full has NPC carts in it. Those NPCs run at `settingsStore.aiDifficulty` — whatever the host last chose **in Solo** — so the setting is live in Friends and unreachable from Friends. A host who has never opened Solo gets `DEFAULT_SOLO` silently. **Scope:** decide first whether Friends *should* expose it (host-only, since it is a room-wide setting and only the host's value can win) — if yes, restore the row on the invite screen and wire it through the existing `selectDifficulty` controller, which already handles multiple rows; if no, delete the false half of the comment so the next reader is not hunting a row that does not exist. Either way **fix the comment in the same commit.** Related: the `.cr-context .cr-diff-btn` reachability FAIL in `npm run states` is a *different* pre-existing issue (the row is Solo-only and the sweep never selects Solo) — do not conflate them. **Does not block ONBOARD-SLIDES-1:** AISLE 8's copy deliberately never says where difficulty is chosen, so it stays true either way. |
...
| Medium | ORIENT-TOAST-Z-1 — phone orientation toast on main menu draws under other UI | **Filed 08-06 from Wyatt's UI-P2-HUD-PT-1 playtest note:** *"the phone orientation toast on the main menu gets drawn under stuff."* Same shape as the closed HUD-TOAST-Z-1 family (six toast-occlusion cards, PASS 08-05) — likely a stacking-context/z-index gap that family didn't cover because this toast lives on the main menu, not in-match. Check the toast's z-index against whatever main-menu layer is covering it before assuming it needs the same fix as the TOAST-* cards. |
...
| Pri | ID | Item | Notes |
|-----|----|------|-------|
| High | MONTAGE-ESC-1 | `tools/lib/montage.mjs` throws `ReferenceError: esc is not defined` — every contact sheet fails to write | **Found 08-06 while running `npm run states` for ONBOARD-SLIDES-1; `tools/` was frozen so it was filed, not fixed.** [montage.mjs:22](../../tools/lib/montage.mjs) is `export { esc } from "./ccStyle.mjs"` — a **re-export, which creates no local binding**, so the two `esc()` calls inside `montagePage` itself throw. Introduced by **CC-ESC-1 (`894b448`, 08-06)**, which replaced a local copy of the escaper with the re-export; the *importers* were fine (a re-export is valid for consumers), which is why it read as correct. **Blast radius: `states` · `sheet` · `podium` · `loadshots` — all four call `montagePage` and all four now crash after their checks complete.** The checks themselves still run and report, so this reads as a passing run with a stack trace at the end; `npm run states` exits non-zero regardless of the sweep result. **Fix is one line:** `import { esc } from "./ccStyle.mjs"; export { esc };` — keep the single-source intent CC-ESC-1 was after, just give the module a local binding too. Worth a regression test that actually *calls* `montagePage`, since nothing currently does in unit tests. |
| Medium | SHOOT-SOFTGL-1 | `npm run shoot` bakes the software-mode modal into every capture, and renders on SwiftShader | **Found 08-06 alongside MONTAGE-ESC-1, same freeze.** Two gaps, one card because the fix is one place. (1) [shoot.mjs:201-213](../../tools/shoot.mjs) dismisses onboarding and hides `#cr-howto-screen`, but never removes **`#cr-softgl-notice`** — so headless captures carry a full-screen "GRAPHICS RUNNING IN SOFTWARE MODE" modal across the middle of the frame. `states.mjs:480`, `loadshots.mjs:629`, `sheet.mjs:203` and `podium.mjs:350` all remove it; `shoot` is the only capture tool that does not. (2) `shoot` passes **no GPU flags**, so Chromium falls back to SwiftShader and the colour is wrong, not merely blurry — Cart Rave's record floor renders **olive green** instead of the dark neon read. [shoot-gpu.mjs](../../tools/shoot-gpu.mjs) already documents both as "visual-qa.md trap #1" and carries the fix (`--enable-gpu --ignore-gpu-blocklist --use-gl=angle` + an `UNMASKED_RENDERER` readout), but it is scoped in its own header as a **one-off for ART-PASS-CLASSIC-1**. **Scope if picked up:** fold shoot-gpu's launch args and vendor readout into `shoot.mjs` behind a `--gpu` flag and add the softgl removal unconditionally; do **not** silently change the default launch, since every committed `shots/` baseline was captured under SwiftShader and MAE comparisons against them would move. |
...
| Low | KBM-TOAST-1 | `.cr-kbm-toast` / `-toast-text` / `-toast-close` (cart-rave-menu.css:130-206) has no JS reference anywhere in `src/` | **Filed 08-06** — found while auditing `text-transform: uppercase` usage (unrelated question). No `document.createElement`/`classList`/template literal anywhere in `src/` sets `cr-kbm-toast` or its children, and `Select-String -Path src -Pattern kbm -Recurse` only hits this one CSS file — looks like orphaned CSS from a removed keyboard/mouse-switch toast. Confirm with a full-text search before deleting (dynamic class construction elsewhere could still reference it), then drop the three rules and their `@media` overrides. Not a ship blocker — dead weight, not a bug. |
