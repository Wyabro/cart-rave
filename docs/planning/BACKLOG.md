...
| ✅ | NET-MIG-3 — freeze vs WebRTC re-handshake | **PASS 2026-07-20:** freeze until first snap or 2s max; residual ghost guard keeps `awaitingFirstSnap` past freeze max + remotes collider-off until first post-epoch snap. Wyatt local clean-close PASS. Ship to prod if not yet deployed. |
...
| ✅ | **HOST-ROLE-1 — weak host poisons every peer** | **PASS** lever 3 `80ecbf6` / `index-DWDp_cX_.js` — lobby migrates host toward higher machine capability score (margin 20). Not a network score. |
...
| Medium | Controller menu navigation polish *(pre-ship 07-19)* | Main menu + customize: focus states, selection feedback, navigation consistency. ~~Known concrete bug (RC hunt, reported-not-fixed): `ui/gamepadNav.js getFocusables` is document-wide — a pad can activate buttons behind an open overlay (incl. PLAY) + per-frame focus re-yank while a pad is connected.~~ **Both fixed 07-20 (unpushed)** — focusables + B-button scoped to topmost open overlay, focus reclaimed on press only; `tests/gamepadNav.test.js` pins it. Needs pad-in-hand validation; remaining row scope = polish (focus states, selection feedback, consistency). |
