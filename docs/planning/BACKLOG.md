...
dev`, or ship first. *(The two NAME-VARIETY rows currently carry that mark.)*
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | NAME-NPC-VARIETY-PT-1 — expanded NPC names fit the roster `[solo]` | ***Pushed but not yet deployed.*** **Owed: Wyatt playtest — NAME-NPC-VARIETY-PT-1 — NPC names feel varied and belong in Cart Clash.** Parent **NAME-VARIETY-1**.<br>1. After ship, hard-refresh production and start Solo three times, reloading between rounds.<br>2. Read the three NPC names in each roster and during play.<br>3. FAIL if a roster repeats a name, a name is hard to read, or a name feels grim or outside the retail-cartoon tone. Random repeats between separate page loads are allowed. |
| Low | NAME-PLAYER-VARIETY-PT-1 — player defaults and rerolls feel varied `[solo]` | ***Pushed but not yet deployed.*** **Owed: Wyatt playtest — NAME-PLAYER-VARIETY-PT-1 — first-run and rerolled player names stay readable, playful, and less repetitive.** Parent **NAME-VARIETY-1**.<br>1. After ship, hard-refresh production and click the name reroll button 20 times.<br>2. Scan for repeated shapes, awkward combinations, overflow, or names outside the retail-brawl tone.<br>3. Open one private window to sample a fresh default name. FAIL if the name is unreadable, grim, or collides with an NPC name. |
