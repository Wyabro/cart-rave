...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 in progress | **SPAWN-SUNDIAL-GAP-1** |
| **2** — PRE-SHIP (before public post) | 🟡 queued | Definition: **CART-COLOR-DEPTH-1**. Then ONBOARD-ART-1 · MENU-CART-1 · LOAD-TIPS-1 · VOICE-BUS-1 · SD-MUSIC-LPF-1 · RECORD-MED-1 · CHUNK-MEMBER-1 · NPC-BOOST-1 (measure) · UI-FRAME-1 + ESC-PANEL-1 · PAD-MENU-1 · KILLFEED-PHONE-1 · ARENA-BUMPER-HINT-1 (after product call) |
...
1. **SPAWN-SUNDIAL-GAP-1** — carts wedge between Sundial booths and platform after solid legs landed. Likely one `booth.gapDistanceByLevel` number + feel check.
...
5. **VOICE-BUS-1** — announcer volume of its own (**AUDIO-MASTER-1 closed 08-07**: the dead `_masterVol` was deleted — a voice slider is genuinely a new third bus).
...
6. **UI-SCALE-P2-MEDIA-1** — Pass 2 media-query cleanup; re-check phone per file.
7. **ORIENT-HINT-SCROLL-1** — landscape stacked column + portrait-hint clip.
8. Opportunistic Low neighbors when idle: **MOTION-A11Y-1** (needs definition of done first) · **CARGO-LATCH-1** · **COUNTDOWN-QUICKPLAY-1** · **countdown survives menu return**.
...
| Medium | PATTERNS-UI-1 — Pattern customize UI `[SHIP-1 C3]` | Unblocked by CART-MODEL-1's re-UV. |
| Low | SHADES-MAT-1 — Sunglasses finish materials broken `[SHIP-1 E2]` | |
...
| Low | SHADES-ZOOM-1 — Animate the customize sunglasses-tab camera zoom | |
...
| Medium | PAD-MENU-1 — Controller menu navigation polish *(pre-ship 07-19)* | Modal-scoping shipped 07-20; remaining = polish + pad-in-hand validation. |
| Medium | UI-FRAME-1 — premium frame/panel styling pass `[SHIP-1 E1]` | |
| Medium | ESC-PANEL-1 — ESC scoring panel refresh `[SHIP-1 E1]` | |
| Low | MENU-SFX-1 — Main-menu SFX slider `[SHIP-1 E3]` | |
