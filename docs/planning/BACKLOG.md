...
| Pri | Item | Notes |
|-----|------|-------|
| High | NET-LAG-1 — Friends/QP lag + rubber-band before playtest | **Wave 1 NET-LAG-1-PT-1 PASS 09-14** (trail gone). **Wave 2 `8fa504ac`:** keep 60 input frames; replay at most 12 without deleting newer inputs. **NET-LAG-PT-2 FAIL 09-24:** caps 400/402 showed 11/10 hard corrections up to 19.9/22.1 m, with low RTT and zero history drops. **Diagnostic trace `820176d2`:** Wyatt's caps 404/405 completed NET-LAG-DIAG-PT-1. Non-host replay changed 0.22 m pre-snap error into 13.56 m after six steps, near cart contact. **Wave 3 `de25d4d7`, Worker `c5123875`:** suppress replay-created custom ram impulse; reset reused replay flag before live contact. 2,481 tests, local and prod Friends 14/14; live entry `index-C20rnYth.js`, 28 refs, 0×404, replay symbol. **NET-LAG-PT-2 human retest still owed.** Do not claim visual PASS from harness. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NET-LAG-PT-2 — non-host stays visually stable `[2pc]` | **Owed: Wyatt playtest — NET-LAG-PT-2 — human retest on prod `de25d4d7`, Worker `c5123875` (entry `index-C20rnYth.js`).** Previous FAIL 09-24 on caps 400/402. Caps 404/405 diagnosed replay-created divergence; the new fix has automated proof only.<br>1. Hard-refresh https://www.cartclash.lol/?diag=1&nettest=1 on the same two machines. Play Friends for two minutes.<br>2. Drive, turn, boost, ram, and recover from collisions while the host also moves.<br>3. FAIL if the non-host repeatedly snaps or loses current steering. PASS only if the path stays continuous and input response is playable.<br>4. Press F8 on both machines at round end. |
