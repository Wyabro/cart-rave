...
| Low | COMFORT-PT-1 — softer hit/GO feedback, reduced-motion kills FOV punch `[1pc]` | **Owed: Wyatt playtest — COMFORT-PT-1 — camera comfort pass: hits read gentler but still land, and reduced-motion now silences the FOV punch as well as shake (CAM-COMFORT-1).** Pushed but **not yet deployed** — use `npm run dev:local`. FOV punches halved (ram 8→4°, KO 9→5°, GO 10→5°), shake scale 6.2→4px.<br>1. With OS reduced-motion OFF, play a solo round. Take normal rams, a boost ram, and score a KO; note the GO! moment.<br>2. FAIL if hits feel like nothing happened or juice reads dead. PASS if punch/shake are visibly gentler than before but still communicate impact.<br>3. Turn ON reduced-motion (Windows Settings → Accessibility → Visual effects), reload, replay a round. FAIL if any FOV zoom-punch or screen shake fires on hits/KO/GO. PASS if both stay off while flash, hitmarker, and vignette cues still show.<br>4. Press F8 at round end. |
