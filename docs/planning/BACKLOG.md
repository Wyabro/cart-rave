...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 playtest | **RAPIER-MAJOR-PT-2** on prod after hard-refresh |
| **2** — PRE-SHIP (before public post) | ✅ clear | NPC-BOOST cluster closed 08-11 |
| **3** — WYATT LANE (blocked on you) | 👤 ongoing | **RAPIER-MAJOR-PT-2** · ANNOUNCER-RERECORD-1 · SHIP-1 D-tier cut/keep |
...
| 🟢 [Playtest owed](#playtest-owed) | 10 | 0 | 7 | 3 |
...
| Medium | RAPIER-MAJOR-1 — `@dimforge/rapier3d` 0.19 → 0.20 | **Deployed `524bd4db`.** Both packages at `0.20.0` (Rust 0.35). No knob retune. SIMD stays opt-in. **RAPIER-MAJOR-PT-1** PASS on `dev:local`. Playtest owed: **RAPIER-MAJOR-PT-2** (live MP on prod after hard-refresh). Do not close until PT-2 PASS. |
...
**08-13, 1/1 PASS, removed:** **RAPIER-MAJOR-PT-1** — Wyatt PASS on `npm run dev:local` (drive, hop, KO, pit). Parent **RAPIER-MAJOR-1** stays open for **RAPIER-MAJOR-PT-2**. Deployed `524bd4db`. Detail: [completed-work.md](./completed-work.md).
...
| Medium | RAPIER-MAJOR-PT-2 — live host+joiner drive and KO `[2pc]` | **Owed: Wyatt playtest — RAPIER-MAJOR-PT-2 — joiner drive and a KO on the joiner still match the host.** Two browsers, Friends room. Prod after hard-refresh. Deployed `524bd4db`. Parent RAPIER-MAJOR-1.<br>1. Host and joiner both drive. FAIL if the joiner rubber-bands or cannot steer.<br>2. Host KOs the joiner. FAIL if the joiner does not shatter or respawn. PASS if both screens agree. |
...
PA-COMBO-1, PA-COMBO-PT-1, STORE-1-PT-1, STORE-MUSIC-PT-1, RAPIER-MAJOR-PT-1.
