...
more engineering. Open owed checks: **PAUSE-SLIDER-DELAY-PT-1** · **SHARD-PT-2** (launch day).
Closed PASS history lives only in [completed-work.md](./completed-work.md).
...
Landed 09-01 — **PAUSE-SLIDER-DELAY-1** (pause AUDIO sliders on first panel paint); owed **PAUSE-SLIDER-DELAY-PT-1**.
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | PAUSE-SLIDER-DELAY-PT-1 — pause AUDIO sliders are there as soon as the panel is `[solo]` | **Owed: Wyatt playtest — PAUSE-SLIDER-DELAY-PT-1 — pause AUDIO sliders are there as soon as the panel is there.** Parent **PAUSE-SLIDER-DELAY-1**. Local `npm run dev:local` is enough. The panel itself may still pop in (~300 ms) — do not FAIL that pop.<br>1. Solo match. Press Esc.<br>2. FAIL if MUSIC / SFX / VOICE sliders lag the panel (panel is there, sliders still missing).<br>3. PASS if those sliders are readable as soon as you can see the panel body.<br>4. Close and open Esc once more. FAIL if the second open blanks the sliders again. |
