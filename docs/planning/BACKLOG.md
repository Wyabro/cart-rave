...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 STORE-PILE-2 | Bounce must cancel inward velocity so NPCs leave the Storerooms pile |
...
| **5** — SWEEP (cheap Lows) | 🟡 queued | MOTION-A11Y-1 · COUNTDOWN-QUICKPLAY-1 · COUNTDOWN-LEAK-1 · CART-HUE-RED-1 · BOOST-SFX-NONHOST-1 · GAMEPAD-FRIENDS-SEATED-1 |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 12 | 0 | 5 | 6 (+1 partial) |
| [Art](#art) | 1 | 0 | 0 | 1 |
| [Audio](#audio) | 2 | 0 | 0 | 2 |
...
engineering. Live owed checks (08-14): **DEEPSEC-1-PT-1** · **CARGO-BAY-INSTANCE-PT-3** ·
**CONN-TRACK-LEAK-PT-1** · **QP-ROTATE-PT-1** (two machines) · **SHARD-PT-2** (launch day) ·
**STORE-PILE-PT-1** (FAIL — bounce loses to drive). Closed PASS history lives only in
[completed-work.md](./completed-work.md).
...
1. **STORE-PILE-2** — bounce must cancel inward drive so NPCs leave the Storerooms pile. Retest **STORE-PILE-PT-1**.
...
4. **GAMEPAD-FRIENDS-SEATED-1** — controller nav after seating in a Friends lobby (filed 08-14 from GAMEPAD-DIRECT-ENTRY-PT-1 PASS note).
...
| Medium | STORE-PILE-2 — bounce must cancel inward velocity on the Storerooms pile | **Filed 08-14 from STORE-PILE-PT-1 FAIL.** Parent STORE-PILE-1 (`0fd9c64`) added tangent steer + a 17 m/s² outward accel. Drive is 125 m/s² (205 boosted), applied the same frame before the bounce, so net force stays into the hull and NPCs grind. One lever: `applyWallKeepOutBounce` must cancel the inward linvel component and add restitution, not only add a weaker accel. Sundial podium stays off (`wall` flag). Retest **STORE-PILE-PT-1**. |
| Low | GAMEPAD-FRIENDS-SEATED-1 — controller nav glitchy after seating in Friends lobby | **Filed 08-14 from GAMEPAD-DIRECT-ENTRY-PT-1 PASS note.** Name + room-code dialogs PASSed. After the Friends room loads / seats, the controller is glitchy and hard to use. Do not reopen GAMEPAD-LOBBY-1. New evidence only — reproduce on the seated Friends lobby before a lever. |
...
| Medium | STORE-PILE-PT-1 — NPCs route around + bounce off the Storerooms furniture pile `[solo]` | **FAIL 08-14 — Owed: Wyatt playtest — STORE-PILE-PT-1 — NPC carts no longer routinely wedge against the center furniture pile in Storerooms.** Parent **STORE-PILE-1** (`0fd9c64`) bounce (17 m/s²) loses to drive (125 m/s²). Next lever is **STORE-PILE-2**. Wyatt: they still drive into the pile and do not bounce off well.<br>1. Solo round on THE STOREROOMS (hard-refresh prod first). Play near the center furniture pile — park near it, then let the NPCs chase you across it.<br>2. Over a full round, watch the NPCs: chasing bots should curve around the pile from ~8 m out; a bot that does contact it (boost push, ram) should bounce back and keep moving.<br>3. FAIL if an NPC grinds the same face of the pile for 4+ seconds, or repeatedly drives straight into it head-on instead of steering around. PASS if contacts are brief and bots visibly route around. |
