...
| Medium | PATTERNS-UI-3-PT-1 — readable seamless cart patterns `[solo]` | **Pushed but not yet deployed — use `npm run dev`, or ship only on `ship it`.** **Owed: Wyatt playtest — PATTERNS-UI-3-PT-1 — Maze reads as a maze, the multicolor tiles cover cleanly, and Cubes reads as an isometric lattice.** Parent **PATTERNS-UI-3**.<br>1. In local dev, open CUSTOMIZE → PATTERN and select Pink.<br>2. Select Maze. PASS if roughly four large orthogonal turns span the cart and it no longer reads as diagonal noise.<br>3. Select Honeycomb, Diamond Weave, and Isometric Cubes. Rotate through front, side, and rear views. PASS if repeated edges connect cleanly and Cubes shows staggered hexes split into three cube faces. FAIL on orange scribble or rectangular loops.<br>4. Repeat steps 2–3 with Blue, then start a solo round. PASS if the pattern stays stable while moving and after a KO rebuild.<br>5. After `ship it`, hard-refresh production and repeat steps 1–4. |
