...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🚧 active | **FRIENDS-JOIN-LAYOUT-1** — implemented locally; ship + production visual PASS remain |
| **2** — PRE-SHIP (before public post) | 🟡 queued | NPC-BOOST-1 (deployed, needs [2pc]) |
...
| [Art](#art) | 10 | 1 | 2 | 7 |
| [Audio](#audio) | 3 | 0 | 2 | 1 |
| [Design / Gameplay](#design--gameplay) | 9 | 0 | 4 | 5 |
...
1. **NPC-BOOST-1** — deployed; needs Wyatt [2pc] playtest.
2. **AI-EASY-SOFTEN-1** — deployed with NPC-BOOST-1; needs the same playtest.
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | NPC-BOOST-1 — NPCs use charged attacks and instant boosts by intent | **Deployed 08-10:** Worker `4e4fd25a-452b-40b6-adb8-c8e1c41d6bde`, commits `033bb8f` / `4e47e16`; entry HTML plus 62 hashed assets returned 200, and the live game bundle contains the NPC intent marker. Attack uses the existing full human charge when target, range, and path stay safe; chase, escape, and recovery use instant boost; patrol never boosts. Unsafe charges hard-cancel with no burst. Easy / Medium / Hard select charge at 9–12m/12°, 8–12m/16°, and 7–12m/20°; boost physics and cooldown stay shared. Human closure is tracked by **NPC-BOOST-PT-1**. |
| Medium | AI-EASY-SOFTEN-1 — Easy NPC decisions 10% slower | **Deployed with NPC-BOOST-1.** Easy decision intervals changed from 1.44× to 1.584× their personality baseline: exactly 10% slower than prior Easy. No physics, boost values, or Medium/Hard changes. Human closure is tracked by **NPC-BOOST-PT-1**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | NPC-BOOST-PT-1 — charged and instant NPC boosts `[2pc]` | **Owed: Wyatt production playtest** on Worker `4e4fd25a-452b-40b6-adb8-c8e1c41d6bde`.<br>1. Hard-refresh two browsers, create a Friends room with bots, and confirm both screens receive the same NPC boost state.<br>2. In a safe attack run, PASS if an NPC visibly charges then releases full boost; PASS if chase and escape use instant boost instead.<br>3. Force a target/path to become unsafe during charge. PASS only if the charge stops with no burst or nitro window.<br>4. Compare rounds: Easy must read less decisive than Medium; Hard must find more charge runs. FAIL on any boost desync, unsafe burst, or reversed difficulty order. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | FRIENDS-JOIN-LAYOUT-1 — room-code box below CART NAME | **IMPLEMENTED LOCALLY 08-09; awaiting explicit `ship it` + Wyatt production visual PASS.** The existing room-code input/GO/error controls moved out of the command list and directly below the cart-name plate in desktop and stacked mobile DOM order. Desktop uses one right-side grid rail: the name plate, join row, a flexible measured cart-preview slot, then context. At 1280×720 the preview slot falls below its existing 240×180 mount gate and releases the canvas instead of overlapping; at 1440×900 it stays visible with zero join/cart/context intersection. Local browser PASS: 1440×900, 1280×720, 390×844, 740×360, plus invalid-code error expansion. Focused tests 13/13. No netcode or join-flow behavior changed. |
...
MENU-SWAP-FLASH-1.
ANLX-GEO-1, ANLX-PAGEHOST-1, ANLX-GLITCH-1.
