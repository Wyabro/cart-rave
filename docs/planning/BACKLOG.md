...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Classic PASS · Sundial + Storerooms remain · **CART-POP-PT-1** open until those two · **NET-LAG-1-PT-1** parked |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Classic PASS 08-20** Wave I Worker `5360f482`. Do not retouch `src/levels/arena.js` floor. **Sundial:** `getZanzibarFloorColliderSpec` — 4 overlapping deck cuboids + podium caps (coplanar double solids). **Storerooms:** `backroomsSupermarket.js` multiple floor colliders. Same first-ejection: supported, `preVy≈0`, tilted manifold, ~24 m/s. Pattern: one floor surface, no overlap, no radial internals. No restitution/friction/CCD/solver tune. **CART-POP-PT-1** open until those two arenas PASS. Lane: **Critical**. |
