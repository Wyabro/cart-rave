...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 20 | 1 | 11 | 7 (+1 partial) |
| [Art](#art) | 16 | 2 | 4 | 10 |
| [Audio](#audio) | 5 | 0 | 3 | 2 |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | FRIENDS-LEVEL-1 — arena picked on the FRIENDS menu is not the arena the private lobby opens on | ⏳ **SHIPPED 08-07 (`f8281c5`), human check owed on FRIENDS-LEVEL-PT-1.** Filed 08-06 from Wyatt's DIFF-FRIENDS-PT-1 note, confirmed the same day as the DIFF-FRIENDS-1 hole one level down: `MSG.hello` adopted the room's default `levelId` (`classicRecord`) into `settingsStore.selectedLevelId` before the Friends host ever got a chance to push their own pick — clobbering it in both the latch and localStorage, so `sendHostRound` re-broadcast the default. Fix: the hello handler now skips that adopt for the Friends host (`isFriendsHostHello`, keyed off `msg.hostId === msg.youConnId` since the module vars aren't assigned yet) so the host's already-loaded arena is never rotated away and back; `adoptFriendsHostLevelFromStore()` stamps the room latch from the store instead, ordered *before* the existing `adoptFriendsHostAiDifficultyFromStore()` call so one `sendHostRound` carries both. Quickplay, guests, rematch rotation and host migration are unaffected — predicate false, or no hello involved. Server untouched (`MSG.hostRound` already latched `levelId` and `aiDifficulty` off one payload). `tests/friendsLevel.test.js`: store-wins-over-stale-latch regression, guest/quickplay no-ops (state-checked, not just return value), no-saved-pick fallback, predicate unit cases — 14/14 targeted, 1626/1626 full suite, qa 7/7. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | FRIENDS-LEVEL-PT-1 — the Friends lobby opens on the arena you picked, not the default `[solo]` | **Owed: Wyatt playtest — FRIENDS-LEVEL-PT-1 — the arena you had selected on the main menu is the arena your private room actually opens on.** Shipped 08-07 (`f8281c5`). The room used to always start on Cart Rave regardless of your pick — that's the bug this fixes. *(Quickplay staying random is covered by an automated regression test, not a step here — one issue per card.)*<br>1. Main menu: select **THE STOREROOMS** (or any arena besides Cart Rave).<br>2. Select **FRIENDS** and create a private room.<br>3. Look at the lobby (CHECKOUT LINE) — it should be **The Storerooms**, with no flash or arena swap as it loads.<br>4. If you have a second device, have a friend join your room link and confirm they also land on The Storerooms.<br>5. PASS if your Friends room opens on the arena you picked. FAIL with what arena you picked and what the lobby showed instead. |
