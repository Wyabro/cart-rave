...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **PAUSE-SLIDER-DELAY-1** · **MENU-SHORTWIN-1** · **CG-ZIP-1** · **CG-COVERS-1** |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 15 | 4 | 4 | 6 (+1 partial) |
...
1. **PAUSE-SLIDER-DELAY-1** — pause overlay empty ~1 s before sliders show.
2. **MENU-SHORTWIN-1** — menu readable at CrazyGames iframe **1077 × 606**.
3. **CG-ZIP-1** — `dist/` zip, `index.html` at root, ≤ 50 MB, no SDK, desktop-only listing.
4. **CG-COVERS-1** — three covers + two silent videos (Wyatt art; required to upload).
5. **NET-LAG-1** — Friends/QP lag + rubber-band (F8 both machines; measure first).
