...
| Medium | PATTERNS-UI-2-PT-1 — nine cart patterns and unlocks `[solo]` | **Deployed Worker `50e663ec` — hard-refresh production first.** **Owed: Wyatt playtest — PATTERNS-UI-2-PT-1 — free basics, earned detail, distinct multicolor pattern art, and NPC variety all read cleanly.** Parent **PATTERNS-UI-2**.<br>1. Hard-refresh production and open CUSTOMIZE → PATTERN. PASS if the nine chips fit 3×3; Classic, Stripes, and Checker select; the other six show their listed locks with `?devUnlocks=off`.<br>2. Select Maze, Honeycomb, Diamond Weave, and Isometric Cubes; cycle Pink and Blue. PASS if chip, preview, and live cart agree. FAIL if the linework shimmers, merges, or loses player colour.<br>3. Start a solo round. PASS if the selected pattern survives spawn and KO rebuild, and NPCs show varied deterministic patterns.<br>4. With a controller, navigate all nine chips and select one. PASS if focus and selection work; touch pattern selection stays unchanged.<br>5. Repeat steps 1–4 after any cache-clearing deploy. |
