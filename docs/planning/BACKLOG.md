...
| Pri | Item | Notes |
|-----|------|-------|
| High | SITE-TRUST-1 — public information and analytics choice | **Authorized 09-13. Critical: static routing.** Publisher Redshift Studios; support redshiftstudiossupport@gmail.com; analytics on by default per Wyatt. Settings links to privacy choice, terms, and email. All optional provider loaders respect saved off. Public pages load no game or analytics. Preserve real 404 status and API/multiplayer routes. PNG uses a reduced palette (1,137,264 to 300,645 bytes). Local implementation; ship authorization pending. Checks: default/saved/URL choice, blocked storage, route tests, build, QA, desktop and narrow-page inspection. No claim of legal compliance or human PASS. |
