...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave F deployed; contact capture due · **CART-POP-PT-1** FAIL / remains open · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 8 playtest blockers |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave F diagnostic deployed 08-20.** Wave D cap-388 shows wedge seams are not required: four single-floor supports have the highest mean rise (+5.42), peak `−8.64 → +2.41`. Wave C rules out control code and restitution. Wave E: tilt/spin can amplify solver rises (`+15.67`, `+16.06`), but upright/no-pre-spin still reaches `+11.01`; it is not sufficient as root cause. Wave F records real contact points, subshapes, separation, and solver impulses. **CART-POP-PT-1 remains FAIL/open.** Reproduce ordinary Cart Rave driving and F8 before a physics change. Lane: **Critical**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | SPAWN-BACKROOMS-PT-2 — Storerooms decks sit near the pit edge `[1pc]` | **Owed: Wyatt playtest — SPAWN-BACKROOMS-PT-2 — Storerooms spawn decks sit near the pit edge, two booth-widths out from the inward try.** Parent **SPAWN-BACKROOMS-2**. Direction corrected 08-20 (ring 38.15). Use `npm run dev` until this commit ships.<br>1. Open The Storerooms. Start a round. Do not drive yet.<br>2. FAIL if the decks still sit inboard with a wide band of carpet behind them, or if a cart hangs in the air / off its deck.<br>3. PASS if all four carts sit on their decks near the pit wall, facing center on the cardinals, with the extra floor now toward the middle of the room.<br>4. Drive off a deck and fall. FAIL if you respawn in the air or on the floor instead of on that same outer deck.<br>5. Fail the card if Classic Record or Sundial Station spawn positions moved. |
