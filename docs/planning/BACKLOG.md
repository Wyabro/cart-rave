...
| Medium | ANLX-BULK-1 — older ~2 s `loss` bulk, second pollution source | Split out of ANLX-ATTRACT-1, which only ever owned the recent draw cluster. **The ring was reset 07-31, so these are the last readable aggregates** (`/api/analytics?view=list` caps at the newest 1000 rows, and ~19,000 of the ring's 20,000 predated 07-20T19:23Z — that bulk was reachable in aggregate only). Pre-reset summary: `match_started=9719` / `match_ended=9545` from **265 sessions but only 8 clients**; `resultSplit` `loss=9327 / draw=209 / win=9`; `matchesByMode` `quickplay=9523 / solo=22`; `matchesByArena` `classicRecord n=6792 avgMs=1829 avgKos=7.07`, `zanzibar n=2636 avgMs=2316 avgKos=3.68`, `backrooms n=117 avgMs=67277`. Signature (`loss`, ~2 s, `classicRecord`, quickplay) is distinct from the phantom draw cluster (`draw`, `<3 s`, zanzibar/backrooms) — reads like a rig or a script booting and ending rounds in bulk rather than a player path; `rewindRoundClock`-style fast-ends produce exactly this shape. **Next step is free:** the post-reset ring is clean, so just re-pull `npm run analytics:pull` after normal play — if ~2 s `loss` rows reappear, the source is still live and the emitting client/session is now isolable (8 clients became 1–2). Do not re-derive from the numbers above; they are a snapshot, not evidence of an active emitter. |
...
| Medium | SHEET-1 — in-match contact-sheet tool | **Plan ACK'D 07-30 — full detail in [sheet-1.md](./sheet-1.md); do not re-plan.** `tools/sheet.mjs` boots via `makeClient({room:"solo", diag:1, perfPump:1, harness:1})` + `waitForState(phase==="running")`, fresh page per cell, `{w,h}` viewport tuples (small default + `--all`). **Blocked on ANLX-ATTRACT-1 acceptance then the DO reset** — it boots solo rounds, which would muddy that acceptance signal. **Blackframes pre-check DONE (07-30): false-black DISPROVEN** (0.5517 headless / 0.2042 live; a cleared buffer reads ~1.0) — do not re-run it. Scope narrowed at ack: in-match HUD responsive sweep + reduced-motion stills only; loading screens, hover/press and menu Pass-1 identity are NOT covered. |
