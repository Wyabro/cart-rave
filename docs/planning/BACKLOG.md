...
| High | MENU-CART-FOLLOW-1 — menu cart preview follows the cursor slightly | **Implemented locally 08-20 (uncommitted).** Desktop Medium/High main-menu preview maps the full-menu mouse position to at most ±5° yaw / ±2° lean, follows in ~180 ms, and returns in ~350 ms. One presentation-pose compositor blends cursor life out for the fake ram and back in on recovery; reduced motion, touch, mobile, Low, suspend, blur, and release reset it. Customize stays untouched. Focused tests 11/11 plus typecheck/build pass. **Owed: MENU-CART-FOLLOW-PT-1.** Do not reopen **MENU-CART-1**. |
