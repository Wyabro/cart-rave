...
1. **UI-SCALE-1 — Pass 1 ✅ shipped + PASSed 08-05; Pass 2 is what's left.** Pass 1 (root scale +
   `cart-rave-menu.css`, `9e2ec60`..`f057abe`) is done and playtest-verified 3/3. Remaining: **Pass
   2** — the same clamp→rem conversion on `hud.css`, `results.css`, `pauseOverlay.css`,
   `announcer.css`, `stickers.css`, `loadingScreen.css` (only `hud.css`/`results.css` have had
   opportunistic phone patches so far, not the systematic sweep). Folds LOAD-SCALE-1 (`loadingScreen.css`
   untouched); TOUCH-HOVER-1 rides the same pass (menu's hover gap fixed in Pass 1 via L5, the rest
   is not). **First in B on purpose:** it changes the unit system every other UI card is authored in,
...
| High | UI-SCALE-1 — responsive root-scale migration — **Pass 2 remains** | Two passes per [responsive-scale-migration.md](./responsive-scale-migration.md): fluid root + rem clamps; media queries stay px. **Pass 1 shipped + PASSed 08-05** (`9e2ec60`..`f057abe`) — `tokens.css` root scale, `cart-rave-menu.css` fully converted, ≤768 reflow answered "phone = fewer elements" (tagline/cmd-qual/stats/context-desc hidden), menu hover gated behind `(hover: hover)`. Playtest evidence: UI-SCALE-RESULTS-PHONE-1 / UI-SCALE-RESULTS-WIDE-1 / UI-SCALE-FEED-PHONE-1 all PASS 08-05 (those three also got opportunistic phone patches to `results.css`/`hud.css`, not the full sweep). **Pass 2 (open):** the same clamp→rem conversion on `hud.css`, `results.css`, `pauseOverlay.css`, `announcer.css`, `stickers.css`, `loadingScreen.css`. SHEET-1 unblocked (tool exists). Sequential single-owner. Safety: ≥1920 pixel-identical pre/post. |
