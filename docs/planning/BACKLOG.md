...
| **5** — SWEEP (cheap Lows) | 🟡 queued | CAPTURE-RING-LIMIT-1 · BOOTH-RAIL-COL-1 · SUNDIAL-LOW-WATER-1 · UI-SCALE-P2-MEDIA-1 · ORIENT-HINT-SCROLL-1 |
...
1. **NPC-BOOST-2** — resume PT-1 [2pc]; code already on prod. Prior NPC-BOOST-PT-1 **FAIL**.
...
4. **UI-SCALE-P2-MEDIA-1** — Pass 2 media-query cleanup; re-check phone per file.
5. **ORIENT-HINT-SCROLL-1** — landscape stacked column + portrait-hint clip.
6. Opportunistic Low neighbors when idle: **MOTION-A11Y-1** (needs definition of done first) · **CARGO-LATCH-1** (fix deployed — playtest owed) · **COUNTDOWN-QUICKPLAY-1** · **countdown survives menu return**.
...
| Low | CARGO-LATCH-1 — `cargoLoad.js` carries the same round-anchor latch as FIX-DIRPAUSE | **Filed 08-04** while fixing FIX-DIRPAUSE (`e7dd92e`) — same bug class: a latch that compares against the stored round-start anchor and treats **any** change as a new round, so a pause-compensation shift can re-fire the overflow announce. **Fix landed 08-09 `a20d547`** (did not cite the card — caught 08-10): `shiftCargoLatchBy(deltaMs)` added to `src/cargoLoad.js` and called beside every `shiftDirectiveTimersBy(delta)` in `roundLifecycle.js` (solo pause) and `gameBoot.js` (host tab-return), with coverage in `tests/cargoLoad.test.js`. **Deployed `7569051`.** **Owed: production playtest** — solo pause AND host tab-return at boss fill, confirm no re-fired `cart_overflow` (see **CARGO-LATCH-PT-1**). |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NPC-BOOST-2 — restore visible NPC charged attacks and stop boost self-KOs | **NOW — PT owed.** Unparked 08-10 after DEV-LOOP-1 PASS. Repair commits include visible charge + all-players charge audio (`3d1e263` / `544f4b5`); earlier CF ship carried them. Close only after **NPC-BOOST-2-PT-1 [2pc]** PASS. Prior NPC-BOOST-PT-1 **FAIL**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NPC-BOOST-2-PT-1 — charged and instant NPC boosts `[2pc]` | **Owed: Wyatt playtest — NPC-BOOST-2-PT-1 — charged NPC boosts are visible and audible on Solo and Friends, without boost self-KOs.** DEPLOYED `7569051` · Worker `8e5f274f` · prefer **https://www.cartclash.lol/?diag=1**. Prior NPC-BOOST-PT-1 **FAIL**.<br>1. Hard-refresh Solo with three NPCs. PASS if each difficulty visibly produces charge-ups in safe attacks, Easy least / Hard most, host hears each charge loop.<br>2. Friends room: both clients see telegraph, hear charge loop, see charged burst.<br>3. Classic rim + hole; octagon + square-hole: unsafe charge cancels / unsafe instant skipped.<br>4. FAIL on NPC boost self-KO, missing charge sound, desync, or reversed difficulty order. |
| Medium | CARGO-LATCH-PT-1 — no re-fired `cart_overflow` after pause or tab return `[solo]` | **Owed: Wyatt playtest — CARGO-LATCH-PT-1 — the "cart overflow" callout does not repeat after a solo pause or a host tab return.** Fix `a20d547` · deployed `7569051` · prefer **https://www.cartclash.lol/?diag=1**.<br>1. Solo: get a cart to boss fill so the overflow callout announces, pause (Esc), resume — the callout must not fire again on its own.<br>2. Host: same fill, hide the tab ~10 s, return — no repeat callout.<br>3. FAIL on a repeat overflow callout after either resume. |
...
MENU-SWAP-FLASH-1.
ANLX-GEO-1, ANLX-PAGEHOST-1, ANLX-GLITCH-1, DEV-LOOP-1, DEPLOY-MAP-1, MENU-SFX-1.
