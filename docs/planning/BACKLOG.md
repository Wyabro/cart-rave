...
| **2** — PRE-SHIP (before public post) | 🟡 queued | SD-SPECTATOR-CHARGE-1 (**COUNTDOWN-HOST-STAMP-1** landed, PT owed; **REMOTE-INPUT-STALE-1** landed, PT owed; **NPC-ABORT-BURST-1** landed, PT owed; **SD-SCORE-STALE-1** Wyatt PASS; **THOST-CEILING-1** Wyatt PASS; **GAMEPAD-FREEZE-1** · **ZOMBIE-HOST-PICK-1** Wyatt PASS) |
| **3** — WYATT LANE (blocked on you) | 👤 ongoing | SHIP-1 D-tier |
...
| 🟢 [Playtest owed](#playtest-owed) | 7 | 0 | 4 | 3 |
...
more engineering. Open owed checks: **SHARD-PT-2**
...
(see `## Playtest owed`).
Landed 08-17 — **REMOTE-INPUT-STALE-1** (host zeros stale remote input after apply-silence); playtest owed
(see `## Playtest owed`).
Landed 08-16 — **COUNTDOWN-HOST-STAMP-1** (skip host countdown stamp while host clock unlocked); playtest owed
(see `## Playtest owed`).
1. **SD-SPECTATOR-CHARGE-1** — SD spectator phantom charge loops SFX up to 45s.
...
3. **CART-HUE-RED-1**.
4. 08-16 audit Lows, top first: **MIG-KO-DROP-1** (KO feed row lost at migration) · **SPILL-DOUBLE-VFX-1** · **PODIUM-DOUBLE-CREDIT-1** · **KEYUP-STUCK-1** · **HOST-PRESENT-ORDER-1** · **ZOMBIE-ROOM-RESET-1** · **DEMOTE-COUNTDOWN-1** · **SPECTATOR-ANNOUNCER-1** · **RD-COUNTER-1**. **SPILL-RAM-CREDIT-1** landed, PT owed.
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | REMOTE-INPUT-STALE-1 — hidden non-host's last input keeps driving on the host sim | **Landed 08-17** — host drain stamps `lastAppliedMs` and zeros `{throttle,steer,nitro}` after `CONFIG.net.remoteInputStaleMs` (300) of apply-silence; latch kept so a held-boost return cannot double-charge from the drain edge. Playtest owed: **REMOTE-INPUT-STALE-PT-1**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | CART-HUE-RED-1 — custom hue red reads as dark orange in-game `[solo]` | **Lever A landed 08-17** — red-end snap `0xff0000` → `0xff2233` (ACES + 0.72 body tint dropped blue on all three arenas; mask was healthy). Playtest owed: **CART-HUE-RED-PT-1**. Art presentation only — cart material traverse stays frozen. |
...
| Low | SPILL-RAM-CREDIT-1 — does a solid knockback ram count as a "spill"? | **Landed 08-17** — SPILL credit is a real spill (tip-over / massive-ram / void fall) attributed to the recent rammer via `lastHitBy` + `hitWindowMs`. Rams on upright victims count for nothing. Playtest owed: **SPILL-RAM-CREDIT-PT-1**. |
...
| Medium | REMOTE-INPUT-STALE-PT-1 — hidden non-host cart coasts on the host `[2pc]` | **Owed: Wyatt playtest — REMOTE-INPUT-STALE-PT-1 — a hidden non-host stops driving on the host after a short silence.** Parent **REMOTE-INPUT-STALE-1**. You host. Non-host URL: `?diag=1&room=quickplay` — no `?perfPump` on the non-host.<br>1. 2pc Quickplay. Non-host holds throttle+steer in a loop, then hides the tab for 2–3 s.<br>2. FAIL if the host still sees that cart drive the loop after ~0.5 s. PASS if it stops steering and coasts in a straight line (it will not halt in 1 s).<br>3. Restore the tab. PASS if it is drivable at once, with no ghost drive. A snap equal to the coast distance is OK.<br>4. Hide mid-charge. PASS if the host sees at most one burst at staleness and no stuck charge SFX. Return still holding boost: FAIL if two bursts or a second drain-edge charge. PASS if at most one new charge bar.<br>5. Host drives through the same window. FAIL if a healthy focused peer hitch-zeros. |
...
| Low | SPILL-RAM-CREDIT-PT-1 — rams count once, at the spill `[2pc]` | **Owed: Wyatt playtest — SPILL-RAM-CREDIT-PT-1 — Spill Master ticks when groceries fly, not per ram.** Parent **SPILL-RAM-CREDIT-1**. You are not host. Hard-refresh. Pushed but not yet deployed — `npm run dev` until ship. No `?perfPump` on the non-host.<br>1. Two PCs. Friends or Quickplay. You ram the host cart until groceries fly. FAIL if Spill Master ticks per ram. PASS if it ticks +1 exactly on the spill.<br>2. Sustained rams on a loaded/tanky upright cart. FAIL if ticks arrive before groceries fly. PASS if nothing ticks until the spill.<br>3. Self-tip with no contact. FAIL if you get a tick. PASS if you get none.<br>4. Solo `npm run dev`. Ram an NPC until groceries fly. PASS if you get a tick on the spill and the receipt SPILLS CAUSED matches. |
| Low | CART-HUE-RED-PT-1 — custom hue red matches the menu swatch `[solo]` | **Owed: Wyatt playtest — CART-HUE-RED-PT-1 — the custom hue slider's far red end reads red in-game, not dark orange.** Parent **CART-HUE-RED-1**. Solo. Hard-refresh. `npm run dev` until ship. DEV unlocks all arenas.<br>1. Solo, Sundial Station. Customize → CUSTOM → hue slider to the far red end.<br>2. Enter the match. FAIL if the cart reads dark orange. PASS if it matches the menu swatch's red.<br>3. Sweep a few degrees either side of 0 — the 14/15° seam is expected (red → orange). FAIL if it reads orange → orange.<br>4. Repeat on The Storerooms and Cart Rave. FAIL if any arena reads dark orange. Pink preset unchanged. HUD "you" accent must stay legible (it will not match the cart). |
...
ZOMBIE-HOST-PICK-PT-1, NPC-ABORT-BURST-1, GAMEPAD-FREEZE-1, GAMEPAD-FREEZE-PT-1,
WARM-QP-ROTATE-PT-1, HOWLER-UPGRADE-1.
