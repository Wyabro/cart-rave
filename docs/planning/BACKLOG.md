...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | ⬜ no open cards | → Block 2 (**ONBOARD-ART-1**) |
| **2** — PRE-SHIP (before public post) | 🟡 queued | ONBOARD-ART-1 · VOICE-BUS-1 · CHUNK-DEFER-1 · NPC-BOOST-1 (measure) |
...
| **5** — SWEEP (cheap Lows) | 🟡 queued | CAPTURE-RING-LIMIT-1 · BOOTH-RAIL-COL-1 · SUNDIAL-LOW-WATER-1 · main-menu SFX · UI-SCALE-P2-MEDIA-1 · ORIENT-HINT-SCROLL-1 |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 17 | 0 | 8 | 8 (+1 partial) |
| [Art](#art) | 12 | 1 | 3 | 8 |
| [Audio](#audio) | 3 | 0 | 2 | 1 |
| [Design / Gameplay](#design--gameplay) | 9 | 0 | 3 | 6 |
| 🟢 [Playtest owed](#playtest-owed) | 8 | 0 | 6 | 2 |
| [UI / UX](#ui--ux) | 4 | 0 | 1 | 3 |
...
*(No open cards as of 08-07 — HOLE-FRICTION-COMBINE-1 and SPAWN-SUNDIAL-GAP-1 both PASSed.
Next work is Block 2. This line is an index, not a claim about the game: file a Block 1 card the
moment a player-facing correctness bug turns up.)*

**Block 2 — PRE-SHIP (should land before the public post).** Best-first among open Medium pre-ship work.
1. **ONBOARD-ART-1** — art rig landed; waiting on Wyatt's webp drops (`src/assets/howto/`) + AISLE 4 callout aim (`data-callouts="aimed"`).
2. **VOICE-BUS-1** — announcer volume of its own (**AUDIO-MASTER-1 closed 08-07**: the dead `_masterVol` was deleted — a voice slider is genuinely a new third bus).
3. **CHUNK-DEFER-1** — dynamic-import deferral of netcode / bootstrap / cartRaveGltf off the cold menu initial set (successor residual from closed CHUNK-MEMBER-1).
4. **NPC-BOOST-1** — measure session only; retune is a separate ack.
...
2. **BOOTH-RAIL-COL-1** — Storerooms booth rails visual-only (posts only if picked up).
3. **SUNDIAL-LOW-WATER-1** — Low still skips ocean normal; check cost on Intel min-spec box.
4. **Main-menu SFX slider** `[SHIP-1 E3]`
5. **UI-SCALE-P2-MEDIA-1** — Pass 2 media-query cleanup; re-check phone per file.
6. **ORIENT-HINT-SCROLL-1** — landscape stacked column + portrait-hint clip.
7. Opportunistic Low neighbors when idle: **MOTION-A11Y-1** (needs definition of done first) · **CARGO-LATCH-1** · **COUNTDOWN-QUICKPLAY-1** · **countdown survives menu return**.
...
| Medium | PATTERNS-UI-1 — Pattern customize UI `[SHIP-1 C3]` | Unblocked by the completed CART-MODEL-1 re-UV (08-09). |
...
| Medium | MENU-SWAP-FLASH-1 — paging arenas no longer flashes/redraws the menu `[solo]` | **Owed: Wyatt playtest — MENU-SWAP-FLASH-1 — paging arenas keeps the menu persistent; the arena crossfades underneath.** Shipped `6b10e2f` — the menu-preview swap now hides behind the opaque menu backdrop instead of fading the canvas, and the menu no longer re-themes per arena.<br>1. On the menu, page through all 3 arenas (Prev/Next arrows or LB/RB bumpers).<br>2. The menu chrome — title, buttons, stat colors, glows — stays put the whole time; only the arena name/subtitle updates.<br>3. The 3D arena behind fades in and out underneath the menu; there is never a dark/black flash of the whole screen.<br>4. FAIL if the menu visibly redraws, re-themes, or the screen flashes black on any arena swap. |
| Medium | CHUNK-DEFER-PT-1 — cold code-split still reaches every entry path `[solo]` | **Owed: Wyatt playtest — CHUNK-DEFER-PT-1 — after cold defer (netcode + cart GLTF off first paint), every entry path still works.** Desk levers L1a–L6 landed; initial set ≈ −176 KB. Hard-refresh production first. Hover Solo/Quickplay may warm chunks — that is fine; no black frame and no console module error is the bar.<br>1. Cold menu: hard-reload, land on title — menu paints, no red module errors in console.<br>2. Cold Solo: click Solo before sitting idle long — overlay covers load; reach carts-ready / drive; no black frame stuck.<br>3. Customize: open Customize — cart becomes the rave GLTF (brief SVG/placeholder OK); menu showcase cart on desktop Medium/High still mounts.<br>4. Harness: open `?harness=` (or nettest path you use) — boots without throw.<br>5. FAIL any step that hangs on a blank/black canvas, throws module-load errors, or never reaches carts-ready. |
| Medium | CHUNK-DEFER-PT-2 — invite + friends still form P2P after defer `[2pc]` | **Owed: Wyatt playtest — CHUNK-DEFER-PT-2 — invite join and friends round still connect after netcode dynamic import.** Two clients (two tabs or two machines). Production hard-refresh both.<br>1. Tab A: Friends → create room; note code.<br>2. Tab B: open `?room=CODE` (or type join) — seats in lobby, both drive when round starts.<br>3. Confirm KO feed / hit FX are not silent on either side (dead bridge = quiet, not crash).<br>4. FAIL if join hangs, no DataChannel drive, or KOs/FX missing on one side. |

## UI / UX

| Pri | Item | Notes |
|-----|------|-------|
| Medium | ONBOARD-ART-1 — HOW TO PLAY art: rig landed; drop-in webp + AISLE 4 aim gate `[pre-ship]` | **Art rig landed 08-07** — the deck now takes art as a drop-in directory: drop `<token>.webp` into [src/assets/howto/](../../src/assets/howto/README.md) (`drive` / `boost` / `ram` / `hud` / `cargo`, matching each slot's `data-media`) and that slot turns on at the next build; no file → the slot stays dark, so the deck reads exactly like the playtested text-only deck. Optional `<token>.still.webp` swaps in under `prefers-reduced-motion`. Kept under ~400 KB each. **Remaining:** Wyatt captures the webp by hand and drops them in, then AISLE 4's callouts get aimed: add `data-callouts="aimed"` to the hud slot in [index.html](../../index.html) once the real frame exists — the current `--x`/`--y` are guesses and stay hidden until then. **Do not shoot these with `npm run shoot`** — see SHOOT-SOFTGL-1; Wyatt captures by hand. |
...
| Low | CHUNK-DEFER-1 | Dynamic-import: defer `netcode` / `bootstrap` / `cartRaveGltf` off the cold menu initial set | **Desk complete 08-09** (L1a–L6; L3 skip). Initial set ~1,229 KB → ~1,053 KB; `netcode` + `cartRaveGltf` out of preload. **Owed human path matrix:** [CHUNK-DEFER-PT-1](#playtest-owed) · [CHUNK-DEFER-PT-2](#playtest-owed). Close this engineering row only after both PT cards PASS (or fail with new residual filed). |
...
CART-MODEL-1, MENU-CART-1, FRIENDS-LEVEL-1, FRIENDS-LEVEL-PT-1, ONBOARD-ATTRACT-1,
...
CART-COLOR-DEPTH-1, PAD-MENU-1, ARENA-BUMPER-HINT-1, UI-FRAME-1, ESC-PANEL-1,
CART-FORK-SWIVEL-1, KILLFEED-PHONE-1.
