...
| [Art](#art) | 10 | 1 | 2 | 7 |
| [Audio](#audio) | 3 | 0 | 2 | 1 |
| [Design / Gameplay](#design--gameplay) | 10 | 1 | 4 | 5 |
...
1. **NPC-BOOST-2** — restore visible charged NPC attacks and stop boost self-KOs. Wyatt's
   production playtest **FAILed** NPC-BOOST-PT-1: NPCs did not visibly charge and self-KOs rose.
   Local repair widens charge recognition, validates the boost launch runway on all intents, and
   carries charged-release mode to remote clients, and plays the NPC charge loop for the Solo host
   and Friends clients. QA/build and explicit ship authorization remain.

**Block 2 — PRE-SHIP (should land before the public post).** Best-first among open Medium pre-ship work.
1. **NPC-BOOST-1** — deployed; its shared [2pc] playtest **FAILed** and is superseded by NPC-BOOST-2.
2. **AI-EASY-SOFTEN-1** — deployed with NPC-BOOST-1; recheck with NPC-BOOST-2-PT-1.
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NPC-BOOST-2 — restore visible NPC charged attacks and stop boost self-KOs | **Local, not deployed.** NPC-BOOST-PT-1 **FAILed** in Wyatt's production playtest: NPCs did not visibly charge and self-KOs rose. The repair widens charge selection to Easy/Medium/Hard 9–12m/20°, 8–12m/25°, and 7–12m/30°; it rejects the 18.9m boost launch runway at Classic, octagon, and square-hole hazards; sends charged-release mode to remote clients; and plays the charge-up loop for the Solo host and Friends clients. QA/build and a fresh `ship it` remain. |
| Medium | NPC-BOOST-1 — NPCs use charged attacks and instant boosts by intent | **Deployed 08-10:** Worker `4e4fd25a-452b-40b6-adb8-c8e1c41d6bde`, commits `033bb8f` / `4e47e16`. **Production playtest FAIL 08-10:** superseded by NPC-BOOST-2; do not close from the old deployment. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NPC-BOOST-2-PT-1 — charged and instant NPC boosts `[2pc]` | **Blocked on a new deployment.** The prior NPC-BOOST-PT-1 is **FAIL**: no visible NPC charge-ups and more NPC self-KOs.<br>1. After deployment, hard-refresh a Solo round with three NPCs. PASS if each difficulty visibly produces charge-ups in safe attacks, with Easy least frequent and Hard most frequent, and the host hears each charge loop.<br>2. In a Friends room, view the same NPC charge and release from both clients. PASS only if both show the charge telegraph, hear the charge loop, and show the charged burst.<br>3. On Classic, drive NPCs near the outer rim and center hole; repeat on octagon and square-hole maps. PASS only if every unsafe charge cancels and every unsafe instant boost is skipped.<br>4. FAIL on any NPC boost self-KO, missing charge sound, boost-state desync, or reversed difficulty order. |
...
CART-MODEL-1, SHADES-MAT-1, MENU-CART-1, FRIENDS-JOIN-LAYOUT-1, FRIENDS-LEVEL-1, FRIENDS-LEVEL-PT-1, ONBOARD-ATTRACT-1,
ONBOARD-ART-1,
