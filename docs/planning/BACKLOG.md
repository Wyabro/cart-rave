...
| Pri | Item | Notes |
|-----|------|-------|
| High | NIGHT-SHIFT-CITY-1 — Night Shift visual-structure pass | **WYATT VISUAL PASS 08-11 — closure waits on global QA.** The working name is not final. Freeze the approved square roof, spawn locations, raised roofs, and AC physics. Lever A: deterministic visual plan for the main tower shell, corner-deck supports, rooftop penthouse masses, and near/mid/far skyline bands. Lever B: causal worn concrete/roofing and facade material maps; no pristine hero surfaces. Lever C: moon key, warm city depth, fog separation, and explicit Low versus Medium/High density. Use instancing/material batching; no per-window lights. Fixed capture: `.diag-captures/night-shift-lights-fixed.png`; facade-light repair `282c7e2`. Full QA remains red only on the unrelated exact-50-row backlog canary introduced with `48e4364`. |
