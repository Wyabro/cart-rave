...
(`npm run dev` until ship) · **KO-CENTER-RING-PT-1** (`npm run dev` until ship) ·
**CUSTOMIZE-SVG-FLASH-PT-1** (`npm run dev` until ship) ·
**FRIENDS-LOBBY-ORDER-PT-1** (`npm run dev` until VERIFY_OK) ·
...
| Medium | KO-CENTER-RING-PT-1 — when you get KO'd, only the red edge pulse flashes `[solo]` | **Owed: Wyatt playtest — KO-CENTER-RING-PT-1 — when you get KO'd, the only flash is the red edge pulse; no centered expanding ring.** Parent **KO-CENTER-RING-1**. Pushed but not yet deployed — `npm run dev` until ship.<br>1. Solo or Quickplay. Get KO'd by a ram or a hazard (fall off the edge works).<br>2. FAIL if a centered circle/ring expands from mid-screen. PASS if only the red edge pulse flashes.<br>3. In the same match, KO an opponent — FAIL if the center hitmarker is gone. PASS if it still shows. |
| Medium | CUSTOMIZE-SVG-FLASH-PT-1 — DONE/apply does not flash the legacy cart SVG `[solo]` | **Owed: Wyatt playtest — CUSTOMIZE-SVG-FLASH-PT-1 — DONE and apply do not flash the old cart drawing.** Parent **CUSTOMIZE-SVG-FLASH-1**. Pushed but not yet deployed — `npm run dev` until ship.<br>1. Menu. Open Customize. Pick a color. Press DONE.<br>2. FAIL if a flat cartoon cart drawing flashes while the screen closes. PASS if the 3D cart stays or the screen just closes.<br>3. Open Customize again. Change color. Stay on the screen. FAIL if the 3D cart turns into that flat drawing. PASS if the 3D cart updates in place. |
| Medium | FRIENDS-LOBBY-ORDER-PT-1 — Friends CHECKOUT LINE lists humans first, first-connect first `[2pc]` | **Owed: Wyatt playtest — FRIENDS-LOBBY-ORDER-PT-1 — Friends CHECKOUT LINE lists humans first, first-connect first.** Parent **FRIENDS-LOBBY-ORDER-1**. Playtest on prod after VERIFY_OK, or `npm run dev` until then.<br>1. Two PCs. Friends room. Host picks a cart color that is not the seat-0 bot color (or confirm host is not seat 0).<br>2. Guest joins. Both screens: FAIL if a bot sits above a human. PASS if humans are at the top, host above guest, bots under.<br>3. FAIL if a human's color or seat mark no longer matches their cart. PASS if color and mark still match the cart.<br>4. Not a FAIL: the small score list at 3-2-1 may still show seat order. |
