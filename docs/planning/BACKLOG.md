...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave A · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 9 playtest blockers |
...
| [Design / Gameplay](#design--gameplay) | 4 | 2 | 2 | 0 |
...
3. **NPC-SELFKO-3** — measure then reduce NPC self-KOs on all three arenas.
4. **SPAWN-BACKROOMS-2** — Storerooms spawns one spawn-width inward.
5. **FRIENDS-ROTATE-1** — Friends rematch rotates arenas, synced.
6. **ONBOARD-JUMP-1** — HOW TO PLAY matches jump+boost.
7. **ONBOARD-WEBP-1** — HOW TO PLAY WebP playback + fallback.
8. **MENU-CART-FOLLOW-1** — subtle menu-cart cursor parallax.
9. **SHARE-CARD-1** — update OG/social share image.
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave A active 08-20.** During ordinary driving, carts briefly pop/bounce off the ground with no jump and no obvious obstacle. Seen on all three arenas and in multiple modes — investigate shared cart physics, floor/collider contact, or suspension, not a level-specific lever. Measure first. Lane when picked: **Critical**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| High | NPC-SELFKO-3 — measure and reduce NPC self-KOs on all three arenas | **Filed 08-19.** Measure n=1 Medium 08-20: Cart Rave 1/6 (`center_hole`), Storerooms 6/6 (void 3 / pit 3), Sundial 30/33 (`outer_edge`). Aggressor 0 on every arena — do not cut hunt weight. **Sundial lever landed:** octagon TTE was band-gated at 5.25 m (0.22 s at maxSpeed); Classic rim TTE is ungated. Ungated TTE + Classic throttle brake. Re-soak 30→3. **Residual: Storerooms.** New evidence — do not blindly retune **AI-ARENA-SELFKO-1** / **STOREROOMS-NPC-SELFKO-2** numbers. Playtest **NPC-SELFKO-3-PT-1**. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Low | NPC-SELFKO-3-PT-1 — Sundial NPCs hunt and stay on the deck `[1pc]` | **Owed: Wyatt playtest — NPC-SELFKO-3-PT-1 — Sundial bots still hunt and do not farm the outer pit.** Parent **NPC-SELFKO-3**. Code landed, not deployed — `npm run dev`.<br>1. Solo Sundial Station, Medium bots, one full round.<br>2. FAIL if NPCs constantly drive themselves off the octagon with no ram, or if they refuse to chase you.<br>3. PASS if they still ram and unforced NPC falls are rare enough not to steal the round.<br>4. Storerooms self-KOs are a residual of this card, not this PT. |
