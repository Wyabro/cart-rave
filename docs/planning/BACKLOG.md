...
| **2** — PRE-SHIP (before public post) | 🟡 queued | **CUSTOMIZE-SPAM-1** landed; PT owed (**CUSTOMIZE-SPAM-PT-1**) |
| **3** — WYATT LANE (blocked on you) | ✅ drained | 08-17 **TRUST-1** · **LEADERBOARD-1** cut from V2 |
| **4** — PERF RESIDUAL (measure-first) | 🟡 queued | **WARM-CLASSIC-JUICE-1** · PERF-CLASSIC-IGPU-1 · WARM-SOLO-1 · PERF-WATCH-1 · NET-PERF-1 / NET-PERF-3 |
...
|---|---:|---:|---:|---:|
| [Engineering](#engineering) | 19 | 0 | 6 | 12 (+1 partial) |
...
1. **CUSTOMIZE-SPAM-1** — rapid Customize open/close remounts WebGL until the attract canvas dies (cap-370). Landed 08-17; playtest owed **CUSTOMIZE-SPAM-PT-1**.
...
1. **WARM-CLASSIC-JUICE-1** — cap-371 Friends Classic join overlay freeze (real telemetry).
2. **PERF-CLASSIC-IGPU-1** — Classic ~30 fps on Intel UHD; not triangles (PERF-PASS-1).
3. **WARM-SOLO-1** — only on real weak-GPU telemetry.
4. **PERF-WATCH-1** — auto-quality step-up path decision.
5. **NET-PERF-1** residual / **NET-PERF-3** — only if F8 still shows rubber-band / alloc pressure.
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | CUSTOMIZE-SPAM-1 — rapid Customize open/close remounts WebGL and kills attract `[solo]` | **Landed 08-17** — keep-alive pause/resume; one Customize context per menu session; generation token blocks stacked imports; `release()` on menu-exit. Do not reopen **ATTRACT-JANK-1** or **CUSTOMIZE-PERF-1**. Distinct from **CUSTOMIZE-SVG-FLASH-1**. Playtest owed: **CUSTOMIZE-SPAM-PT-1**. |
| Medium | WARM-CLASSIC-JUICE-1 — Friends/QP join to Classic freezes the overlay ~10 s on iGPU | **Filed 08-18 from cap-371** (Intel UHD, Low, `?diag=1&room=ROLL2`, Friends non-host). **WARM-QP-ROTATE-1** adopt already ran (`play-arena-adopt` zanzibar→classicRecord). Residual: first Classic play compile is 451 materials / 3.1 s + 2.8 s flyover poll after carts spawn; Low still `initLasers` then hides them. Lever: warm adopted arena (no flyover) before carts, consume `juiceFresh`, skip laser init when `laserBudget === "off"`. Do not reopen **WARM-QP-ROTATE-1** or **WARM-IGPU-1**. Distinct from **WARM-SOLO-1** (countdown first-draw) and **PERF-CLASSIC-IGPU-1** (sustained 30 fps). Playtest owed: **WARM-CLASSIC-JUICE-PT-1**. |
| Medium | PERF-CLASSIC-IGPU-1 — Classic holds ~30 fps on Intel UHD after rsm 0.7 | **Filed 08-18 from cap-371** (same session as WARM-CLASSIC-JUICE-1). Friends Classic 33.8 fps vs Sundial 51–55 fps; Classic has *fewer* draws (285 vs 310). PERF-PASS-1 Wave 3 falsified triangle cuts (−0.12 ms for −220k tris). rsm already 0.7. Low already has fill lights / sky / lasers / crowd-anim off. First probe if picked: record body Physical→Standard (Wave 5 unproven). Do not reopen **PERF-PASS-1**. Wyatt sign-off on any look change. |
...
| Pri | Item | Notes |
|-----|------|-------|
| Medium | WARM-CLASSIC-JUICE-PT-1 — Friends join to Classic does not freeze the overlay `[2pc]` | **Owed: Wyatt playtest — WARM-CLASSIC-JUICE-PT-1 — Friends join to a Classic room does not freeze the overlay ~10 s.** Parent **WARM-CLASSIC-JUICE-1**. Not on prod yet — `npm run dev` or wait for deploy. Intel UHD / Low. Use `?diag=1`.<br>1. Host: Friends room, pick CART RAVE (Classic). Guest: join the code on the Intel box.<br>2. FAIL if the loading overlay locks for many seconds (page dead, meter stuck). PASS if the overlay keeps painting and CHECKOUT LINE appears without a ~10 s freeze.<br>3. Ready up. FAIL if 3-2-1 skips digits. PASS if countdown is even. Low Classic still has no lasers; billboard stays. |
