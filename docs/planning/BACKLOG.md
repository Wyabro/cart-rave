...
| Pri | Item | Notes |
|-----|------|-------|
| High | NET-LAG-1 — Friends/QP lag + rubber-band before playtest | **Wave 1 landed 08-20.** Cause: NH-SMOOTH v3 display low-pass (`displayPosRate` 14) trailed ~1.6 m on a clean wire (cap-373/374 vs host 375, room `SAGO6`). Lever: non-host local mesh+camera **copy** the physics pose. Transport / host send / rewind-replay not this wave. **NET-LAG-1-PT-1 PASS 09-14** (trail gone). Residual open per Wyatt 09-14 note: significant non-host lag drags experience, looks amateur — next wave measures input delay path. Do not restore lerp or add a catch-up distance gate on a vibration FAIL without a new ack. Do not reopen **NET-1** · **NET-2** · **NET-MIG-3**. |
