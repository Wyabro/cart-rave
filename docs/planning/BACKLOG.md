...
|-------|-------|-------------|
| **1** — NOW (player-facing correctness) | 🟡 active | **CART-POP-1** Wave E orientation trace · **CART-POP-PT-1** FAIL / remains open · **NET-LAG-1-PT-1** `[2pc]` parked by Wyatt · 8 playtest blockers |
...
| High | CART-POP-1 — carts pop off the floor in normal driving | **Wave D read 08-20.** Production cap-388 (`b4ba0125`, Cart Rave) has 20 local rises at radius 5.43–21.29 m. Sixteen span adjacent wedges, but four single-floor supports have the highest mean rise (+5.42) and peaks `−8.64 → +2.41` / `−6.91 → +1.91`; wedge seams are not required. Wave C already rules out control code and restitution. Candidate is free pitch/roll or another cart-vs-single-floor solver interaction. **CART-POP-PT-1 remains FAIL/open.** Wave E must trace pre/post solver up-dot and angular velocity before a physics change. Lane: **Critical**. |
