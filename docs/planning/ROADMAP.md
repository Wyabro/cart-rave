...
| Netcode audit follow-through | Code-complete, pending smoke | Host transform, round duration, SD timeout, remote boost, slot resurrection, ram FX dedup, `isSuddenDeath`, clock-offset timer — statically verified. July 10 regression audit also cleared solo rubberband MP leak, hop-land broadcast double-thud, directive snapshot self-heal — still need two-browser confirmation. |
