...
| # | Item | Notes |
|---|------|-------|
| A1 | Host hitch forensics → capability-based host preference | **Tab-out latch VALIDATED 07-21** — cap-122 caught a real 6.55s backgrounding cleanly (`hiddenDuringGap:true`), confirming all prior "not backgrounding" readings. **Round-start jank (COUNTDOWN-WARM-1) root-caused and fixed**, pending Wyatt's playtest — see BACKLOG. Original chronic-host-freeze question (Intel-as-host) still open: 4 sessions in a row picked the 4090 as host; still zero Intel-as-host captures. |
