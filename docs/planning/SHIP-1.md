...
| # | Item | Notes |
|---|------|-------|
| A1 | Host hitch forensics → capability-based host preference | **First real verdict 07-20, still open.** Fresh F8 pass with the latch fields: a genuine 22.1s host-adjacent freeze had `hiddenDuringGap:false, blurredDuringGap:false` — ruling out "just a mislabeled backgrounded tab." Long Task coverage was only ~4.5% (idle, not busy) — ruling out "too much synchronous JS/physics work" too. Points toward a GPU-driver/OS-scheduling stall. Only 2 captures from one match, and Intel was non-host that round — get an Intel-as-host capture on build `2293b57`+ before committing to HOST-CAP-1. |
| A2 | INPUT-KB-1 — keyboard parity | **Done 07-20, pushed, deploy pending.** Wyatt clarified: the real ask was driving *feel*, not menu access. Root cause: `simulation.js` reads `axis.turn`/`axis.forward` as an analog deflection (desiredYawRate, drift grip, drift impulse all scale directly off it) — gamepad's stick is continuous, keyboard's old `getAxis()` was literal -1/0/1, so every A/D tap was an instant full-lock max-rate turn with full drift grip. Fixed with a digital-to-analog ease in `src/input.js` (0.14s attack / 0.09s release ramp) instead of touching physics tuning. Also landed: arrow-key menu nav (`gamepadNav.js`) and the UI-active driving-suppression parity bug found along the way. Tests: `gamepadNav.test.js` +5, `input.test.js` +7 (exact ramp math). `npm run qa` 641/641. **Feel change — needs Wyatt's own playtest**, not self-certifiable. |
