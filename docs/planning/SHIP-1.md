...
| # | Item | Notes |
|---|------|-------|
| A1 | Host hitch forensics → capability-based host preference | **Three rounds of data, still open.** Round 1: genuine 22.1s freeze on Intel-non-host, ruled out mislabeled backgrounding and sync JS/physics work. Round 2 + round 3 (07-21) both reproduce the *same* early-round (t≈11-14s) stall cluster on the **4090-as-host**, with real Long Task backing (up to 2.4s, ~99% attribution on one gap) — a repeated pattern now, pointing at a one-time boot/init cost (closer to BOOT-PERF-1) over a chronic host-selection problem. All three sessions had the 4090 as host — still zero Intel-as-host captures; party server keeps picking the same machine first. Wyatt also tabbed out mid-session as a real test of the `hiddenDuringGap` fix, but neither round-3 capture's retained window (each ~2 min, no ring eviction) shows it — likely happened after both F8 presses, not a confirmed latch failure. Ask: F8 shortly after a tab-out next time to actually validate it. |
