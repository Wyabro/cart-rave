...
| A4 | ARENA-COL-1 — Cart Rave pit KO detection | 📋 **Landed locally (unpushed)** — rim entry pose (`fallEntryPos`) & round-clock timestamp (`fallEntryTimeMs`) captured at `FALL_ENTRY_Y = -2.0` passed via `{ classifyPos, creditTimeMs }` options object to `buildKOEvent`. Prevents 30m drop drift misclassification and deep-shaft attribution expiry. Tests: `scoringEvent.test.js` +2 (27/27 pass). Awaiting Wyatt playtest. |
