...
| # | Item | Notes |
|---|------|-------|
| A1 | Host hitch forensics → capability-based host preference | **Two rounds of data, still open, new lead.** Round 1: genuine 22.1s freeze on Intel-non-host, `hiddenDuringGap:false`/`blurredDuringGap:false` — rules out mislabeled backgrounding; ~4.5% Long Task coverage rules out sync JS/physics work. Round 2 (07-21): Intel-non-host was mild (max 438ms), but the **4090-as-host** hit a serious stall cluster **early in the round** — single Long Tasks up to 2361ms, high attribution (~99% on one gap) — then ran clean for the rest of a 14-minute session. Points at a one-time boot/init cost (closer to BOOT-PERF-1) rather than a chronic host-selection problem. Both sessions had the 4090 as host — still zero Intel-as-host captures; party server keeps picking the same machine first. |
| A2 | INPUT-KB-1 — keyboard parity | **Tuned 07-21, deploy pending.** First pass (0.14s/0.09s ramp) read as "too controller-y" — halved to **0.07s attack / 0.05s release** in `src/input.js`, still removing the instant-full-lock snap but closer to keyboard-native response; internal dt safety clamp tightened 0.1s→0.05s to stay below the new attack window. Root cause (unchanged from first pass): `simulation.js` reads `axis.turn`/`axis.forward` as an analog deflection — gamepad's stick is continuous, keyboard was literal -1/0/1. Also landed: arrow-key menu nav (`gamepadNav.js`) and a UI-active driving-suppression parity fix. Tests: `gamepadNav.test.js` +5, `input.test.js` 7 (updated for new constants). `npm run qa` 641/641. **Feel change — needs another Wyatt playtest.** |
