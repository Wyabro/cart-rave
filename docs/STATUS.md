...
| # | What | Status |
|---|------|--------|
| **STORE-WALL-SLIDE-1** | **Storerooms walls grab instead of letting a cart scrape past** | ✅ **DEPLOYED 08-05 (`f28268b`) · Version `a9dfef85`, prod SHA + all 5 rule calls verified — playtest owed.** Third in the Storerooms pit chain (STORE-PLAT-WALL-1 PASS → STORE-PIT-WEDGE-1 PASS → this). **Rapier averages collider friction and the cart carries 1.1**, so a wall written 0.05 acted like 0.575 and the perimeter (0.4) like 0.75 — the stickiest surface in the room. `FrictionCombineRule.Min` on all five vertical surfaces + perimeter 0.4 → 0.15. Friction twin of the phantom-bounce bug Sundial fixed with `RestitutionCombineRule.Min` ([zanzibarPlatform.js:25](../src/levels/zanzibarPlatform.js:25)). Floors keep Average on purpose. [BACKLOG § Playtest owed](./planning/BACKLOG.md). |
