...
**BOOST-SFX-RESPAWN-1** applied 08-13, **unshipped** — charge-up SFX is no longer swept by other carts' respawns. `doRespawn` stopped calling `stopAllSfx("chargeUp")`; the global sweep cut the local player's live charge loop whenever any NPC/remote cart respawned (NPCs have played chargeUp since 08-10). Respawn cleanup is per-cart by id (`resetCartTransientState`); round-boundary sweep + rematch nuclear remain. Contract test rewritten with unique-Howler-id mock (red→green), QA 7/7 PASS. Owed check: **BOOST-SFX-RESPAWN-PT-1**. Non-host follow-up filed: **BOOST-SFX-NONHOST-1**.
