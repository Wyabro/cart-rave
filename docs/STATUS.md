...
| *(closed)* | FIX-MIG — disconnect migration visibility + continuous tests | ✅ **PASS 08-05** on prod `a65d3c9` (FIX-MIG-PT-1). First FAIL was bare `host_migrated` (warm DO / missing reason); client treats bare A→B as disconnect. [completed-work.md](./planning/completed-work.md). |
