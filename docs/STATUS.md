...
| # | What | Status |
|---|------|--------|
| DEV-GRAPH-2 | Hardened maker and Luna reviewer graph | 🟡 **ACTIVE 08-12.** Wave 1 hardens the logical lock and binds a fixed plan-only maker receipt. No live maker or reviewer stage has started. |
...
1. **DEV-GRAPH-2** — Wave 1 local implementation is ready for the reviewer boundary. No live maker or reviewer stage has started.
