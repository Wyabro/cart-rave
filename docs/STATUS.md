...
| # | What | Status |
|---|------|--------|
| **STORE-PIT-WEDGE-1** | **Carts wedge in the Storerooms pit band** | ✅ **DEPLOYED 08-05 (`152d835`) · Version `c5711dd4`, prod SHA verified — playtest owed.** Caused by STORE-PLAT-WALL-1's own fix (PASS 08-05, `8ec9e3a` — the arena cliff stops carts now): sealing the cliff made the pit band a corridor walled on both sides, with the gondolas 2.0 m off the inner one and a cart diagonal of 2.83 m. Gondola 9 → 7, band centre 45.5 → 46.7 = **4.2 m each side**; dressing friction 0.4 → 0.05 so the tops shed instead of hold. **Pushing the band outward alone rebuilds the wedge against the room wall** — the test pins both gaps against `CONFIG.cart.size`. [BACKLOG § Playtest owed](./planning/BACKLOG.md). |
