...
- **Sunglasses**: one-piece visor GLB integrated at source load (`cartRaveGltf.js integrateOnePieceSunglasses`; master `art/models/sunglasses-visor.glb`, runtime 4.7 KB draco). Placement baked on the mesh (body-scale reparents face meshes by position); mirror styles/lens envMap/face policy unchanged. Verified in customize preview. **Wyatt-corrected same day (×2)**: `tripo_part_7` is the cart's SMILE, not a glasses accent — it now stays authored; the visor replaces only frame 8 + lenses 9/11. Coverage tuning: width +18% over the box fit (the hole wraps around the basket sides) and an independent ~9% vertical stretch to span the frame box's full height, centered on the hole (the basket body has a hole behind the glasses footprint — `tripo_part_0` re-mesh is the eventual long-term fill, unneeded while the visor covers it).
