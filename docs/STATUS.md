...
| # | What | Status |
|---|------|--------|
| **STORE-WALL-SLIDE-1** | **Storerooms walls grab instead of letting a cart scrape past** | ✅ **CODE LANDED 08-05 (`f28268b`), unshipped — playtest owed.** Third card in the Storerooms pit chain (STORE-PLAT-WALL-1 PASS → STORE-PIT-WEDGE-1 PASS → this). **Rapier averages collider friction by default and the cart carries 1.1**, so a wall written 0.05 acted like 0.575 and the perimeter walls (0.4) like 0.75 — the stickiest surface in the room. `FrictionCombineRule.Min` on all five vertical surfaces + perimeter 0.4 → 0.15. **Friction twin of the phantom-bounce bug Sundial fixed with `RestitutionCombineRule.Min`** ([zanzibarPlatform.js:25](../src/levels/zanzibarPlatform.js:25)); nobody had done the friction side. Floors keep Average on purpose. [BACKLOG § Playtest owed](./planning/BACKLOG.md). |
