...
| # | What | Status |
|---|------|--------|
| **STORE-PLAT-WALL-1** | **Storerooms pit cliff had no collider** | ✅ **DEPLOYED 08-05 (`8ec9e3a`) · Version `251c51e4` · chunk `backroomsSupermarket-DSK1Rh_H.js`, prod SHA verified — playtest owed.** The filing named the wrong geometry (booth rails); Wyatt's screenshot was at pit-floor level. Real bug: `addCliffRing(ARENA_HALF)` drew the 26 m arena cliff as plain meshes with zero colliders, so a cart in the pit drove under the playfield. Fixed with an exported spec + four cuboids on `buildFallContainment`'s body, top tucked `CHAMFER_TUCK` above `FLOOR_BOTTOM_Y` to avoid a coplanar seam. [BACKLOG § Playtest owed](./planning/BACKLOG.md). |
