...
> Generated 2026-08-13 at commit `ff8175b` on `cart-clash`. If docs/STATUS.md's digested sections have changed since, `npm run briefing:check` (inside `npm run qa`) fails until this is regenerated.
> Source digest: `f4ccfa6e`
...
Playtest PA-COMBO-PT-1 on `npm run dev:local` (not prod)
