...
> Generated 2026-08-17 at commit `fcdde64b` on `cart-clash`. If docs/STATUS.md's digested sections have changed since, `npm run briefing:check` (inside `npm run qa`) fails until this is regenerated.
> Source digest: `d3d2b952`
...
Playtest THOST-CEILING-PT-1 · ZOMBIE-HOST-PICK-PT-1 · GAMEPAD-FREEZE-PT-1 ([pad]) after ship. NPC-ABORT-BURST-PT-1 on `npm run dev` until ship. Deferred: SHARD-PT-2 (launch day) · WARM-QP-ROTATE-PT-1
