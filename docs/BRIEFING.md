...
> Generated 2026-08-11 at commit `d027a19` on `cart-clash`. If docs/STATUS.md's digested sections have changed since, `npm run briefing:check` (inside `npm run qa`) fails until this is regenerated.
> Source digest: `8c045eeb`
...
NIGHT-SHIFT-BLOCKOUT-1: in Solo Dev, load `?level=rooftop`; check the four spawn arms, two high roofs, and four corner gaps

Plan → Wyatt ack → apply. This heading names the card; it is **not** permission to edit.

Self-directed queue (one at a time, within the declared phase):
- **NIGHT-SHIFT-BLOCKOUT-1** Night Shift rooftop blockout — 🔧 Local code and gates pass. Four corner voids, two high roofs on inset utility plinths, inert AC markers,…
