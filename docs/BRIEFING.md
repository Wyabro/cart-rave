...
> Generated 2026-08-17 at commit `15fd1eb0` on `cart-clash`. If docs/STATUS.md's digested sections have changed since, `npm run briefing:check` (inside `npm run qa`) fails until this is regenerated.
> Source digest: `754ea9a7`
...
Playtest NPC-ABORT-BURST-PT-1 · LAST-STANDING-DEAD-PT-1 on `npm run dev` until ship. REMOTE-INPUT-STALE-PT-1 after ship (`[2pc]`, no `?perfPump` on the non-host). Deferred: SHARD-PT-2 (launch day). New engineering: COUNTDOWN-HOST-STAMP-1
