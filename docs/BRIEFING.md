...
> Generated 2026-08-14 at commit `b3ca563` on `cart-clash`. If docs/STATUS.md's digested sections have changed since, `npm run briefing:check` (inside `npm run qa`) fails until this is regenerated.
> Source digest: `827266ca`
...
STORE-PILE-2 — bounce must cancel inward velocity. Wait for plan ack, then one lever. Retest STORE-PILE-PT-1
