...
> Generated 2026-08-17 at commit `30920e8a` on `cart-clash`. If docs/STATUS.md's digested sections have changed since, `npm run briefing:check` (inside `npm run qa`) fails until this is regenerated.
> Source digest: `db93989e`
...
Playtest COUNTDOWN-HOST-STAMP-PT-1 on `npm run dev` until ship
