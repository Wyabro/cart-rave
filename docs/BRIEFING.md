...
> Generated 2026-08-10 at commit `42c4e9c` on `cart-clash`. If docs/STATUS.md's digested sections have changed since, `npm run briefing:check` (inside `npm run qa`) fails until this is regenerated.
> Source digest: `f3c7639d`
...
NOW: NPC-BOOST-2 — restore visible NPC charged attacks and stop boost self-KOs. Code is local; QA/build and a new explicit ship authorization remain
