...
> Generated 2026-08-11 at commit `e139bb6` on `cart-clash`. If docs/STATUS.md's digested sections have changed since, `npm run briefing:check` (inside `npm run qa`) fails until this is regenerated.
> Source digest: `bf47eea8`
...
NIGHT-SHIFT-BLOCKOUT-1: in Solo Dev, load `?level=rooftop`; confirm the roof stays still, all four carts start on supported platforms, then test both roof launches and the stronger center launch

Plan → Wyatt ack → apply. This heading names the card; it is **not** permission to edit.

Self-directed queue (one at a time, within the declared phase):
- **NIGHT-SHIFT-BLOCKOUT-1** Night Shift rooftop playable blockout — 🔧 Local correction in progress. Rooftop visuals are static; four elevated spawn slabs match the cart spawn…
