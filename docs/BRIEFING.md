...
> Generated 2026-08-15 at commit `f92f918` on `cart-clash`. If docs/STATUS.md's digested sections have changed since, `npm run briefing:check` (inside `npm run qa`) fails until this is regenerated.
> Source digest: `47adb17a`
...
STORE-PILE-PT-1 — STORE-PILE-2 is on prod. Hard-refresh, then retest the same card
