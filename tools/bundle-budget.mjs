 ...
 *   2. set MEMBERSHIP — a chunk that newly ENTERS the preload set fails even when the byte
 *      total looks fine. That is the signal that catches someone re-eagering a module a
 *      code-split had deferred. A chunk LEAVING the set is an improvement: reported, never fatal.
 *
 ...
 * @returns {{
 *   files: {file: string, key: string, raw: number, gzip: number, kind: "entry"|"preload"}[],
 *   totalRaw: number, totalGzip: number, count: number, missing: string[],
 * }}
 */
export function analyzeInitialSet(html, sizeByFile) {
  /** @param {string} href */
  ...
  for (const ref of refs) {
    ...
    usedKeys.add(key);
    files.push({ file: ref.file, key, raw: size.raw, gzip: size.gzip, kind: ref.kind });
  }
 ...
 * @param {{initialSet: {raw: number, gzip: number}, chunks: Record<string, {raw: number, gzip: number}>}} baseline
 */
export function compareToBaseline(analysis, baseline) {
  ...
  const entered = analysis.files.map((f) => f.key).filter((k) => !(k in baseChunks));
  ...
  return {
    ...
    left,
    deltas,
    failed: overBudget || entered.length > 0,
  };
}
...
if (isMain) {
  ...
  const analysis = analyzeInitialSet(html, measureDistAssets());
  ...
  let baseline;
  try {
    baseline = JSON.parse(readFileSync(resolve(BASELINE_PATH), "utf8"));
  } catch {
